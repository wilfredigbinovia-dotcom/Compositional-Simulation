...
class CompositionalSimulator1D:
    ...
    def pressure_update(self, state: SimulationState, dt_days: float) -> np.ndarray:
        ...
        if injector is not None and self.scenario.name != "natural_depletion" and injector.rate_lbmol_day > 0.0:
            ...
            # pressure, then cap it so gas cycling provides meaningful pressure
            # maintenance without creating unrealistic pressure blow-up.
            raw_pinj_equiv = p_old[ii] + 1.0e-4 * injector.rate_lbmol_day / max(Jin, 1e-12)
...
def display_streamlit_app() -> None:
    ...
    with result_tabs[2]:
        figs = [
            ...
            make_line_figure(time_axis, [(np.asarray(history.well_effective_wi), "Effective Well Index")], "Date", "Effective Well Index", "Effective Well Index versus Time"),
        ]

        s1, s2 = st.columns(2)
        s1.metric("Initial Effective Well Index", f"{history.well_effective_wi[0]:.5f}")
        s2.metric("Final Effective Well Index", f"{history.well_effective_wi[-1]:.5f}")
