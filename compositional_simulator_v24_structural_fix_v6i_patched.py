...
def generate_chapter4_summary(history: SimulationHistory) -> List[List[object]]:
    ...
    def initial_value(values: List[float]) -> float:
        if not values:
            return float("nan")
        if not history.time_days:
            return float(values[0])
        t_arr = np.asarray(history.time_days, dtype=float)
        v_arr = np.asarray(values, dtype=float)
        n = min(len(t_arr), len(v_arr))
        if n == 0:
            return float(values[0])
        idx0 = int(np.argmin(np.abs(t_arr[:n])))
        return float(v_arr[idx0])
    ...
    wi_min, wi_min_t = min_with_time(history.well_effective_wi)
    ...
    return [
        ...
        ["Effective Well Index (-)", initial_value(history.well_effective_wi), wi_min, wi_min_t, final_value(history.well_effective_wi)],
...
def display_streamlit_app() -> None:
    ...
    with result_tabs[0]:
        ...
        c16.metric("Initial Effective Well Index", f"{history.well_effective_wi[0]:.5f}")
        c17.metric("Final Effective Well Index", f"{history.well_effective_wi[-1]:.5f}")
    ...
    with result_tabs[2]:
        ...
        wi1.metric("Initial Effective Well Index", f"{history.well_effective_wi[0]:.5f}")
        wi2.metric("Final Effective Well Index", f"{history.well_effective_wi[-1]:.5f}")

        fig_wi = make_line_figure(time_axis, [(np.asarray(history.well_effective_wi), "Effective Well Index")], "Date", "Effective Well Index (-)", "Effective Well Index versus Time")
        st.pyplot(fig_wi)
        plt.close(fig_wi)
