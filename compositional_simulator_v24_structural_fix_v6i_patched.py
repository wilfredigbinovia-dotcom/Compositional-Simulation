...
def _default_composition_dataframe(component_names: List[str], values_fraction: np.ndarray, basis: str) -> pd.DataFrame:
    ...
    return pd.DataFrame([{name: value for name, value in zip(component_names, display_values)}])
...
def _read_composition_dataframe(df: pd.DataFrame, component_names: List[str], basis: str) -> np.ndarray:
    ...
    missing_columns = [name for name in component_names if name not in work_df.columns]
    if missing_columns:
        raise ValueError(f"Missing composition columns: {', '.join(missing_columns)}")

    values = pd.to_numeric(work_df.iloc[0][component_names], errors="coerce").to_numpy(dtype=float)
...
def _parse_pasted_composition_text(raw_text: str, component_names: List[str]) -> list[float] | None:
    ...
    if len(tokens) != len(component_names):
        raise ValueError(
            f"Paste exactly {len(component_names)} numeric values for: {', '.join(component_names)}"
        )
    return tokens
...
def _composition_editor(
    ...
    if st.button("Apply pasted values", key=f"{editor_key}_apply_paste"):
        ...
        if pasted_values is not None:
            st.session_state[state_df_key] = pd.DataFrame([{name: value for name, value in zip(component_names, pasted_values)}])
            st.session_state[state_basis_key] = basis
            st.success("Composition values pasted into the table.")

    column_config = {}
    for component in component_names:
        column_config[component] = st.column_config.NumberColumn(
            component,
