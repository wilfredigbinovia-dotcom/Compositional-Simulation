...
class CompositionalSimulator1D:
    ...
    def solve_gas_rate_control(self, state: SimulationState, tol_mmscf_day: float = 1e-4, max_iter: int = 60) -> Dict[str, float | np.ndarray]:
        ...
        feasible, profile = self._lift_profile_ok(state, candidate)
        if feasible:
        ...
        # Target rate is not lift-feasible. Fall back to the highest-rate solution
        # that still satisfies the minimum liftable tubing head pressure.
        pwf_low = float(candidate["pwf_psia"])
        pwf_high = max(pr - 1e-6, pwf_low + 1e-6)
        resp_high = self.well_response_at_pwf(state, pwf_high)
        feasible_high, profile_high = self._lift_profile_ok(state, resp_high)
        ...
        for _ in range(max_iter):
            ...
            resp_mid = self.well_response_at_pwf(state, pwf_mid)
            feasible_mid, profile_mid = self._lift_profile_ok(state, resp_mid)
            if feasible_mid:
                best_resp = resp_mid
