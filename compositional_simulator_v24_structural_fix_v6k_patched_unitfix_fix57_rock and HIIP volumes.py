...
class CompositionalSimulator1D:
    ...
    def pressure_update(self, state: SimulationState, dt_days: float) -> np.ndarray:
        ...
        # Producer well coupling (Peaceman WI * mobility * BHP coupling scale)
        _WELL_PROD_SCALE = 2.5e-3
        wi = self.well.cell_index
        well_mob = self.phase_mobility_data(state, wi)
        Jw = self.peaceman_well_index(wi) * well_mob["lam_t"] * _WELL_PROD_SCALE
        response = self.solve_well_control(state)
    ...
    def transport_update(self, state: SimulationState, dt_days: float) -> TransportResult:
        ...
        for _ in range(n_sub):
            ...
            ws = self.well_sink(work_state)
            q_total = ws.q_total_lbmol_day
            ...
            wi = self.well.cell_index
            ncomp_new[wi] -= dt_sub * ws.hc_sink
            nw_new[wi] -= dt_sub * ws.qw_lbmol_day
...
def display_streamlit_app() -> None:
    ...
    with tab_producer_well:
        ...
        wellhead_temperature_R = fahrenheit_to_rankine(wellhead_temperature_F)
        tubing_model_label = st.selectbox("Tubing hydraulics model", ["Mechanistic proxy", "Simple coefficient"], index=0, key="tubing_model_label")
        tubing_model = "mechanistic" if tubing_model_label == "Mechanistic proxy" else "simple"
        thp_friction_coeff = st.number_input("THP friction coefficient", min_value=0.0, max_value=1.0, value=0.02, step=0.005, help="Retained as an empirical backpressure term and as the main loss term under Simple coefficient mode.", key="thp_friction_coeff")
