...
class CompositionalSimulator1D:
    ...
    def tubing_pressure_profile_from_response(self, state: SimulationState, response: Dict[str, float | np.ndarray]) -> Dict[str, float]:
        ...
        if self.well.tubing_model == "simple":
            ...
            thp_raw = pwf - hydrostatic_psi - friction_psi - accel_psi
            thp_clipped = float(max(thp_raw, 14.7))
            return {
                "estimated_thp_psia": thp_clipped,
                "estimated_thp_raw_psia": float(thp_raw),
                "tubing_pressure_clipped": float(1.0 if thp_raw < 14.7 else 0.0),
                "lift_feasible_flag": float(1.0 if thp_raw >= 14.7 else 0.0),
        ...
        def segment_state_props(p_seg_psia: float, t_seg_R: float) -> Dict[str, float | np.ndarray | str]:
            ...
            qg_seg_lbmol_day = hc_total_lbmol_day * beta_seg
            qo_seg_lbmol_day = hc_total_lbmol_day * (1.0 - beta_seg)
        ...
        return {
            "estimated_thp_psia": float(max(thp_raw, 14.7)),
            "estimated_thp_raw_psia": float(thp_raw),
            "tubing_pressure_clipped": float(1.0 if thp_raw < 14.7 else 0.0),
            "lift_feasible_flag": float(1.0 if thp_raw >= 14.7 else 0.0),
            "tubing_hydrostatic_psi": float(max(hydrostatic_psi, 0.0)),
    ...
    @staticmethod
    def tubing_profile_is_lift_feasible(profile: Dict[str, float | np.ndarray], min_thp_psia: float = 14.7, margin_psia: float = 1.0) -> bool:
        raw_thp = float(profile.get("estimated_thp_raw_psia", profile.get("estimated_thp_psia", min_thp_psia)))
        return raw_thp >= (min_thp_psia + margin_psia)
    ...
    def solve_gas_rate_control(self, state: SimulationState, tol_mmscf_day: float = 1e-4, max_iter: int = 60) -> Dict[str, float | np.ndarray]:
        ...
        resp_lo_with_profile = dict(resp_lo)
        resp_lo_with_profile.update(profile_lo)
        resp_lo_with_profile["gas_rate_target_feasible"] = float(1.0 if self.tubing_profile_is_lift_feasible(profile_lo) else 0.0)

        resp_hi = self.well_response_at_pwf(state, pwf_hi)
        profile_hi = self.tubing_pressure_profile_from_response(state, resp_hi)
        resp_hi_with_profile = dict(resp_hi)
        resp_hi_with_profile.update(profile_hi)
        resp_hi_with_profile["gas_rate_target_feasible"] = float(1.0 if self.tubing_profile_is_lift_feasible(profile_hi) else 0.0)

        if not self.tubing_profile_is_lift_feasible(profile_hi):
            resp_hi_with_profile["gas_rate_target_achieved_mmscf_day"] = float(gas_lbmol_day_to_mmscf_day(resp_hi_with_profile["qg_lbmol_day"]))
            resp_hi_with_profile["gas_rate_target_requested_mmscf_day"] = float(self.well.target_gas_rate_mmscf_day)
            return resp_hi_with_profile

        feasible_resp = resp_lo_with_profile
        if not self.tubing_profile_is_lift_feasible(profile_lo):
            lo = pwf_lo
            hi = pwf_hi
            for _ in range(max_iter):
                pwf_mid = 0.5 * (lo + hi)
                ...
                feasible_mid = self.tubing_profile_is_lift_feasible(profile_mid)
                if feasible_mid:
                    feasible_resp = dict(resp_mid)
                    feasible_resp.update(profile_mid)
                    feasible_resp["gas_rate_target_feasible"] = 1.0
                    hi = pwf_mid
                else:
                    lo = pwf_mid
            feasible_pwf = float(feasible_resp["pwf_psia"])
        else:
            feasible_pwf = pwf_lo

        qg_feasible_max = float(feasible_resp["qg_lbmol_day"])
        feasible_resp["gas_rate_target_requested_mmscf_day"] = float(self.well.target_gas_rate_mmscf_day)
        feasible_resp["gas_rate_target_achieved_mmscf_day"] = float(gas_lbmol_day_to_mmscf_day(qg_feasible_max))

        if target_qg_lbmol_day >= qg_feasible_max:
            feasible_resp["gas_rate_target_feasible"] = 0.0
            return feasible_resp
        ...
        profile_best = self.tubing_pressure_profile_from_response(state, best)
        best.update(profile_best)
        best["gas_rate_target_requested_mmscf_day"] = float(self.well.target_gas_rate_mmscf_day)
        best["gas_rate_target_achieved_mmscf_day"] = float(gas_lbmol_day_to_mmscf_day(best["qg_lbmol_day"]))
        best["gas_rate_target_feasible"] = float(1.0 if self.tubing_profile_is_lift_feasible(profile_best) else 0.0)
        return best
    ...
    def enforce_gas_rate_target(self, response: Dict[str, float | np.ndarray]) -> Dict[str, float | np.ndarray]:
        ...
        if float(response.get("gas_rate_target_feasible", 1.0)) < 0.5:
            return response
