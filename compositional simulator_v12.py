...
class CompositionalSimulator1D:
    ...
    def reported_well_response(self, state: SimulationState) -> Dict[str, float | np.ndarray]:
        return self.solve_well_control(state)
...
def display_streamlit_app() -> None:
    ...
    st.write(
        ...
        "producer control by fixed BHP, fixed gas rate, or simplified fixed THP, and one-stage surface separator flash for reporting gas and condensate rates."
    )
    ...
    controlled_gas_rate = np.asarray(history.controlled_gas_rate_mmscf_day, dtype=float)
    ...
    with result_tabs[0]:
        ...
        if history.well_control_mode[-1] == "bhp":
            ...
            st.caption("In fixed gas-rate mode, 'Controlled Gas Rate' is the enforced schedule target. 'Separator Gas Rate' may vary slightly because surface separator flash redistributes the produced wellstream at separator conditions.")
        else:
    ...
    with result_tabs[1]:
        figs = [
            ...
            make_line_figure(
                time_axis,
                [
                    (controlled_gas_rate, "Controlled Gas Rate"),
                    (np.asarray(history.gas_rate_mmscf_day), "Separator Gas Rate"),
                    (gas_rate_plot, "Smoothed Separator Gas Rate"),
                ],
                "Time (days)",
                "Gas Rate (MMscf/day)",
            ...
            make_line_figure(time_axis, [(np.asarray(history.separator_vapor_fraction), "Separator Vapor Fraction")], "Time (days)", "Vapor Fraction", "Surface Separator Flash Vapor Fraction History"),
        ]
        for fig in figs:
            st.pyplot(fig)
            plt.close(fig)
