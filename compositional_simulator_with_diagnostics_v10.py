...
class SimulationHistory:
    ...
    well_beta: List[float] = field(default_factory=list)
    well_Sg: List[float] = field(default_factory=list)
    well_So: List[float] = field(default_factory=list)
    well_mu_g_cp: List[float] = field(default_factory=list)
    well_mu_o_cp: List[float] = field(default_factory=list)
    well_lam_g: List[float] = field(default_factory=list)
    well_lam_o: List[float] = field(default_factory=list)
...
class CompositionalSimulator1D:
    ...
    def well_response_at_pwf(self, state: SimulationState, pwf_psia: float) -> Dict[str, float | np.ndarray]:
        ...
        return {
            ...
            "So": float(mob["So"]),
            "Sw": float(mob["Sw"]),
            "mu_g_cp": float(mob["mu_g"]),
            "mu_o_cp": float(mob["mu_o"]),
            "mu_w_cp": float(mob["mu_w"]),
            "lam_g": float(lam_g),
            "lam_o": float(lam_o),
            "lam_w": float(lam_w),
            "lam_t": float(lam_t),
    ...
    def run(self, state0: SimulationState, t_end_days: float, dt_days: float) -> Tuple[SimulationState, SimulationHistory]:
        ...
        while t <= t_end_days + 1e-12:
            ...
            beta_0 = float(response0["beta"])
            Sg_0 = float(response0["Sg"])
            So_0 = float(response0["So"])
            mu_g_0 = float(response0["mu_g_cp"])
            mu_o_0 = float(response0["mu_o_cp"])
            lam_g_0 = float(response0["lam_g"])
            lam_o_0 = float(response0["lam_o"])
            ...
            if t <= 1e-12:
                ...
                history.well_rate_total.append(float(q_total_0))
                history.well_gas_fraction.append(float(gas_frac_0))
                history.well_oil_fraction.append(float(oil_frac_0))
                history.well_water_fraction.append(float(water_frac_0))
                history.well_dropout_indicator.append(float(dropout_0))
                history.well_krg.append(float(krg_0))
                history.well_kro.append(float(kro_0))
                history.well_krw.append(float(krw_0))
                history.well_beta.append(float(beta_0))
                history.well_Sg.append(float(Sg_0))
                history.well_So.append(float(So_0))
                history.well_mu_g_cp.append(float(mu_g_0))
                history.well_mu_o_cp.append(float(mu_o_0))
                history.well_lam_g.append(float(lam_g_0))
                history.well_lam_o.append(float(lam_o_0))
                history.well_damage_factor.append(float(damage_factor_0))
                history.well_effective_wi.append(float(wi_eff_0))
                history.well_rate_undamaged.append(float(q_undamaged_0))
                history.productivity_loss_fraction.append(float(ploss_0))
            ...
            beta_1 = float(response1["beta"])
            Sg_1 = float(response1["Sg"])
            So_1 = float(response1["So"])
            mu_g_1 = float(response1["mu_g_cp"])
            mu_o_1 = float(response1["mu_o_cp"])
            lam_g_1 = float(response1["lam_g"])
            lam_o_1 = float(response1["lam_o"])
            ...
            kro = 0.5 * (kro_0 + kro_1)
            krw = 0.5 * (krw_0 + krw_1)
            beta = 0.5 * (beta_0 + beta_1)
            Sg = 0.5 * (Sg_0 + Sg_1)
            So = 0.5 * (So_0 + So_1)
            mu_g = 0.5 * (mu_g_0 + mu_g_1)
            mu_o = 0.5 * (mu_o_0 + mu_o_1)
            lam_g = 0.5 * (lam_g_0 + lam_g_1)
            lam_o = 0.5 * (lam_o_0 + lam_o_1)
            ...
            history.well_beta.append(float(beta))
            history.well_Sg.append(float(Sg))
            history.well_So.append(float(So))
            history.well_mu_g_cp.append(float(mu_g))
            history.well_mu_o_cp.append(float(mu_o))
            history.well_lam_g.append(float(lam_g))
            history.well_lam_o.append(float(lam_o))
...
def history_dataframe(history: SimulationHistory, start_date_text: str | None = None) -> pd.DataFrame:
    df = pd.DataFrame({
        ...
        "well_beta": history.well_beta,
        "well_Sg": history.well_Sg,
        "well_So": history.well_So,
        "well_mu_g_cp": history.well_mu_g_cp,
        "well_mu_o_cp": history.well_mu_o_cp,
        "well_lam_g": history.well_lam_g,
        "well_lam_o": history.well_lam_o,
...
def display_streamlit_app() -> None:
    ...
    with st.spinner("Running compositional simulation..."):
        try:
            result = run_example(
                start_date_text=start_date_text,                scenario_name=scenario_name,
