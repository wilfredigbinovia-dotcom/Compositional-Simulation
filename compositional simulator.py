...
def run_example() -> None:
    ...
    export_history_csv(history, str(output_dir / "history.csv"))
    export_spatial_csv(final_diag, str(output_dir / "final_spatial_diagnostics.csv"))
    export_chapter4_summary_csv(history, str(output_dir / "chapter4_summary.csv"))
    plot_history(history, output_dir)
    plot_spatial_diagnostics(final_diag, output_dir)

    print("Simulation complete")
    print(f"Final average pressure:   {history.avg_pressure_psia[-1]:.2f} psia")
    print(f"Final minimum pressure:   {history.min_pressure_psia[-1]:.2f} psia")
    print(f"Final well total rate:    {history.well_rate_total[-1]:.6f} lbmol/day")
    print(f"Final well gas fraction:  {history.well_gas_fraction[-1]:.4f}")
    print(f"Final well liquid frac.:  {history.well_liquid_fraction[-1]:.4f}")
    print(f"Dropout indicator:        {history.well_dropout_indicator[-1]:.4f}")
    print(f"Damage factor:            {history.well_damage_factor[-1]:.4f}")
    print(f"Effective WI:             {history.well_effective_wi[-1]:.5f}")
    print(f"Undamaged rate:           {history.well_rate_undamaged[-1]:.6f} lbmol/day")
    print(f"Productivity loss frac.:  {history.productivity_loss_fraction[-1]:.4f}")
    print(f"Well krg / krl:           {history.well_krg[-1]:.4f} / {history.well_krl[-1]:.4f}")
    print("Final well-cell composition:")
    for comp, zi in zip(fluid.components, final_state.z[well.cell_index]):
        print(f"  {comp.name:>4s}: {zi:.5f}")

    print(f"Output directory:          {output_dir.resolve()}")
    print("Generated files:")
    print(f"  - near-well minimum dx (ft): {grid.cell_width(well.cell_index):.3f}")
    print(f"  - Peaceman WI geom: {sim.peaceman_well_index(well.cell_index):.5f}")
    print("  - history.csv")
    print("  - final_spatial_diagnostics.csv")
    print("  - chapter4_summary.csv")
    print("  - pressure_history.png")
    print("  - well_rate_history.png")
    print("  - phase_fraction_history.png")
    print("  - dropout_history.png")
    print("  - productivity_loss_history.png")
    print("  - productivity_loss_fraction_history.png")
    print("  - damage_factor_history.png")
    print("  - pressure_profile.png")
    print("  - dropout_profile.png")
    print("  - relperm_profile.png")
    print("  - phase_distribution_profile.png")

    print_chapter4_summary(history)

    print()
    print("Selected time-history samples:")
    sample_step = max(len(history.time_days) // 8, 1)
    for idx in range(0, len(history.time_days), sample_step):
        print(
            f"t={history.time_days[idx]:6.1f} d | "
            f"Pavg={history.avg_pressure_psia[idx]:8.1f} psia | "
            f"q={history.well_rate_total[idx]:10.5f} | "
            f"fg={history.well_gas_fraction[idx]:6.3f} | "
            f"fl={history.well_liquid_fraction[idx]:6.3f} | "
            f"drop={history.well_dropout_indicator[idx]:6.3f} | "
            f"df={history.well_damage_factor[idx]:6.3f} | "
            f"ploss={history.productivity_loss_fraction[idx]:6.3f}"
        )
