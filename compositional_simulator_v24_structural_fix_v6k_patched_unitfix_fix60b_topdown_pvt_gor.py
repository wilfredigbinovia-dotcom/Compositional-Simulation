...
class CompositionalSimulator1D:
    ...
    def _top_down_pvt_gor_profile(self, state: SimulationState) -> Dict[str, float | np.ndarray]:
        ...
        for seg in range(nseg):
            ...
            gor = self.pvt_table.interp(p_ref, "gas_oil_ratio_scf_stb", default=None) if self.pvt_table is not None else None
            if gor is None or gor <= 1e-9:
                qo_stb_day = 0.0
            else:
                qo_stb_day = qg_scf_day / max(gor, 1e-9)
        ...
        if self.pvt_table is not None:
            gor_bottom = self.pvt_table.interp(pwf, "gas_oil_ratio_scf_stb", default=None)
            if gor_bottom is not None and gor_bottom > 1e-9:
                qo_stb_day_bottom = self.well.target_gas_rate_mmscf_day * 1.0e6 / gor_bottom
