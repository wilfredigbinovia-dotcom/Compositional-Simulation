...
class CompositionalSimulator1D:
    ...
    def step_once(self, state: SimulationState, dt_days: float) -> Tuple[SimulationState, Dict[str, float]]:
        ...
        try:
            ...
            nt_before = float(state.nt.sum())

            # Pressure correction from moles only — no pv geometry needed.
            # The ratio nt_after/nt_before equals p_new/Z_new / (p_old/Z_old)
            # for a closed reservoir. We know p_old (from the state entering this step),
            # so we can compute p/Z_new and iterate for p_new.
            p_old_avg = float(np.mean(state.pressure))
            if nt_before > 1e-6 and nt_after > 0.0 and p_old_avg > 14.7:
                try:
                    Z_old = float(self.eos.z_factor(
                        z_avg, max(p_old_avg, 14.7), self.T, phase="v"))
                    Z_old = max(Z_old, 0.05)
                except Exception:
                    Z_old = 0.87
                pZ_old = p_old_avg / Z_old
                pZ_new = pZ_old * (nt_after / nt_before)

                p_avg_new = pZ_new * Z_old  # initial guess
                for _ in range(15):
                    try:
                ...
                p_avg_pred = float(np.mean(p_pred))
                dp_corr = p_avg_new - p_avg_pred
    ...
    def run(self, state0: SimulationState, t_end_days: float, dt_days: float) -> Tuple[SimulationState, SimulationHistory]:
        ...
        # Hard sanity check: pv must equal π×r²×h×φ (not π×r²×length×φ).
        # If this assertion fires, the wrong version of the file is running.
        _expected_pv = self.grid.area_ft2 * self.grid.thickness_ft * self.phi
        _ratio = float(self.pv.sum()) / max(_expected_pv, 1.0)
        if not (0.99 < _ratio < 1.01):
            raise RuntimeError(
                f"FATAL: sim.pv is wrong! ratio={_ratio:.4f} "
                f"(got {self.pv.sum():,.0f} ft³, expected {_expected_pv:,.0f} ft³). "
                f"This means length_ft ({self.grid.length_ft}) was used instead of "
                f"thickness_ft ({self.grid.thickness_ft}). "
                f"You are running an old version of the file."
            )
