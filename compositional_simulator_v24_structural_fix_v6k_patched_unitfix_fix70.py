...
def run_example(
    ...
    refined_cells = max(2, min(refined_cells, nx - 1))
    # 1D grid length = drainage radius; length_ft kept for API back-compat
...
def display_streamlit_app() -> None:
    ...
    length_ft = radius_ft  # 1D grid extends from wellbore to drainage radius
    area_ft2 = math.pi * radius_ft * thickness_ft  # GRV = area * L = pi*r*h * r = pi*r^2*h
    ...
    with tab_reservoir_grid:
        ...
        length_ft = radius_ft  # 1D grid length = drainage radius
        area_ft2 = math.pi * radius_ft * thickness_ft  # GRV = area * L = pi*r*h * r = pi*r^2*h
        ...
        st.caption(f"Drainage area = π × r² = {drainage_area_acres:,.2f} acres  |  GRV = π × r² × h = {area_ft2*radius_ft/1e6:,.2f} MMft³")
        st.info(
            "Grid: 1D nonuniform Cartesian with near-well refinement. "
            "Cross-sectional area is computed as πr² from the drainage radius. "
