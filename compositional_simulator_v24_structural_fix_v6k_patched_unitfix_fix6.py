...
class CompositionalSimulator1D:
    ...
    def separator_rates(self, response: Dict[str, float | np.ndarray]) -> Dict[str, float]:
        ...
        # Stabilized field reporting:
        # use the wellstream gas and liquid phase rates from the producer response,
        # not the one-stage separator split, for the primary gas/condensate histories.
        # The separator flash is still evaluated and retained as a diagnostic, but
        # using it directly for reported rates created non-physical late-time spikes
        # when the flash state switched abruptly near phase boundaries.
        ...
        condensate_rate_stb_day = liquid_lbmol_day_to_stb_day(qo_report_lbmol_day, x_report, self.mw_components, self.reporting.condensate_density_lbft3)
