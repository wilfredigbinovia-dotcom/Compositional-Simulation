...
class CompositionalSimulator1D:
    ...
    def run(self, state0: SimulationState, t_end_days: float, dt_days: float) -> Tuple[SimulationState, SimulationHistory]:
        ...
        while t <= t_end_days + 1e-12:
            ...
            # MB correction diagnostics — print for first 3 steps to Streamlit logs
            _mbd = summary.get("_mb_debug", {})
            if _mbd and t < accepted_dt * 3:
                import sys
                _d = _mbd
                print(
                    f"[MB t={t:.0f}d]"
                    f" nt={_d.get('nt_total',0):.3e} nt0={_d.get('nt_init',0):.3e}"
                    f" pZ={_d.get('pZ_target',0):.1f} p_start={_d.get('p_start',0):.1f}"
                    f" p_mb={_d.get('p_mb',0):.1f} dp={_d.get('dp_corr',0):.1f}"
                    f" p_final={_d.get('p_final',0):.1f} exc={_d.get('exception','none')}",
