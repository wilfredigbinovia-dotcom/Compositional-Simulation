...
class CompositionalSimulator1D:
    ...
    def transport_update(self, state: SimulationState, dt_days: float) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, float, float, float, float, float, float, float, float, float, float, float, float]:
        """
        Systematic transport / well update:
        1) move intercell and injector transport with light global substeps;
        2) treat the producer-cell sink with bounded local phase-aware substeps;
        3) reflash the producer cell after each local sink substep.

        This avoids the old one-shot producer withdrawal that could collapse the
        well cell into a numerically frozen regime, without imposing a blunt
        inventory cap that distorts depletion behaviour.
        """
        max_global_substep_days = 20.0
        n_global = max(1, min(3, int(math.ceil(dt_days / max_global_substep_days))))
        dt_global = dt_days / n_global
        ...
        for _ in range(n_global):
            ...
            for i in range(self.grid.nx - 1):
                ...
                ncomp_new[i] -= dt_global * hc_flux
                ncomp_new[i + 1] += dt_global * hc_flux
                nw_new[i] -= dt_global * w_flux
                nw_new[i + 1] += dt_global * w_flux

            inj_cell, inj_source = self.injector_source(work_state)
            if inj_cell is not None and inj_source is not None:
                ncomp_new[inj_cell] += dt_global * inj_source

            ncomp_new = np.clip(ncomp_new, 1e-12, None)
            nw_new = np.clip(nw_new, 1e-12, None)

            # Local producer-cell sink update with phase-aware substepping.
            ncomp_cell = ncomp_new[wi].copy()
            nw_cell = float(nw_new[wi])
            p_cell = float(work_state.pressure[wi])

            nt_cell = float(np.sum(ncomp_cell))
            z_cell = ncomp_cell / max(nt_cell, 1e-12)
            sw_cell = float(saturation_from_water_moles(np.array([nw_cell]), np.array([self.pv[wi]]))[0])
            local_state = SimulationState(
                pressure=np.array([p_cell]),
                z=z_cell[None, :],
                nt=np.array([nt_cell]),
                nw=np.array([nw_cell]),
                sw=np.array([sw_cell]),
            ...
            self._set_cache_state(local_state)
            original_well_cell_index = self.well.cell_index
            self.well.cell_index = 0
            try:
                response0 = self.solve_well_control(local_state)
            finally:
                self.well.cell_index = original_well_cell_index
            beta0 = float(response0.get("beta", 0.0))
            qg0 = float(response0["qg_lbmol_day"])
            qo0 = float(response0["qo_lbmol_day"])
            qw0 = float(response0["qw_lbmol_day"])
            ng0 = max(beta0 * nt_cell, 1e-12)
            no0 = max((1.0 - beta0) * nt_cell, 1e-12)
            nw0 = max(nw_cell, 1e-12)

            target_phase_frac = 0.35
            sink_cfl = max(
                (qg0 * dt_global) / max(target_phase_frac * ng0, 1e-12),
                (qo0 * dt_global) / max(target_phase_frac * no0, 1e-12),
                (qw0 * dt_global) / max(target_phase_frac * nw0, 1e-12),
                1.0,
            )
            n_sink = max(1, min(4, int(math.ceil(sink_cfl))))
            dt_sink = dt_global / n_sink
            ...
            for _ in range(n_sink):
                ...
                self.well.cell_index = 0
                try:
                    response = self.solve_well_control(local_state)
                finally:
                    self.well.cell_index = original_well_cell_index

                q_total = float(response["q_total_lbmol_day"])
                ...
                x = np.asarray(response["x"], dtype=float)
                y = np.asarray(response["y"], dtype=float)
                beta = float(response.get("beta", 0.0))

                ng_avail = max(beta * nt_cell, 1e-12)
                no_avail = max((1.0 - beta) * nt_cell, 1e-12)
                nw_avail = max(nw_cell, 1e-12)

                scale_g = min(1.0, 0.85 * ng_avail / max(qg * dt_sink, 1e-18)) if qg > 1e-18 else 1.0
                scale_o = min(1.0, 0.85 * no_avail / max(qo * dt_sink, 1e-18)) if qo > 1e-18 else 1.0
                scale_w = min(1.0, 0.85 * nw_avail / max(qw * dt_sink, 1e-18)) if qw > 1e-18 else 1.0
                sink_scale = float(np.clip(min(scale_g, scale_o, scale_w), 0.0, 1.0))

                dg = sink_scale * qg * dt_sink
                do = sink_scale * qo * dt_sink
                dw = sink_scale * qw * dt_sink

                ncomp_cell -= dg * y + do * x
                nw_cell -= dw

                ncomp_cell = np.clip(ncomp_cell, 1e-12, None)
                nw_cell = max(nw_cell, 1e-12)

                # Keep reported rates consistent with any phase-aware sink scaling.
                q_total *= sink_scale
                q_undamaged *= sink_scale

            ncomp_new[wi] = ncomp_cell
            nw_new[wi] = nw_cell
