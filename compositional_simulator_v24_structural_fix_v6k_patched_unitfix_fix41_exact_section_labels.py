...
def display_streamlit_app() -> None:
    ...
    with st.sidebar.expander("Rock / Rock-Fluid", expanded=False):
        st.caption("Rock, relative permeability, capillary pressure, and hysteresis inputs are configured in the current sidebar workflow and will be consolidated into this section in the next structural pass.")

    with st.sidebar.expander("Initial Conditions", expanded=False):
        st.caption("Initial pressure, temperature, dew point, and initial water saturation inputs are configured in the current sidebar workflow and will be consolidated into this section in the next structural pass.")

    with st.sidebar.expander("Producer Well", expanded=False):
        st.caption("Producer controls, tubing, and lift inputs are configured in the current sidebar workflow and will be consolidated into this section in the next structural pass.")

    with st.sidebar.expander("Injector Well", expanded=False):
        st.caption("Injection controls, schedules, and injected-gas composition are configured in the current sidebar workflow and will be consolidated into this section in the next structural pass.")

    with st.sidebar.expander("Aquifer / Boundaries", expanded=False):
        st.caption("Aquifer and boundary-condition inputs are configured in the current sidebar workflow and will be consolidated into this section in the next structural pass.")

    with st.sidebar.expander("Advanced Physics", expanded=False):
        st.caption("Transport, capillary pressure, hysteresis, and other advanced-physics controls are configured in the current sidebar workflow and will be consolidated into this section in the next structural pass.")
    with st.sidebar.expander("Case Setup", expanded=True):
        ...
        with col1:
            ...
            injection_control_label = st.selectbox(
                "Injection mode",
                ["Simple schedule-driven", "Enhanced pressure-controlled"],
                index=1,
                disabled=(scenario_name == "natural_depletion"),
            )
            injection_control_mode = "enhanced" if injection_control_label == "Enhanced pressure-controlled" else "simple"
            injection_pressure_psia = st.number_input(
            ...
            max_injection_bhp_psia = st.number_input(
                "Maximum bottom-hole injection pressure (psia)",
                min_value=14.7,
                max_value=25000.0,
                value=9000.0,
                step=100.0,
                disabled=(scenario_name == "natural_depletion" or injection_control_mode != "enhanced"),
            )
            injectivity_index_mmscf_day_psi = st.number_input(
                "Injectivity index (MMscf/day/psi)",
                min_value=0.0,
                max_value=1.0e4,
                value=gas_lbmol_day_to_mmscf_day(500.0),
                step=0.01,
                format="%.4f",
                disabled=(scenario_name == "natural_depletion" or injection_control_mode != "enhanced"),
            )
    ...
    with st.sidebar.expander("Reservoir / Grid", expanded=False):
        ...
        st.caption("Used for THP calculation/reporting; under Fixed THP mode the solver now uses a tubing-hydraulics proxy closer to nodal analysis.")
        ...
        with thp3:
            wellhead_temperature_F = st.number_input(
        ...
        with thp4:
            tubing_model_label = st.selectbox(
                "Tubing hydraulics model",
                ["Mechanistic proxy", "Simple coefficient"],
                index=0,
            )
            tubing_model = "mechanistic" if tubing_model_label == "Mechanistic proxy" else "simple"

        thp5, thp6, thp7 = st.columns(3)
        with thp5:
            thp_friction_coeff = st.number_input(
                ...
                value=0.02,
                step=0.005,
                help="Retained as an empirical backpressure term and as the main loss term under Simple coefficient mode.",
            )
        with thp6:
            tubing_roughness_in = st.number_input(
                "Tubing roughness (in)",
                min_value=0.0,
                max_value=0.05,
                value=0.0006,
                step=0.0002,
                format="%.4f",
                disabled=(tubing_model != "mechanistic"),
            )
        with thp7:
            tubing_calibration_factor = st.number_input(
                "Tubing calibration factor",
                min_value=0.1,
                max_value=10.0,
                value=1.0,
                step=0.1,
        ...
        st.subheader("Capillary Pressure")
        ...
        with cp3:
            st.caption("Capillary endpoints are tied to the entered relperm residual saturations for consistency.")
            st.write(f"Using Swir = {Swc:.2f}, Sorw = {Sorw:.2f}, Sgc = {Sgc:.2f}, Sorg = {Sorg:.2f}")

        st.subheader("Relative-Permeability Hysteresis")
        hy1, hy2, hy3 = st.columns(3)
        with hy1:
        ...
        with hy3:
            ...
            st.caption("This first-order model tracks the maximum gas saturation reached in each cell and reduces gas mobility on saturation reversal to mimic trapped gas during cycling.")

        st.subheader("Compositional Transport")
        ...
        with tr3:
            ...
            st.caption("Adds phase-split hydrocarbon advection and a capped Fickian-style dispersion term to make intercell compositional transport less purely upwind and more commercial-simulator-like.")

        st.subheader("Boundary Conditions and Aquifer")
