...
class CompositionalSimulator1D:
    ...
    def step_once(self, state: SimulationState, dt_days: float) -> Tuple[SimulationState, Dict[str, float]]:
        ...
        try:
            ...
            nt_after = float(tr.nt.sum())

            # Use geometry-anchored pv_hc (from grid.area×h×φ) and the
            # corresponding correct nt_init — both set at run() start from
            # the same corrected geometry, so they are always consistent.
            _pv_hc = getattr(self, '_pv_hc_init', None)
            if _pv_hc is not None and _pv_hc > 1e-6 and nt_after > 0.0:
                pZ_avg    = nt_after * R * self.T / _pv_hc
