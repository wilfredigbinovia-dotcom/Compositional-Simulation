...
WellControlMode = Literal["bhp", "gas_rate", "thp"]
...
class CompositionalSimulator1D:
    ...
    def run(self, state0: SimulationState, t_end_days: float, dt_days: float) -> Tuple[SimulationState, SimulationHistory]:
        ...
        while t <= t_end_days + 1e-12:
            ...
            response0 = self.solve_well_control(state)
            ...
            if next_dt <= 1e-12:
                ...
                qo_lbmol_day = q_total * oil_frac
                qw_lbmol_day = q_total * water_frac
                x = np.asarray(response0["x"], dtype=float)
            ...
            response1 = self.solve_well_control(state_new)
            ...
            gas_rate_mmscf_day = gas_lbmol_day_to_mmscf_day(qg_lbmol_day)
            condensate_rate_stb_day = liquid_lbmol_day_to_stb_day(qo_lbmol_day, x_avg, self.mw_components, CONDENSATE_DENSITY_LBFT3)
            water_rate_stb_day = water_lbmol_day_to_stb_day(qw_lbmol_day)
...
def display_streamlit_app() -> None:
    ...
    st.write(
        ...
        "and producer control by fixed BHP, fixed gas rate, or simplified fixed THP."
    )
    ...
    with input_tabs[1]:
        ...
        with c2:
            ...
            well_control_label = st.selectbox("Producer control mode", ["Fixed BHP", "Fixed Gas Rate", "Fixed THP"], index=0)
            control_map = {"Fixed BHP": "bhp", "Fixed Gas Rate": "gas_rate", "Fixed THP": "thp"}
            well_control_mode = control_map[well_control_label]

            bhp_psia = st.number_input("Bottom-hole pressure target (psia)", min_value=100.0, max_value=20000.0, value=3000.0, step=100.0)
            target_gas_rate_mmscf_day = st.number_input("Target gas rate (MMscf/day)", min_value=0.0, max_value=5000.0, value=5.0, step=0.5)
            thp_psia = st.number_input("Tubing head pressure target (psia)", min_value=14.7, max_value=10000.0, value=500.0, step=25.0)
        ...
        if well_control_mode == "thp":
            st.subheader("Tubing / Lift Inputs for THP Control")
            ...
            with thp3:
                wellhead_temperature_R = st.number_input("Wellhead temperature (°R)", min_value=400.0, max_value=800.0, value=520.0, step=5.0)
            with thp4:
                thp_friction_coeff = st.number_input("THP friction coefficient", min_value=0.0, max_value=1.0, value=0.02, step=0.005)
        else:
            tvd_ft = 8000.0
            tubing_id_in = 2.441
    ...
    with input_tabs[2]:
        ...
        with c3:
            ...
            st.write("Dew point is user-specified and used for diagnostics.")
            st.write("THP mode uses a simplified tubing-loss model with hydrostatic and empirical friction terms.")
    ...
    with input_tabs[4]:
        st.write("Producer supports fixed BHP, fixed gas rate, and simplified fixed THP control.")
        st.write("Injector uses fixed-rate injection in gas cycling and lean gas injection scenarios.")
    ...
    with result_tabs[0]:
        ...
        w4.metric("Target Gas Rate (MMscf/day)", f"{history.well_target_gas_rate_mmscf_day[-1]:.3f}")
        ...
        d1, d2 = st.columns(2)
        d1.write(f"Near-well minimum dx (ft): {grid.cell_width(well.cell_index):.3f}")
        d2.write(f"Peaceman WI geom: {sim.peaceman_well_index(well.cell_index):.5f}")

    with result_tabs[1]:
        figs = [
            make_line_figure(time_axis, [(np.asarray(history.avg_pressure_psia), "Average Pressure"), (np.asarray(history.well_pressure_psia), "Well-Cell Pressure"), (np.asarray(history.dew_point_psia), "Dew Point")], "Time (days)", "Pressure (psia)", "Pressure and Dew-Point History"),
            make_line_figure(time_axis, [(np.asarray(history.well_pressure_minus_dewpoint_psia), "Well Pressure - Dew Point"), (np.asarray(history.avg_pressure_minus_dewpoint_psia), "Average Pressure - Dew Point")], "Time (days)", "Pressure Margin (psia)", "Pressure Margin to Dew Point"),
            make_line_figure(time_axis, [(np.asarray(history.gas_rate_mmscf_day), "Raw Gas Rate"), (gas_rate_plot, "Smoothed Gas Rate")], "Time (days)", "Gas Rate (MMscf/day)", "Gas Production Rate History"),
            make_line_figure(time_axis, [(np.asarray(history.condensate_rate_stb_day), "Condensate Rate"), (np.asarray(history.water_rate_stb_day), "Water Rate")], "Time (days)", "Liquid Rate (STB/day)", "Condensate and Water Production Rate History"),
            make_line_figure(time_axis, [(np.asarray(history.cum_gas_bscf), "Cumulative Gas")], "Time (days)", "Cumulative Gas (Bscf)", "Cumulative Gas Production"),
            make_line_figure(time_axis, [(np.asarray(history.cum_condensate_mstb), "Cumulative Condensate"), (np.asarray(history.cum_water_mstb), "Cumulative Water")], "Time (days)", "Cumulative Liquid (MSTB)", "Cumulative Condensate and Water Production"),
            make_line_figure(time_axis, [(np.asarray(history.well_damage_factor), "Damage Factor"), (np.asarray(history.productivity_loss_fraction), "Productivity Loss Fraction")], "Time (days)", "Dimensionless", "Condensate-Bank Impairment"),
            make_line_figure(time_axis, [(np.asarray(history.avg_sw), "Average Sw")], "Time (days)", "Average Sw", "Average Water Saturation History"),
            make_line_figure(time_axis, [(np.abs(np.asarray(history.hc_mass_balance_error_lbmol)), "|HC Mass Balance Error|"), (np.abs(np.asarray(history.water_mass_balance_error_lbmol)), "|Water Mass Balance Error|")], "Time (days)", "Absolute Error (lbmol)", "Mass Balance Error History"),
            make_line_figure(time_axis, [(np.asarray(history.well_flowing_pwf_psia), "Flowing Pwf")], "Time (days)", "Pwf (psia)", "Flowing Bottom-Hole Pressure History"),
            make_line_figure(time_axis, [(np.asarray(history.well_estimated_thp_psia), "Estimated THP")], "Time (days)", "THP (psia)", "Tubing Head Pressure History"),
        ]
        for fig in figs:
            st.pyplot(fig)
            plt.close(fig)
    ...
    with result_tabs[4]:
        ...
        st.caption(
            "Gas rates are reported in MMscf/day using standard conditions of 14.7 psia and 60°F. Condensate rates are approximate STB/day estimates. "
