...
class CompositionalSimulator1D:
    ...
    def reported_well_response(self, state: SimulationState) -> Dict[str, float | np.ndarray]:
        ...
        if actual_qg_lbmol_day >= 0.999 * target_qg_lbmol_day and target_qg_lbmol_day > 0.0:
            response = dict(response)
            gas_frac = max(float(response["gas_frac"]), 1e-12)
            q_total_lbmol_day = target_qg_lbmol_day / gas_frac
            ...
            y = np.asarray(response["y"], dtype=float)
            x = np.asarray(response["x"], dtype=float)
            qo_lbmol_day = q_total_lbmol_day * float(response["oil_frac"])
            qg_lbmol_day = target_qg_lbmol_day
            response["qo_lbmol_day"] = float(qo_lbmol_day)
            response["hc_sink"] = qg_lbmol_day * y + qo_lbmol_day * x
