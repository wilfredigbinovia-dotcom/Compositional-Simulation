...
class CompositionalSimulator1D:
    ...
    def step_once(self, state: SimulationState, dt_days: float) -> Tuple[SimulationState, Dict[str, float]]:
        ...
        try:
            ...
            # Use the HC pore volume computed from initial conditions at run() start.
            # This is always self-consistent with nt (both derived from the same pv),
            # and is a constant so it cannot oscillate between steps.
    ...
    def run(self, state0: SimulationState, t_end_days: float, dt_days: float) -> Tuple[SimulationState, SimulationHistory]:
        ...
        # Compute HC pore volume from initial moles and initial pressure using the
        # real gas law: pv_hc = nt * Z * R * T / P.  This is ALWAYS self-consistent
        # with how nt was computed in initialize_state (same pv was used for both),
        # so it is immune to any pv/thickness mismatch between sessions.
        # Stored once here and used by every step's pressure correction.
        try:
            _z_init = np.average(state0.z, axis=0, weights=state0.nt)
            _z_init /= max(float(_z_init.sum()), 1e-12)
            _p_init = float(np.mean(state0.pressure))
            _Z_init = float(self.eos.z_factor(_z_init, max(_p_init, 14.7), self.T, phase="v"))
            _Z_init = max(_Z_init, 0.05)
            self._pv_hc_init = float(state0.nt.sum()) * _Z_init * R * self.T / max(_p_init, 14.7)
            self._nt_init    = float(state0.nt.sum())
            self._pZ_init    = _p_init / _Z_init
        except Exception:
            self._pv_hc_init = None
            self._nt_init    = None
            self._pZ_init    = None
