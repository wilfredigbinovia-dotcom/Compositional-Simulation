...
def beta_to_flowing_saturations(
    ...
    beta_lo: float = 0.03,
    beta_hi: float = 0.90,
    min_gas_sat: float = 0.04,
    beta_weight: float = 0.35,
) -> Tuple[float, float, float]:
    """
    Smoothly map flash vapor fraction beta to flowing gas/oil saturations.

    A pure smoothstep mapping can become too flat over long depletion tails,
    while a hard beta mapping is too sharp. This blended version preserves
    monotonic decline but keeps gas mobility responsive as pressure falls.
    """
    ...
    Sg = Shc * (min_gas_sat + (1.0 - min_gas_sat) * gas_proxy)
    So = Shc - Sg

    Sg = float(np.clip(Sg, 0.0, Shc))
    So = float(np.clip(So, 0.0, Shc))
...
class CompositionalSimulator1D:
    ...
    def well_response_at_pwf(self, state: SimulationState, pwf_psia: float) -> Dict[str, float | np.ndarray]:
        ...
        # Startup ramp removed so the well is fully active from day 0
        ...
        if lam_hc > 1e-12 and hc_frac > 1e-12:
            ...
            # keep the wellstream hydrocarbon split tied to both flowing mobility
            # and in-situ flash behavior so the gas fraction cannot collapse to zero
            # while the produced stream still flashes significant gas at surface.
            fg_hc_blend = 0.45 * fg_hc_mob + 0.55 * fg_hc_flash
            min_fg_hc = 0.30 * fg_hc_flash if fg_hc_flash > 1e-6 else 0.0
