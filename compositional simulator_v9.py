...
class CompositionalSimulator1D:
    ...
    def pressure_update(self, state: SimulationState, dt_days: float) -> np.ndarray:
        ...
        Jw = self.peaceman_well_index(wi) * well_mob["lam_t"] * 1.0e-2
        A[wi, wi] += Jw
    ...
    def run(self, state0: SimulationState, t_end_days: float, dt_days: float) -> Tuple[SimulationState, SimulationHistory]:
        ...
        while t <= t_end_days + 1e-12:
            ...
            self.record_mass_balance(history, state, dt_days, q_total, gas_frac, oil_frac, water_frac, injector_rate_lbmol_day)
            ...
            state, _, accepted_dt, retries = self.adaptive_step(state, next_dt)
...
def display_streamlit_app() -> None:
    ...
    with input_tabs[2]:
        ...
        with c2:
            ...
            try:
                ...
                st.error(str(e))
                return
            dt_days = st.number_input("Requested time step (days)", min_value=0.01, max_value=1000.0, value=2.0, step=0.5)
    ...
    with result_tabs[1]:
        ...
        fig3 = make_line_figure(
            time_axis,
            [(np.asarray(history.gas_rate_mmscf_day), "Gas Rate")],
            "Time (days)",
            "Gas Rate (MMscf/day)",
