...
class CompositionalSimulator1D:
    ...
    def step_once(self, state: SimulationState, dt_days: float) -> Tuple[SimulationState, Dict[str, float]]:
        ...
        # Average pressure correction anchored to INITIAL conditions.
        # pZ_current = pZ_init × (nt_current / nt_init)
        # This is stable because it always references the same fixed baseline
        # (pZ_init and nt_init set at run() start), so errors cannot compound.
        # The step-to-step ratio approach (p_old × nt_new/nt_old) is unstable
        # because any overcorrection in step N shifts p_old for step N+1.
        try:
            _pZ0   = getattr(self, '_pZ_init_ref', None)   # p_init/Z_init
            _nt0   = getattr(self, '_nt_init_ref', None)   # nt at t=0
            _z_ref = getattr(self, '_z_init_ref', None)    # composition at t=0
            nt_cur = float(tr.nt.sum())

            if (_pZ0 is not None and _nt0 is not None and _nt0 > 1e-6
                    and _z_ref is not None and nt_cur > 0.0):
                pZ_cur = _pZ0 * (nt_cur / _nt0)
                # Iterate for p from pZ
                p_guess = pZ_cur * 0.87   # seed with Z≈0.87
                for _ in range(15):
                    try:
                        Z_cur = float(self.eos.z_factor(
                            _z_ref, max(p_guess, 14.7), self.T, phase="v"))
                        Z_cur = max(Z_cur, 0.05)
                    except Exception:
                        Z_cur = 0.87
                    p_new = pZ_cur * Z_cur
                    if abs(p_new - p_guess) < 0.05:
                        break
                    p_guess = 0.5 * (p_guess + p_new)
                p_new_avg = p_guess
