...
def run_example(
    ...
    _GIIP_diag = float(state0.nt.sum()) * (10.7316*519.67/14.7) / 1e9
    try:
        _st_diag.sidebar.markdown("---")
        _st_diag.sidebar.markdown(f"**Diag v{FILE_VERSION}**")
        _st_diag.sidebar.markdown(f"pv = {sim.pv.sum():,.0f} ft³")
        _st_diag.sidebar.markdown(f"GIIP = {_GIIP_diag:.2f} Bscf")
        _st_diag.sidebar.markdown("✅ OK" if abs(_GIIP_diag-86)<5 else "❌ WRONG")
    except Exception:
        pass
    # === END DIAGNOSTIC ===
