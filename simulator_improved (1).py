...
class CompositionalSimulator1D:
    ...
    def step_once(self, state: SimulationState, dt_days: float) -> Tuple[SimulationState, Dict[str, float]]:
        ...
        # Use p_pred for transport/flow calculations (correct spatial gradient).
        # Store material-balance pressure in new_state so history is always correct.
        # p_mb = pZ_new × Z_new where pZ_new = pZ_old × (nt_new/nt_old).
        # This is exact, uses no stored state, and cannot oscillate.
        try:
            nt_old = float(state.nt.sum())
            nt_new = float(tr.nt.sum())
            p_old_avg = float(np.mean(state.pressure))
            if nt_old > 1e-6 and nt_new > 0.0 and p_old_avg > 14.7:
                z_avg = np.average(tr.z, axis=0, weights=tr.nt)
                z_avg /= max(float(z_avg.sum()), 1e-12)
                Z_old = float(self.eos.z_factor(z_avg, p_old_avg, self.T, phase="v"))
                Z_old = max(Z_old, 0.05)
                pZ_new = (p_old_avg / Z_old) * (nt_new / nt_old)
                p_mb_avg = pZ_new * Z_old
                for _ in range(12):
                    Z_new = float(self.eos.z_factor(
                        z_avg, max(p_mb_avg, 14.7), self.T, phase="v"))
                    Z_new = max(Z_new, 0.05)
                    p_iter = pZ_new * Z_new
                    if abs(p_iter - p_mb_avg) < 0.1: break
                    p_mb_avg = 0.5 * (p_mb_avg + p_iter)
                # Build state pressure: uniform MB average, preserving p_pred shape
                # for the next step's transmissibility (gradient only, not level).
                p_mb_avg = max(p_mb_avg, 14.7)
                dp_gradient = p_pred - float(np.mean(p_pred))  # zero-mean gradient
                p_state = np.clip(p_mb_avg + dp_gradient, 14.7, 50000.0)
            else:
                p_state = p_pred.copy()
        except Exception:
            p_state = p_pred.copy()

        p_corrected = p_state  # used for history reporting and next-step state
...
def display_streamlit_app() -> None:
    ...
    st.caption("Build: v4-mbstate-2026-04-20 · pressure from mole-ratio MB stored in state")
