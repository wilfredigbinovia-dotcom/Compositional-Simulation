...
class CompositionalSimulator1D:
    ...
    def separator_rates(self, response: Dict[str, float | np.ndarray]) -> Dict[str, float]:
        ...
        ------------------------
        1. EOS separator train (always computed, always physically consistent).
           The wellstream is flashed through the configured surface separator
           stages and the stock-tank liquid is converted to STB/day using the
           composition-weighted molecular weight and user API-gravity density.

        2. PVT table CGR override (vaporized_cgr or reservoir_cgr column).
           If a CGR column is present in the uploaded PVT table, it is applied
           directly as:  condensate_rate = CGR_table × gas_rate_mmscf_day.
           This takes priority over the EOS separator result because the PVT
           table typically comes from a more carefully tuned PVT study.

        The previous 60/20/20 blend between three CGR estimates is removed.
        Blending sources that may disagree produces a result that is
        inconsistent with all of them and obscures which model is driving the
        answer.  The priority list above is explicit and auditable.

        Reported diagnostic fields
        --------------------------
        condensate_reporting_basis : string indicating which route was used
        eos_separator_cgr_stb_mmscf : CGR from the separator train alone
        pvt_table_cgr_stb_mmscf    : CGR from the PVT table (nan if absent)
        ...
        # Reported gas rate: for gas-rate control use the target (well control
        # already enforces it); for other modes use the actual wellstream rate.
        ...
        # ── Route 1: EOS separator train ─────────────────────────────────────
        # Convert stock-tank liquid (lbmol/day) to STB/day using the
        # composition-weighted Mw and user API-gravity density.
        ...
        # ── Route 2: PVT table CGR (higher priority when available) ─────────
        ...
        if gas_rate_mmscf_day <= 1e-12:
            ...
            # PVT table provides a measured/tuned CGR — use it directly.
            ...
            # No PVT table CGR — use the EOS separator train result.
            effective_cgr_stb_mmscf   = eos_sep_cgr
            condensate_rate_stb_day   = eos_sep_stb_day
            condensate_reporting_basis = "eos_separator_train"

        # ── Surface fractions for history tracking ────────────────────────────
    ...
    def record_mass_balance(
        ...
        w_err = (self.initial_water_lbmol
                 - self.cum_water_produced_lbmol
                 - w_current)
