...
class CompositionalSimulator1D:
    def __init__(
        ...
        # _pv is computed once here and exposed via the pv property below.
        # Using a property prevents any code path from accidentally overwriting
        # self.pv with the wrong bulk_volume formula (π×r²×length instead of π×r²×h).
        self._pv = (self.grid.area_ft2 * self.grid.thickness_ft * self.phi
                    * self.grid.dx_array / self.grid.length_ft)
    ...
    def pv(self) -> np.ndarray:
        """Per-cell pore volume (ft³). Always π×r²×h×φ × (dx/length).
        Implemented as a read-only property so no code path can overwrite it
        with the wrong bulk_volume formula (π×r²×length instead of π×r²×h)."""
        return self._pv

    @pv.setter
    def pv(self, value: np.ndarray) -> None:
        """Accept external assignments for backward compatibility but recompute
        from grid geometry to guarantee correctness."""
        # Recompute from first principles — ignore the supplied value entirely.
        self._pv = (self.grid.area_ft2 * self.grid.thickness_ft * self.phi
                    * self.grid.dx_array / self.grid.length_ft)
    ...
    def initialize_state(self, p_init_psia: float, z_init: np.ndarray, sw_init: float = 0.20) -> SimulationState:
        ...
        hc_pv = (self.grid.area_ft2 * self.grid.thickness_ft * self.phi * self.grid.dx_array / self.grid.length_ft) * (1.0 - sw)
        nt = molar_density * hc_pv
        _pv_geo = self.grid.area_ft2 * self.grid.thickness_ft * self.phi * self.grid.dx_array / self.grid.length_ft
        nw = water_moles_from_saturation(sw, _pv_geo)
    ...
    def pressure_update(self, state: SimulationState, dt_days: float) -> np.ndarray:
        ...
        # Compute per-cell pore volume directly from grid geometry.
        # This avoids any dependency on self.pv / self._pv and is guaranteed
        # correct: π×r²×h×φ × (dx_i / length_ft)
        pv_cell = (self.grid.area_ft2 * self.grid.thickness_ft * self.phi
                   * self.grid.dx_array / self.grid.length_ft)
    ...
    def transport_update(self, state: SimulationState, dt_days: float) -> TransportResult:
        ...
        for i in range(nx - 1):
            ...
            if hc_flux_mag > 0.0:
                ...
                pv_upstream = max((self.grid.area_ft2 * self.grid.thickness_ft * self.phi * self.grid.dx_array / self.grid.length_ft)[upstream], 1e-12)
        ...
        for _ in range(n_sub):
            ...
            sw_new = saturation_from_water_moles(nw_new, self.grid.area_ft2 * self.grid.thickness_ft * self.phi * self.grid.dx_array / self.grid.length_ft)
    ...
    def step_once(self, state: SimulationState, dt_days: float) -> Tuple[SimulationState, Dict[str, float]]:
        ...
        # Pressure correction: recompute per-cell average pressure from the
        # updated mole inventory using the real gas law, then apply as a uniform
        # scalar shift on p_pred (preserving the spatial gradient).
        # This uses ONLY tr.nt, tr.z, tr.sw, self.T and the EOS — no stored
        # geometry, no _pv_hc_init, no session-state-dependent quantities.
        try:
            # Compute what p/Z should be after transport using only mole ratio.
            # For a closed volume: p_new/Z_new = (nt_new/nt_old) × (p_old/Z_old)
            # This is purely mole-counting and is immune to any pv inconsistency.
            nt_old = float(state.nt.sum())
            nt_new = float(tr.nt.sum())
            if nt_old > 1e-6 and nt_new > 0.0:
                p_old_avg = float(np.mean(state.pressure))
                if p_old_avg > 14.7:
                    z_avg = np.average(tr.z, axis=0, weights=tr.nt)
                    z_avg /= max(float(z_avg.sum()), 1e-12)
                    try:
                        Z_old = float(self.eos.z_factor(
                            z_avg, p_old_avg, self.T, phase="v"))
                        Z_old = max(Z_old, 0.05)
                    except Exception:
                        Z_old = 0.87
                    pZ_new = (p_old_avg / Z_old) * (nt_new / nt_old)
                    # Iterate Z at p_new
                    p_new_avg = pZ_new * Z_old
                    for _ in range(15):
                        try:
                            Z_new = float(self.eos.z_factor(
                                z_avg, max(p_new_avg, 14.7), self.T, phase="v"))
                            Z_new = max(Z_new, 0.05)
                        except Exception:
                            break
                        p_iter = pZ_new * Z_new
                        if abs(p_iter - p_new_avg) < 0.05:
                            break
                        p_new_avg = 0.5 * (p_new_avg + p_iter)
                    # Apply as scalar shift on p_pred
                    p_pred_avg = float(np.mean(p_pred))
                    dp = float(np.clip(p_new_avg - p_pred_avg, -500.0, 500.0))
                    min_p = max(14.7, min(
                        (w.min_bhp_psia for w in self.wells), default=14.7))
                    p_corrected = np.clip(p_pred + dp, min_p * 0.5, 50000.0)
                else:
    ...
    def run(self, state0: SimulationState, t_end_days: float, dt_days: float) -> Tuple[SimulationState, SimulationHistory]:
        ...
        # Auto-correct pv if it appears to have been set with length_ft instead of
        # thickness_ft (the historical bug). Ratio > 5 means something is very wrong.
        _expected_pv = self.grid.area_ft2 * self.grid.thickness_ft * self.phi
        _ratio = float(self.pv.sum()) / max(_expected_pv, 1.0)
        if not (0.5 < _ratio < 2.0):
            # Force recompute via the setter which always uses correct geometry
            self.pv = None  # triggers setter → recomputes _pv correctly
        ...
        # HC pore volume — computed from GEOMETRY (not from state0.nt) so it is
        # always correct even if state0 was initialised with a stale/wrong pv.
        # pv_hc = π×r²×h×φ×(1−Sw_init) — a constant for the whole simulation.
        self._pv_hc_init = (self.grid.area_ft2 * self.grid.thickness_ft * self.phi
                            * max(1.0 - float(np.mean(state0.sw)), 0.0))
        # Also recompute nt_init from the CORRECT pv so the correction is anchored
        # to the right material balance. Use Z-factor from initial pressure.
        try:
            _z0 = np.average(state0.z, axis=0, weights=np.ones(self.grid.nx))
            _z0 /= max(float(_z0.sum()), 1e-12)
            _p0 = float(np.mean(state0.pressure))
            _Z0 = float(self.eos.z_factor(_z0, max(_p0, 14.7), self.T, phase="v"))
            _Z0 = max(_Z0, 0.05)
            # nt that is consistent with the CORRECT pv and initial pressure
            self._nt_init_correct = self._pv_hc_init * _p0 / (_Z0 * R * self.T)
            # actual nt from state0 (may differ if pv was wrong at init time)
            self._nt_init_actual  = float(state0.nt.sum())
        except Exception:
            self._nt_init_correct = None
...
def run_example(
    ...
    # Diagnostic: print pv sum so the terminal confirms the correct value.
    import sys as _sys
    _expected_pv = area_ft2 * thickness_ft * porosity
    _ratio = sim.pv.sum() / max(_expected_pv, 1.0)
    print(f"[PV CHECK] sim.pv.sum()={sim.pv.sum():,.0f} ft³  expected={_expected_pv:,.0f} ft³  ratio={_ratio:.4f}",
...
def display_streamlit_app() -> None:
    ...
    st.caption("Build: pv-fix-2026-04-19 · pv = π×r²×h×φ in __init__")
    ...
    with tab_case_setup:
        ...
        dt_days = st.number_input("Requested time step (days)", min_value=0.005, max_value=1000.0, value=30.0, step=1.0, key="dt_days")
    ...
    with tab_reservoir_grid:
        ...
        min_dx_ft = st.number_input("Minimum near-well cell size (ft)", min_value=1.0, max_value=500.0, value=50.0, step=5.0, key="min_dx_ft")
