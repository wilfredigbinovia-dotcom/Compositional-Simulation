...
class CompositionalSimulator1D:
    ...
    def step_once(self, state: SimulationState, dt_days: float) -> Tuple[SimulationState, Dict[str, float]]:
        ...
        # Pressure correction: compute the correct average pressure from the
        # mole ratio (nt_new/nt_old) and the step-start pressure, then apply
        # as a uniform scalar shift on p_pred to preserve the spatial gradient.
        #
        # Key: use state.pressure (step-start, not p_pred) as p_old so the
        # correction does not compound across steps. The ratio nt_new/nt_old
        # is computed from state.nt (before transport) vs tr.nt (after transport),
        # both produced in this same step — always self-consistent, no stored state.
        try:
            nt_old = float(state.nt.sum())
            nt_new = float(tr.nt.sum())
            if nt_old > 1e-6 and nt_new > 0.0:
                p_old_avg = float(np.mean(state.pressure))  # step-start, not p_pred
                if p_old_avg > 14.7:
                    ...
                    for _ in range(12):
                        try:
                            ...
                            break
                        p_iter = pZ_new * Z_new
                        if abs(p_iter - p_new_avg) < 0.1:
                            break
                        p_new_avg = 0.5 * (p_new_avg + p_iter)
                    # Shift p_pred by the difference between MB-correct and
                    # p_pred average. Clamp to ±200 psia per step for stability.
                    dp = float(np.clip(p_new_avg - float(np.mean(p_pred)), -200.0, 200.0))
                    min_p = max(14.7, min(
    ...
    def run(self, state0: SimulationState, t_end_days: float, dt_days: float) -> Tuple[SimulationState, SimulationHistory]:
        ...
        # Store fixed initial references for the pressure correction.
        # pZ_init_ref = p0/Z0, nt_init_ref = nt0 — both from state0.
        # The correction computes pZ_current = pZ_init_ref * (nt_current / nt_init_ref)
        # and derives p_current. Because both pZ and nt are anchored to the SAME
        # initial state, they are always self-consistent regardless of pv.
        # This avoids oscillation (step-to-step ratio compounds errors) and avoids
        # geometry dependence (no pv_hc needed).
        try:
            _z0 = np.average(state0.z, axis=0, weights=state0.nt)
            _z0 /= max(float(_z0.sum()), 1e-12)
            _p0 = float(np.mean(state0.pressure))
            _Z0 = float(self.eos.z_factor(_z0, max(_p0, 14.7), self.T, phase="v"))
            _Z0 = max(_Z0, 0.05)
            self._pZ_init_ref = _p0 / _Z0
            self._nt_init_ref = float(state0.nt.sum())
            self._z_init_ref  = _z0
        except Exception:
            self._pZ_init_ref = None
            self._nt_init_ref = None
            self._z_init_ref  = None
...
def display_streamlit_app() -> None:
    ...
    st.caption("Build: v3-stepstart-2026-04-20 · mole-ratio anchored to step-start pressure")
