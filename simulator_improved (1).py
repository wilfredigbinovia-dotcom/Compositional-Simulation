FILE_VERSION = "fix80-deployed-2026-04-20-v1"
...
def run_example(
    ...
    # === DEPLOYMENT DIAGNOSTIC — remove after confirming correct version runs ===
    import streamlit as _st_diag
    _pv_sum = float(sim.pv.sum())
    _nt_sum = float(state0.nt.sum())
    _R_diag = 10.7316; _T_diag = temperature_R; _SCF_diag = _R_diag*519.67/14.7
    _Z_diag = 0.87
    _GIIP_diag = _nt_sum * _SCF_diag / 1e9
    try:
        _st_diag.sidebar.markdown("---")
        _st_diag.sidebar.markdown(f"**Diagnostic (v{FILE_VERSION})**")
        _st_diag.sidebar.markdown(f"pv.sum = {_pv_sum:,.0f} ft³")
        _st_diag.sidebar.markdown(f"nt_init = {_nt_sum:,.0f} lbmol")
        _st_diag.sidebar.markdown(f"GIIP = {_GIIP_diag:.2f} Bscf")
        _st_diag.sidebar.markdown(f"Expected GIIP ≈ 86 Bscf")
        _st_diag.sidebar.markdown("✅ OK" if abs(_GIIP_diag - 86) < 5 else "❌ WRONG VERSION")
    except Exception:
        pass
    # === END DIAGNOSTIC ===
