...
def display_streamlit_app() -> None:
    ...
    st.sidebar.caption("Model inputs are grouped into expandable sidebar sections for easier navigation. Inputs now stack vertically in the sidebar for a cleaner CMG-style setup flow.")

    with st.sidebar.expander("1. Case Setup & Reservoir / Grid", expanded=True):
    ...
    with st.sidebar.expander("2. Rock, Wells & Rock-Fluid", expanded=False):
    ...
    with st.sidebar.expander("3. PVT, Separator & Schedule", expanded=False):
        c1, c2, c3 = st.columns(3)
        with c1:
    ...
    with st.sidebar.expander("4. Components & Composition", expanded=False):
    ...
    with st.sidebar.expander("5. Analysis Tools & Run Model", expanded=False):
