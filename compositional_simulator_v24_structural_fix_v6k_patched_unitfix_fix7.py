...
class CompositionalSimulator1D:
    ...
    def separator_rates(self, response: Dict[str, float | np.ndarray]) -> Dict[str, float]:
        ...
        # Primary field reporting should not be driven directly by the one-stage
        # separator split because that can create non-physical switching noise.
        # Report gas from the wellstream gas phase, and report condensate using
        # a user-supplied pressure-dependent CGR table whenever available.
        ...
        pressure_for_cgr = float(response.get("pwf_psia", response.get("pr_psia", self.separator.pressure_psia)))
        ...
        if cgr_stb_mmscf is not None and gas_rate_mmscf_day > 0.0:
            condensate_rate_stb_day = max(float(cgr_stb_mmscf), 0.0) * gas_rate_mmscf_day
        ...
        return {
            ...
            "cgr_stb_mmscf": float(cgr_stb_mmscf) if cgr_stb_mmscf is not None else float("nan"),
