...
def beta_to_flowing_saturations(
    ...
    beta_lo: float = 0.05,
    beta_hi: float = 0.95,
    min_gas_sat: float = 0.12,
) -> Tuple[float, float, float]:
    """
    Smoothly map flash vapor fraction beta to flowing gas/oil saturations.

    The previous direct mapping:
        Sg = (1 - Sw) * beta
        So = (1 - Sw) * (1 - beta)

    was too sharp and caused abrupt krg/kro regime switching.
    """
    ...
    Sg = min_gas_sat * Shc + (Shc - min_gas_sat * Shc) * t
    So = Shc - Sg
...
class CompositionalSimulator1D:
    ...
    def well_response_at_pwf(self, state: SimulationState, pwf_psia: float) -> Dict[str, float | np.ndarray]:
        ...
        if lam_hc > 1e-12:
            fg_hc = lam_g / lam_hc
            min_fg_hc = 0.05 if beta > 1e-6 else 0.0
