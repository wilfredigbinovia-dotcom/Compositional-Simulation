...
class CompositionalSimulator1D:
    ...
    def run(self, state0: SimulationState, t_end_days: float, dt_days: float) -> Tuple[SimulationState, SimulationHistory]:
        ...
        while t <= t_end_days + 1e-12:
            ...
            history.reported_gas_rate_mmscf_day.append(float(response0.get("reported_gas_rate_mmscf_day", float(response0["qg_lbmol_day"]) * SCF_PER_LBMOL * MM_PER_ONE)))
...
def display_streamlit_app() -> None:
    ...
    raw_reported_gas_rate = np.asarray(history.reported_gas_rate_mmscf_day, dtype=float)
    if well.control_mode == "gas_rate":
        gas_rate_plot = raw_reported_gas_rate
    else:
        gas_rate_plot = pd.Series(raw_reported_gas_rate).rolling(window=3, center=True, min_periods=1).mean().to_numpy()
