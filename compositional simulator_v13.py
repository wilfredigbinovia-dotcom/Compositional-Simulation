...
class CompositionalSimulator1D:
    ...
    def reported_well_response(self, state: SimulationState) -> Dict[str, float | np.ndarray]:
        ...
        if self.well.control_mode in ("bhp", "thp"):
            response = dict(response)
...
def display_streamlit_app() -> None:
    ...
    gas_rate_plot = pd.Series(history.reported_gas_rate_mmscf_day).rolling(window=3, center=True, min_periods=1).mean().to_numpy()
