...
class CompositionalSimulator1D:
    ...
    def pressure_update(self, state: SimulationState, dt_days: float) -> np.ndarray:
        ...
        Vb = self.grid.area_ft2 * self.grid.dx_array  # bulk volume per cell [ft³]
        pv_cell = self.phi * Vb                        # pore volume per cell [ft³]
        ...
        for i in range(n):
            ...
            rho_cell[i] = Pi / max(Zi * R * self.T, 1e-12) * FT3_PER_STB  # lbmol/RB (5.6146 ft³/RB)
            cg_i = 1.0 / Pi           # 1/psi
            ct_i = cg_i + cr          # 1/psi
        ...
        # so J_w = WI * lam_t * rho  [lbmol/day/psi] — no extra DARCY needed.
        orig_well = self.well
        for _w in self.wells:
            self.well = _w
            self._mobility_cache = {}
            ...
            _Jw  = self.peaceman_well_index(_wi) * _mob["lam_t"] * rho_cell[_wi]
        ...
        # Injector well coupling (same: WI contains 0.00708)
        injector     = self.active_injector()
        ...
        if injector is not None and self.scenario.name != "natural_depletion" and actual_inj_rate > 0.0:
            ...
            Jin     = self.peaceman_well_index(ii) * inj_mob["lam_t"] * rho_cell[ii]
    ...
    def transport_update(self, state: SimulationState, dt_days: float) -> TransportResult:
        ...
        for i in range(nx - 1):
            ...
            pv_min = max(min(self.pv[i], self.pv[i + 1]), 1e-12)
            if hc_flux_mag > 0.0:
                max_cfl_per_day = max(max_cfl_per_day, hc_flux_mag / pv_min)
