...
def display_streamlit_app() -> None:
    ...
    with input_tabs[0]:
        ...
        with col2:
            ...
            area_ft2 = math.pi * radius_ft ** 2
            equivalent_width_ft = area_ft2 / thickness_ft
        ...
        st.info(
            ...
            "Reservoir radius is converted internally to equivalent cross-sectional area using A = πr². "
    ...
    with result_tabs[0]:
        ...
        d1.write(f"Reservoir radius (ft): {radius_ft:,.2f}")
        d2.write(f"Equivalent cross-sectional area (ft²): {area_ft2:,.2f}")
        d3.write(f"Near-well minimum dx (ft): {grid.cell_width(well.cell_index):.3f}")
        d4.write(f"Peaceman WI geom: {sim.peaceman_well_index(well.cell_index):.5f}")
