...
class CompositionalSimulator1D:
    ...
    def step_once(self, state: SimulationState, dt_days: float) -> Tuple[SimulationState, Dict[str, float]]:
        ...
        try:
            ...
            # Compute pv_hc directly from grid geometry — avoids any dependency
            # on self.pv or self._pv which may not be correctly set in all code paths.
            # pv_hc = π×r²×h×φ×(1−Sw)  — the correct cylindrical HC pore volume.
            _pv_hc_tot = (self.grid.area_ft2 * self.grid.thickness_ft * self.phi
                          * max(1.0 - float(np.mean(tr.sw)), 0.0))

            if _pv_hc_tot > 1e-6 and nt_total > 0.0:
                pZ_avg    = nt_total * R * self.T / _pv_hc_tot
