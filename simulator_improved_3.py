    ...
    def run(self, state0: SimulationState, t_end_days: float, dt_days: float) -> Tuple[SimulationState, SimulationHistory]:
        ...
        while t <= t_end_days + 1e-12:
            ...
            if _mbd and t < accepted_dt * 3:
                import sys
                print(f"[MB t={t:.0f}d] nt={_mbd.get('nt_total',0):.3e} pv_hc={_mbd.get('pv_hc_tot',0):.3e} "
                      f"pZ={_mbd.get('pZ_target',0):.1f} p_start={_mbd.get('p_start',0):.1f} "
                      f"p_mb={_mbd.get('p_mb',0):.1f} dp={_mbd.get('dp_corr',0):.1f} "
                      f"p_final={_mbd.get('p_final',0):.1f} exc={_mbd.get('exception','none')}",
