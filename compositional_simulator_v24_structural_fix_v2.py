...
def beta_to_flowing_saturations(
    ...
    sg_crit = float(np.clip(params.Sgc + 0.015, 0.0, 0.98))
    sg_floor_dynamic = sg_crit * smoothstep(beta / 0.12) if beta > 1e-8 else 0.0
...
class CompositionalSimulator1D:
    ...
    def well_response_at_pwf(self, state: SimulationState, pwf_psia: float) -> Dict[str, float | np.ndarray]:
        ...
        if lam_hc > 1e-12 and hc_frac > 1e-12:
            ...
            # Keep the hydrocarbon split dominated by mobility, but impose a
            # modest flash-informed lower bound so the stream does not become
            # oil-only while flashed gas still exists.
            fg_hc_blend = 0.70 * fg_hc_mob + 0.30 * fg_hc_flash
            min_fg_hc = 0.12 * fg_hc_flash if fg_hc_flash > 1e-6 else 0.0
