...
class CompositionalSimulator1D:
    ...
    def transport_update(self, state: SimulationState, dt_days: float) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, float, float, float, float, float, float, float, float, float, float, float, float]:
        ...
        are refreshed within a reporting timestep. Rather than hard-capping the
        producer withdrawal, choose the number of explicit substeps from a local
        producer-cell CFL estimate so no single substep removes too much of the
        well-cell inventory.
        """
        max_substep_days = 10.0
        base_n_sub = max(1, min(4, int(math.ceil(dt_days / max_substep_days))))
        ...
        target_total_frac = 0.10
        target_comp_frac = 0.12
        target_water_frac = 0.10
