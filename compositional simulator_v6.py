...
- Editable Corey relative permeability parameters from the sidebar
...
- No explicit aqueous phase: water production is currently reported as zero
...
P_STD_PSIA = 14.7
T_STD_R = 519.67  # 60 F
SCF_PER_LBMOL = R * T_STD_R / P_STD_PSIA  # ~379.49 scf/lbmol
MM_PER_ONE = 1.0e-6

# Approximate liquid reporting assumptions
...
def format_time_axis(time_days: List[float], unit: str) -> np.ndarray:
    ...
    if unit not in factors:
        raise ValueError(f"Unsupported time unit: {unit}")
    return np.asarray(time_days, dtype=float) / factors[unit]
...
class CompositionalSimulator1D:
    ...
    def phase_mobility_data(self, state: SimulationState, i: int) -> Dict[str, float]:
        ...
        Sl = 1.0 - Sg
        krg, krl = corey_relperm(
            Sg,
            Sgc=self.relperm.Sgc,
            Slr=self.relperm.Slr,
            ng=self.relperm.ng,
            nl=self.relperm.nl,
        )
    ...
    def run(self, state0: SimulationState, t_end_days: float, dt_days: float) -> Tuple[SimulationState, SimulationHistory]:
        ...
        while t <= t_end_days + 1e-12:
            ...
            condensate_rate_stb_day = liquid_lbmol_day_to_stb_day(
                ql_lbmol_day,
                liquid_comp=x,
                mw_components=self.mw_components,
                density_lbft3=CONDENSATE_DENSITY_LBFT3,
            )
...
def plot_history(history: SimulationHistory, output_dir: Path) -> None:
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(output_dir / "gas_rate_history.png", dpi=200)
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(output_dir / "liquid_rate_history.png", dpi=200)
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(output_dir / "cum_gas_history.png", dpi=200)
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(output_dir / "cum_liquid_history.png", dpi=200)
    plt.close()
...
def run_example(
    ...
    relperm = RelPermParams(Sgc=Sgc, Slr=Slr, ng=ng, nl=nl)
...
def display_streamlit_app() -> None:
    ...
    st.sidebar.subheader("Relative Permeability")
    Sgc = st.sidebar.number_input(
    ...
    nl = st.sidebar.number_input(
        "Liquid Corey exponent, nl",
        min_value=0.1,
        max_value=10.0,
        value=2.0,
        step=0.1,
    ...
    if Sgc + Slr >= 1.0:
        st.error("Invalid relative permeability inputs: Sgc + Slr must be less than 1.0.")
        return
    ...
    st.sidebar.write("Water reporting: zero-output placeholder (no aqueous phase model yet)")
    ...
    if st.button("Run Simulation", type="primary"):
        ...
        c4.metric("Final Condensate Rate (STB/day)", f"{history.condensate_rate_stb_day[-1]:.3f}")
        c5.metric("Final Water Rate (STB/day)", f"{history.water_rate_stb_day[-1]:.3f}")
        c6.metric("Final Cumulative Gas (Bscf)", f"{history.cum_gas_bscf[-1]:.6f}")

        c7, c8, c9 = st.columns(3)
        c7.metric("Final Cumulative Condensate (MSTB)", f"{history.cum_condensate_mstb[-1]:.6f}")
        c8.metric("Dropout Indicator", f"{history.well_dropout_indicator[-1]:.4f}")
        c9.metric("Damage Factor", f"{history.well_damage_factor[-1]:.4f}")

        c10, c11, c12 = st.columns(3)
        c10.metric("Effective WI", f"{history.well_effective_wi[-1]:.5f}")
        c11.metric("Undamaged Rate (lbmol/day)", f"{history.well_rate_undamaged[-1]:.6f}")
        c12.metric("Well Gas Fraction", f"{history.well_gas_fraction[-1]:.4f}")

        st.subheader("Relative Permeability Settings")
        rp1, rp2, rp3, rp4 = st.columns(4)
        rp1.metric("Sgc", f"{Sgc:.2f}")
        rp2.metric("Slr", f"{Slr:.2f}")
        rp3.metric("ng", f"{ng:.1f}")
        rp4.metric("nl", f"{nl:.1f}")
        ...
        st.write(f"Relative permeability inputs: Sgc={Sgc:.2f}, Slr={Slr:.2f}, ng={ng:.1f}, nl={nl:.1f}")
        ...
        st.caption(
            ...
            "Condensate rates are approximate STB/day estimates derived from liquid molar rate, mixture molecular weight, "
            "and an assumed condensate density of 50 lbm/ft³. Water is currently zero because no aqueous phase is modeled."
        )
    else:
