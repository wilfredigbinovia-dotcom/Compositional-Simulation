...
def parse_deviation_survey_text(text: str | None) -> tuple[np.ndarray | None, np.ndarray | None]:
    if text is None:
        return None, None
    ...
    md = np.array([r[0] for r in rows], dtype=float)
    tvd = np.array([r[1] for r in rows], dtype=float)
    keep = np.concatenate([[True], np.diff(md) > 1e-9])
    md = md[keep]
    tvd = tvd[keep]
    if len(md) < 2 or md[-1] <= md[0]:
        return None, None
    if md[0] > 1e-9:
        md = np.insert(md, 0, 0.0)
...
def liquid_stb_day_to_lbmol_day(rate_stb_day: float, liquid_comp: np.ndarray, mw_components: np.ndarray, density_lbft3: float = CONDENSATE_DENSITY_LBFT3) -> float:
    comp = np.asarray(liquid_comp, dtype=float)
    comp = comp / max(np.sum(comp), 1e-12)
    mw = float(np.dot(comp, mw_components))
    mass_lb_day = max(rate_stb_day, 0.0) * FT3_PER_STB * density_lbft3
    return mass_lb_day / max(mw, 1e-12)

ScenarioName = Literal["natural_depletion", "gas_cycling", "lean_gas_injection"]
WellControlMode = Literal["bhp", "drawdown", "gas_rate", "thp", "top_down_vlp"]
...
class PengRobinsonEOS:
    ...
    def fugacity_coefficients(self, x: np.ndarray, P: float, T: float, phase: str) -> np.ndarray:
        ...
        # Vectorised over all components — no Python loop needed.
        bi_over_bm = bi / max(bm, 1e-12)
        log_ZmB = math.log(max(Z - B, 1e-12))
        ratio = (Z + (1.0 + sqrt2) * B) / max(Z + (1.0 - sqrt2) * B, 1e-12)
        log_ratio = math.log(max(ratio, 1e-12))
        coeff = A / max(2.0 * sqrt2 * B, 1e-12) * log_ratio

        ln_phi = (bi_over_bm * (Z - 1.0)
                  - log_ZmB
                  - coeff * (2.0 * sum_aij / max(am, 1e-12) - bi_over_bm))
...
class FlashCalculator:
    ...
    def flash(self, z: np.ndarray, P: float, T: float, max_iter: int = 50, tol: float = 1e-7) -> Dict[str, np.ndarray | float | str]:
        ...
        # Successive substitution with GDEM (Dominant Eigenvalue Method) acceleration.
        # Every gdem_freq iterations, the K-factor update is extrapolated along the
        # dominant eigenvector of the successive-substitution mapping, which dramatically
        # reduces iterations near the critical point where plain SS stalls.
        gdem_freq = 5
        lnK = np.log(np.maximum(K, 1e-30))
        lnK_prev = lnK.copy()
        d_prev = np.zeros_like(lnK)
        ...
        for it in range(max_iter):
            ...
            lnK_new = np.log(phi_l / np.maximum(phi_v, 1e-12))
            lnK_new = np.clip(lnK_new, -18.0, 18.0)

            d = lnK_new - lnK
            err = float(np.max(np.abs(d)))

            # GDEM acceleration every gdem_freq steps after the first
            if it > 0 and it % gdem_freq == 0:
                dd = d - d_prev
                denom_gdem = float(np.dot(dd, dd))
                if denom_gdem > 1e-30:
                    lam = -float(np.dot(d_prev, dd)) / denom_gdem
                    lam = float(np.clip(lam, -5.0, 20.0))
                    lnK_new = lnK + (1.0 + lam) * d
                    lnK_new = np.clip(lnK_new, -18.0, 18.0)
...
class Well:
    ...
    rw_ft: float = 0.35
    skin: float = 0.0
    deviation_md_ft: np.ndarray | None = None
    deviation_tvd_ft: np.ndarray | None = None
...
class CompositionalSimulator1D:
    def __init__(
        ...
        detailed_mass_balance_diagnostics: bool = False,
    ):
        self.grid = grid
        self.rock = rock
        self.fluid = fluid
        self.eos = eos
        self.flash = flash
        # Support list of wells; keep self.well as alias to first well for
        # backward-compatibility with all single-well code paths.
        if isinstance(well, list):
            self.wells: List[Well] = well if well else [well]
        else:
            self.wells = [well]
        ...
        self.detailed_mass_balance_diagnostics = bool(detailed_mass_balance_diagnostics)
        ...
        # Secondary content-addressed cache: survives state object identity changes
        # when pressure/composition haven't actually changed (e.g. retry steps).
        self._flash_content_cache: Dict[tuple, Dict[str, np.ndarray | float | str]] = {}
        # Well response cache: solve_well_control is called multiple times per
        # timestep (pressure_update + reporting). Cache result for the same state.
        self._well_response_cache: Dict[int, Dict[str, float | np.ndarray]] = {}
    ...
    def _invalidate_caches(self) -> None:
        ...
        self._well_response_cache = {}
        # Content cache is kept — it is invalidated only when it grows too large
        if len(self._flash_content_cache) > 4096:
            self._flash_content_cache = {}

    def _set_cache_state(self, state: SimulationState) -> None:
        self._cache_state_id = id(state)
        self._invalidate_caches()

    def _flash_content_key(self, state: SimulationState, i: int) -> tuple:
        """Cheap content fingerprint: rounded pressure + first/last/mid z values."""
        p = round(float(state.pressure[i]), 2)
        z = state.z[i]
        nc = len(z)
        mid = nc // 2
        return (i, p, round(float(z[0]), 8), round(float(z[mid]), 8), round(float(z[-1]), 8))
    ...
    def cell_flash_cached(self, state: SimulationState, i: int) -> Dict[str, np.ndarray | float | str]:
        ...
        if i in self._flash_cache:
            return self._flash_cache[i]
        # Fall back to content-addressed cache to avoid recomputing identical flashes
        # (e.g. when the same state is accessed via different object identities during
        # adaptive timestep retries).
        ck = self._flash_content_key(state, i)
        if ck in self._flash_content_cache:
            result = self._flash_content_cache[ck]
            self._flash_cache[i] = result
            return result
        result = self.flash.flash(state.z[i], state.pressure[i], self.T)
        self._flash_cache[i] = result
        self._flash_content_cache[ck] = result
        return result
    ...
    def _wellbore_profile_md_tvd(self) -> Tuple[np.ndarray, np.ndarray]:
        nseg = max(int(getattr(self.well, "tubing_segments", 25)), 2)
        md = getattr(self.well, "deviation_md_ft", None)
        tvd = getattr(self.well, "deviation_tvd_ft", None)
        if md is not None and tvd is not None:
            md_arr = np.asarray(md, dtype=float)
            tvd_arr = np.asarray(tvd, dtype=float)
            if md_arr.ndim == 1 and tvd_arr.ndim == 1 and len(md_arr) >= 2 and len(md_arr) == len(tvd_arr) and md_arr[-1] > md_arr[0]:
                return md_arr, tvd_arr
        md_arr = np.linspace(0.0, max(float(self.well.tvd_ft), 1.0), nseg + 1)
        return md_arr, md_arr.copy()

    def _top_down_pvt_gor_profile(self, state: SimulationState) -> Dict[str, float | np.ndarray]:
        qg_mmscf_day = max(float(self.well.target_gas_rate_mmscf_day), 0.0)
        qg_lbmol_day = gas_mmscf_day_to_lbmol_day(qg_mmscf_day)
        qg_scf_day = qg_mmscf_day * 1.0e6
        p_local = max(float(self.well.thp_psia), 14.7)

        md_nodes, tvd_nodes = self._wellbore_profile_md_tvd()
        nseg = max(int(getattr(self.well, "tubing_segments", 25)), 2)
        md_path = np.linspace(float(md_nodes[0]), float(md_nodes[-1]), nseg + 1)
        tvd_path = np.interp(md_path, md_nodes, tvd_nodes)
        ...
        z_ref = np.asarray(state.z[self.well.cell_index], dtype=float)
        z_ref = z_ref / max(np.sum(z_ref), 1e-12)
        x_ref = z_ref.copy()
        y_ref = z_ref.copy()
        try:
            fl_ref = self.flash.flash(z_ref, max(float(state.pressure[self.well.cell_index]), 14.7), self.T)
            x_ref = np.asarray(fl_ref["x"], dtype=float)
            y_ref = np.asarray(fl_ref["y"], dtype=float)
        except Exception:
            pass
        ...
        regime_samples: List[str] = []
        prev_specific_volume: float | None = None
        prev_mass_flux: float | None = None

        for seg in range(nseg):
            dmd_ft = max(float(md_path[seg + 1] - md_path[seg]), 1e-9)
            dtvd_ft = max(float(tvd_path[seg + 1] - tvd_path[seg]), 0.0)
            frac = (seg + 0.5) / nseg
            t_local = self.well.wellhead_temperature_R + frac * (self.T - self.well.wellhead_temperature_R)

            p_ref = max(p_local, 14.7)
            cgr = self.pvt_table.interp(p_ref, "vaporized_cgr_stb_mmscf", default=None) if self.pvt_table is not None else None
            if cgr is None or cgr <= 1e-9:
                qo_stb_day = 0.0
            else:
                qo_stb_day = self.well.target_gas_rate_mmscf_day * cgr

            bg = self.pvt_table.interp(p_ref, "gas_fvf_ft3_scf", default=None) if self.pvt_table is not None else None
            if bg is None or bg <= 0.0:
                gas_vol_ft3_day = hydrocarbon_gas_rate_ft3_day(qg_lbmol_day, y_ref, p_ref, t_local, self.eos)
            else:
                gas_vol_ft3_day = qg_scf_day * bg

            bo = self.pvt_table.interp(p_ref, "oil_fvf_rb_stb", default=1.0) if self.pvt_table is not None else 1.0
            oil_vol_ft3_day = max(qo_stb_day, 0.0) * max(bo, 0.2) * FT3_PER_STB
            vsg = gas_vol_ft3_day / max(area_ft2 * 86400.0, 1e-12)
            vsl = oil_vol_ft3_day / max(area_ft2 * 86400.0, 1e-12)
            vm = vsg + vsl

            rho_g = self.pvt_table.interp(p_ref, "gas_density_lbft3", default=None) if self.pvt_table is not None else None
            if rho_g is None or rho_g <= 0.0:
                rho_g = gas_density_lbft3_from_eos(self.eos, y_ref, p_ref, t_local)
            _rho_sc2 = self.reporting.condensate_density_lbft3
            rho_l = float(np.clip(_rho_sc2 * (1.0 + 5.0e-6 * max(p_local - 14.7, 0.0)), _rho_sc2, 65.0))
            hl, regime = regime_liquid_holdup(vsg, vsl, diameter_ft, rho_l, rho_g)
            rho_mix = hl * rho_l + (1.0 - hl) * rho_g

            mu_g_cp = self.pvt_table.interp(p_ref, "gas_viscosity_cp", default=None) if self.pvt_table is not None else None
            if mu_g_cp is None or mu_g_cp <= 0.0:
                mu_g_cp = phase_viscosity_cp("v", y_ref, p_ref, t_local)
            mu_o_cp = self.pvt_table.interp(p_ref, "oil_viscosity_cp", default=None) if self.pvt_table is not None else None
            if mu_o_cp is None or mu_o_cp <= 0.0:
                mu_o_cp = phase_viscosity_cp("l", x_ref, p_ref, t_local)
            mu_mix_cp = max(hl * mu_o_cp + (1.0 - hl) * mu_g_cp, 1e-6)
            ...
            segment_hydro_psi = hydro_grad_psi_ft * dtvd_ft
            segment_friction_psi = friction_grad_psi_ft * dmd_ft
            segment_accel_psi = 0.0
            if prev_specific_volume is not None and prev_mass_flux is not None:
                g_mass = 0.5 * (prev_mass_flux + mass_flux)
                d_specific_volume_dL = (specific_volume - prev_specific_volume) / dmd_ft
                segment_accel_psi = max((g_mass ** 2) * d_specific_volume_dL * dmd_ft / max(32.174 * 144.0, 1e-12), 0.0)

            hydrostatic_psi += segment_hydro_psi
            friction_psi += segment_friction_psi
            accel_psi += segment_accel_psi
            velocity_samples.append(float(vm))
            reynolds_samples.append(float(reynolds))
            friction_factor_samples.append(float(friction_factor))
            regime_samples.append(regime)

            p_local = max(p_local + segment_hydro_psi + segment_friction_psi + segment_accel_psi, 14.7)
            prev_specific_volume = specific_volume
            prev_mass_flux = mass_flux

        pwf = float(p_local)
        result = self.well_response_at_pwf(state, pwf)
        target_qg_lbmol_day = gas_mmscf_day_to_lbmol_day(self.well.target_gas_rate_mmscf_day)
        result["qg_lbmol_day"] = float(target_qg_lbmol_day)
        qo_stb_day_bottom = 0.0
        if self.pvt_table is not None:
            cgr_bottom = self.pvt_table.interp(pwf, "vaporized_cgr_stb_mmscf", default=None)
            if cgr_bottom is not None and cgr_bottom > 1e-9:
                qo_stb_day_bottom = self.well.target_gas_rate_mmscf_day * cgr_bottom
        result["qo_lbmol_day"] = float(liquid_stb_day_to_lbmol_day(qo_stb_day_bottom, np.asarray(result["x"], dtype=float), self.mw_components, self.reporting.condensate_density_lbft3))
        qt = max(float(result["qg_lbmol_day"]) + float(result["qo_lbmol_day"]) + float(result.get("qw_lbmol_day", 0.0)), 1e-12)
        result["q_total_lbmol_day"] = qt
        result["gas_frac"] = float(result["qg_lbmol_day"] / qt)
        result["oil_frac"] = float(result["qo_lbmol_day"] / qt)
        result["water_frac"] = float(float(result.get("qw_lbmol_day", 0.0)) / qt)
        result["hc_sink"] = float(result["qg_lbmol_day"]) * np.asarray(result["y"], dtype=float) + float(result["qo_lbmol_day"]) * np.asarray(result["x"], dtype=float)
        result["pwf_psia"] = pwf
        result["control_mode"] = "top_down_vlp"
        result["raw_estimated_thp_psia"] = float(self.well.thp_psia)
        result["estimated_thp_psia"] = float(max(self.well.thp_psia, 14.7))
        result["thp_clipped_to_atmospheric"] = float(self.well.thp_psia < 14.7)
        result["lift_feasible"] = 1.0
        result["lift_rate_feasible"] = 1.0
        result["reported_gas_rate_mmscf_day"] = float(self.well.target_gas_rate_mmscf_day)
        result["raw_bottomhole_pressure_from_topdown_psia"] = pwf
        result["tubing_hydrostatic_psi"] = float(max(hydrostatic_psi, 0.0))
        result["tubing_friction_psi"] = float(max(friction_psi, 0.0))
        result["tubing_acceleration_psi"] = float(max(accel_psi, 0.0))
        result["tubing_mixture_velocity_ft_s"] = float(np.mean(velocity_samples)) if velocity_samples else 0.0
        result["tubing_reynolds_number"] = float(np.mean(reynolds_samples)) if reynolds_samples else 0.0
        result["tubing_friction_factor"] = float(np.mean(friction_factor_samples)) if friction_factor_samples else 0.0
        if regime_samples:
            result["tubing_flow_regime"] = max(set(regime_samples), key=regime_samples.count)
        return result
    ...
    def tubing_pressure_profile_from_response(self, state: SimulationState, response: Dict[str, float | np.ndarray]) -> Dict[str, float]:
        ...
        def segment_state_props(p_seg_psia: float, t_seg_R: float) -> Dict[str, float | np.ndarray | str]:
            ...
            rho_o_sc = self.reporting.condensate_density_lbft3
            co_psi = 5.0e-6  # isothermal compressibility for light condensate
            rho_o = float(np.clip(rho_o_sc * (1.0 + co_psi * max(p_local - 14.7, 0.0)), rho_o_sc, 65.0))
        ...
        # Pre-compute the first segment entry so we can roll props forward,
        # halving the number of flash calls from 2*nseg to nseg+1.
        t_entry = self.T + 0.5 / nseg * (self.well.wellhead_temperature_R - self.T)
        t_entry = float(np.clip(t_entry, min(self.T, self.well.wellhead_temperature_R), max(self.T, self.well.wellhead_temperature_R)))
        props_here = segment_state_props(p_local, t_entry)
        ...
        for seg in range(nseg):
            ...
            p_local = p_next
            # Roll forward: next iteration's "here" is this iteration's "next"
            props_here = props_next
    ...
    def solve_well_control(self, state: SimulationState) -> Dict[str, float | np.ndarray]:
        sid = id(state)
        if self._cache_state_id == sid and sid in self._well_response_cache:
            return self._well_response_cache[sid]
        if self._cache_state_id != sid:
            self._set_cache_state(state)
        ...
        if mode == "bhp":
            ...
            if "estimated_thp_psia" not in response:
                response.update(self.tubing_pressure_profile_from_response(state, response))
        elif mode == "top_down_vlp":
            response = self._top_down_pvt_gor_profile(state)
            response["control_mode"] = "top_down_vlp"
        else:
            raise ValueError(f"Unsupported well control mode: {mode}")
        self._well_response_cache[sid] = response
        return response
    ...
    def separator_rates(self, response: Dict[str, float | np.ndarray]) -> Dict[str, float]:
        ...
        # Align reported condensate with the actual separator-train stock-tank liquid.
        # Keep the PVT/user CGR and wellstream-derived CGR only as diagnostics.
        if gas_rate_mmscf_day <= 1e-12:
            ...
            effective_cgr_stb_mmscf = stock_tank_liquid_rate_stb_day / gas_rate_mmscf_day
    ...
    def well_sinks_all(self, state: SimulationState):
        """Return aggregated sink arrays and per-well breakdown for all producers.
        Returns:
          agg_hc_sink  : np.ndarray shape (nx, nc) — total HC removed per cell
          agg_qw       : float — total water lbmol/day
          per_well      : list of dicts with per-well scalars for reporting
          agg_scalars  : dict of aggregated scalar diagnostics
        """
        nx = self.grid.nx
        nc = self.nc
        ...
        # Temporarily swap self.well to each producer in turn so that
        # solve_well_control (and its cache) work per-well correctly.
        orig_well = self.well
        for w in self.wells:
            self.well = w
            self._well_response_cache = {}  # invalidate per-well
            hc_sink, qw, q_total, gas_frac, oil_frac, water_frac, dropout, krg, kro, krw, damage_factor, wi_eff, q_undamaged, ploss = self.well_sink(state)
            wi = w.cell_index
            # hc_sink is a composition-weighted vector (nc,); add to the right cell
            agg_hc_sink[wi] += np.asarray(hc_sink, dtype=float)
            agg_qw        += float(qw)
            agg_q_total   += float(q_total)
            agg_qg        += float(q_total) * float(gas_frac)
            agg_qo        += float(q_total) * float(oil_frac)
            agg_dropout   += float(dropout)
            agg_krg       += float(krg)
            agg_kro       += float(kro)
            agg_krw       += float(krw)
            agg_damage    += float(damage_factor)
            agg_wi_eff    += float(wi_eff)
            agg_q_undamaged += float(q_undamaged)
            agg_ploss     += float(ploss)
            per_well.append({
                "cell_index": wi,
                "name": getattr(w, "name", f"Well {len(per_well)+1}"),
                "q_total_lbmol_day": float(q_total),
                "qg_lbmol_day": float(q_total) * float(gas_frac),
                "qo_lbmol_day": float(q_total) * float(oil_frac),
                "qw_lbmol_day": float(qw),
                "gas_frac": float(gas_frac),
                "oil_frac": float(oil_frac),
        ...
        # Build aggregated scalar dict (averages where appropriate, sums otherwise)
        w = max(n_wells, 1)
        gas_frac_agg  = agg_qg / max(agg_q_total, 1e-12)
        oil_frac_agg  = agg_qo / max(agg_q_total, 1e-12)
        water_frac_agg = agg_qw / max(agg_q_total + agg_qw, 1e-12)
        agg_scalars = {
            "q_total_lbmol_day": agg_q_total,
            "qw_lbmol_day": agg_qw,
            "gas_frac": gas_frac_agg,
            "oil_frac": oil_frac_agg,
            "water_frac": water_frac_agg,
            "dropout": agg_dropout / w,
            "krg": agg_krg / w,
            "kro": agg_kro / w,
            "krw": agg_krw / w,
            "damage_factor": agg_damage / w,
            "wi_eff": agg_wi_eff / w,
    ...
    def _total_compressibility_arr(self, p_arr: np.ndarray, sw_arr: np.ndarray) -> np.ndarray:
        """Vectorised total compressibility for all grid cells at once."""
        cf = 4.0e-6
        cw = 3.0e-6
        p_arr = np.asarray(p_arr, dtype=float)
        sw_arr = np.asarray(sw_arr, dtype=float)
        if self.pvt_table is not None and self.pvt_table.has('gas_fvf_ft3_scf'):
            bg_tbl = self.pvt_table.gas_fvf_ft3_scf
            p_tbl  = self.pvt_table.pressure_psia
            bg_arr = np.interp(p_arr, p_tbl, bg_tbl, left=bg_tbl[0], right=bg_tbl[-1])
            dp = np.maximum(p_arr * 0.01, 10.0)
            bg_hi = np.interp(p_arr + dp, p_tbl, bg_tbl, left=bg_tbl[0], right=bg_tbl[-1])
            bg_lo = np.interp(p_arr - dp, p_tbl, bg_tbl, left=bg_tbl[0], right=bg_tbl[-1])
            safe  = bg_lo > 1e-12
            cg = np.where(safe, (bg_hi - bg_lo) / np.maximum(bg_lo * 2.0 * dp, 1e-20), 1.0 / np.maximum(p_arr, 1.0))
        else:
            cg = 1.0 / np.maximum(p_arr, 1.0)
        sg = np.maximum(1.0 - sw_arr, 0.0)
        ct = sg * np.abs(cg) + sw_arr * cw + cf
        return np.clip(ct, 1.0e-6, 5.0e-3)

    def _total_compressibility_psi(self, p_psia: float, sw: float) -> float:
        """Scalar wrapper kept for API compatibility."""
        return float(self._total_compressibility_arr(np.array([p_psia]), np.array([sw]))[0])

    def pressure_update(self, state: SimulationState, dt_days: float) -> np.ndarray:
        p_old = state.pressure.copy()
        n = self.grid.nx

        # Store the tridiagonal system in banded form (3 x n):
        #   ab[0, j] = superdiagonal (A[i, i+1]), undefined for j=0
        #   ab[1, j] = main diagonal (A[i, i])
        #   ab[2, j] = subdiagonal  (A[i+1, i]), undefined for j=n-1
        ab = np.zeros((3, n), dtype=float)
        b = np.zeros(n, dtype=float)

        # Compute ct per-cell from PVT Bg rather than using a hardcoded scalar.
        # For a gas condensate at ~4000 psia, cg ~ 1e-4 psi-1, which is 10x
        # larger than the previous hardcoded value of 1e-5.
        ct_arr = self._total_compressibility_arr(p_old, state.sw)
        acc_scale = ct_arr * self.pv / max(dt_days, 1e-12)

        # Accumulation terms — fully vectorised
        ab[1] += acc_scale
        b += acc_scale * p_old
        ...
        for i in range(n - 1):
            ...
            ab[1, i]     += Tij   # A[i,   i  ]
            ab[1, i + 1] += Tij   # A[i+1, i+1]
            ab[0, i + 1] -= Tij   # A[i,   i+1]  (superdiag, stored at col i+1)
            ab[2, i]     -= Tij   # A[i+1, i  ]  (subdiag,   stored at col i)
        ...
        # Add a diagonal source/sink term for every producer
        orig_well = self.well
        for _w in self.wells:
            ...
            _Jw = self.peaceman_well_index(_wi) * _mob["lam_t"] * 2.5e-3
        ...
        self.well = orig_well
        self._well_response_cache = {}
        # Restore primary well alias so downstream code stays consistent
        response = self.solve_well_control(state)
        ...
        try:
            # O(n) banded solve instead of O(n³) dense solve
            p_solved = solve_banded((1, 1), ab, b, overwrite_ab=True, overwrite_b=True, check_finite=False)
        except Exception:
            p_solved = p_old.copy()
    ...
    def transport_update(self, state: SimulationState, dt_days: float) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float]:
        ...
        # Optional step-integrated production accounting. This is more accurate for
        # mass-balance diagnostics, but adds bookkeeping overhead within each
        # accepted timestep. Leave it off for faster routine forecasting runs.
        hc_removed_lbmol = 0.0
        water_removed_lbmol = 0.0
        gas_removed_lbmol = 0.0
        oil_removed_lbmol = 0.0
        injector_added_lbmol = 0.0
        use_detailed_mb = bool(self.detailed_mass_balance_diagnostics)
        ...
        for _ in range(n_sub):
            ...
            if inj_cell is not None and inj_source is not None:
                ...
                if use_detailed_mb:
                    injector_added_lbmol += float(np.sum(np.asarray(inj_source, dtype=float))) * dt_sub
            ...
            agg_hc_sink, qw, agg_scalars, _per_well = self.well_sinks_all(work_state)
            ...
            # Apply all well sinks to ncomp_new (agg_hc_sink is shape (nx, nc))
            ncomp_new  -= dt_sub * agg_hc_sink
            nw_new[self.well.cell_index] -= dt_sub * float(qw)  # water at primary well cell
            for _pw in _per_well[1:]:
                nw_new[_pw["cell_index"]] -= dt_sub * float(_pw["qw_lbmol_day"])

            hc_rate_lbmol_day = float(np.sum(agg_hc_sink))
            qw_rate_lbmol_day = float(qw)
            qg_rate_lbmol_day = float(q_total) * float(gas_frac)
            qo_rate_lbmol_day = float(q_total) * float(oil_frac)
            if use_detailed_mb:
                hc_removed_lbmol += hc_rate_lbmol_day * dt_sub
                water_removed_lbmol += qw_rate_lbmol_day * dt_sub
                gas_removed_lbmol += qg_rate_lbmol_day * dt_sub
                oil_removed_lbmol += qo_rate_lbmol_day * dt_sub
        ...
        # Convert integrated removal/source totals back to accepted-step average
        # rates only when the detailed mass-balance audit is enabled. Otherwise,
        # return the accepted-step rates from the final refreshed subcycle for speed.
        dt_total = max(float(dt_days), 1e-12)
        if use_detailed_mb:
            avg_hc_rate_lbmol_day = hc_removed_lbmol / dt_total
            avg_w_rate_lbmol_day = water_removed_lbmol / dt_total
            avg_qg_rate_lbmol_day = gas_removed_lbmol / dt_total
            avg_qo_rate_lbmol_day = oil_removed_lbmol / dt_total
            avg_q_total_lbmol_day = avg_hc_rate_lbmol_day + avg_w_rate_lbmol_day
            avg_injector_rate_lbmol_day = injector_added_lbmol / dt_total if injector_added_lbmol > 0.0 else float(injector_actual_rate_lbmol_day)

            if avg_q_total_lbmol_day > 1e-12:
                gas_frac = avg_qg_rate_lbmol_day / avg_q_total_lbmol_day
                oil_frac = avg_qo_rate_lbmol_day / avg_q_total_lbmol_day
                water_frac = avg_w_rate_lbmol_day / avg_q_total_lbmol_day
            else:
                gas_frac = oil_frac = water_frac = 0.0
            q_total = avg_q_total_lbmol_day
        else:
            avg_q_total_lbmol_day = float(q_total)
            avg_injector_rate_lbmol_day = float(injector_actual_rate_lbmol_day)

        return (
            work_state.nt, work_state.z, work_state.nw, work_state.sw,
            avg_q_total_lbmol_day, gas_frac, oil_frac, water_frac,
            dropout, krg, kro, krw, damage_factor, wi_eff, q_undamaged, ploss,
            avg_injector_rate_lbmol_day,
    ...
    def record_mass_balance(self, history: SimulationHistory, state: SimulationState, dt_days: float, q_total: float, gas_frac: float, oil_frac: float, water_frac: float, injector_rate_lbmol_day: float) -> None:
        # Derive cumulative HC production directly from the inventory change in
        # state.nt rather than integrating rate*dt. The rate*dt path uses
        # separator-flash-based wellstream rates which are computed independently
        # of the moles actually removed by well_sink in transport_update, causing
        # the two accounting streams to drift apart and produce a large spurious error.
        #
        # Conservation identity (exact, consistent with transport equations):
        #   nt_current = nt_initial + cum_injected - cum_produced
        #   => cum_produced = nt_initial + cum_injected - nt_current
        #
        # We still integrate injection via rate*dt because injection is added
        # to ncomp_new explicitly and is already consistent with the transport step.
        ...
        # cum_hc_produced is back-calculated from inventory — always consistent.
        self.cum_hc_produced_lbmol = (
            self.initial_hc_lbmol + self.cum_hc_injected_lbmol - hc_current
        )
    ...
    def run(self, state0: SimulationState, t_end_days: float, dt_days: float,
            reporting_dt_days: float | None = None,
            start_date_text: str | None = None,
            year_end_reporting: bool = False) -> Tuple[SimulationState, SimulationHistory]:
        if dt_days <= 0.0:
            raise ValueError("Time step must be positive")
        rep_dt = float(reporting_dt_days) if reporting_dt_days is not None and reporting_dt_days > dt_days else None

        # Build mandatory output targets: 31 Dec of every year inside the window.
        mandatory_output_days: list[float] = []
        if year_end_reporting and start_date_text is not None:
            try:
                from datetime import date as _date
                _start = parse_ddmmyyyy(start_date_text)
                _end_abs = _start + timedelta(days=t_end_days)
                for _yr in range(_start.year, _end_abs.year + 1):
                    _ye = _date(_yr, 12, 31)
                    _ye_days = (_ye - _start.date()).days
                    if 0 < _ye_days < t_end_days:
                        mandatory_output_days.append(float(_ye_days))
            except Exception:
                pass
        mandatory_output_days.append(float(t_end_days))
        mandatory_output_days = sorted(set(mandatory_output_days))
        _next_mandatory_idx = 0
        ...
        while t <= t_end_days + 1e-12:
            ...
            while _next_mandatory_idx < len(mandatory_output_days) and mandatory_output_days[_next_mandatory_idx] <= t + 1e-6:
                _next_mandatory_idx += 1
            if _next_mandatory_idx < len(mandatory_output_days):
                _snap = mandatory_output_days[_next_mandatory_idx] - t
                next_dt = min(rep_dt if rep_dt is not None else dt_days, _snap, t_end_days - t)
            else:
            ...
            if next_dt <= 1e-12:
                ...
                history.reported_gas_rate_mmscf_day.append(float(gas_rate_mmscf_day))
            ...
            # Advance by up to one reporting interval using multiple adaptive sub-steps.
            report_target = t + (rep_dt if rep_dt is not None else next_dt)
            report_target = min(report_target, t_end_days)
            chunk_dt = report_target - t

            if rep_dt is not None:
                # Sub-step within the reporting interval using dt_days as the trial step.
                t_chunk = t
                state_chunk = state
                total_retries = 0
                summary = None
                ...
                while remaining > 1e-6:
                    sub_dt = min(dt_days, remaining)
                    state_chunk, summary, sub_accepted, sub_retries = self.adaptive_step(state_chunk, sub_dt)
                    t_chunk += sub_accepted
                    accepted_dt += sub_accepted
                    total_retries += sub_retries
                    remaining -= sub_accepted
                state_new = state_chunk
            ...
            # Use the accepted-step transport basis from the adaptive step summary
            # for hydrocarbon/water accounting and accepted-step average rates.
            q_total = float(summary["well_rate_total"])
            gas_frac = float(summary["well_gas_fraction"])
            oil_frac = float(summary["well_oil_fraction"])
            water_frac = float(summary["well_water_fraction"])
...
def run_example(
    ...
    nx: int = 40,
    length_ft: float | None = None,  # if None, derived from radius_ft (1D grid = drainage radius)
    wells_config: List[dict] | None = None,  # list of per-well dicts; if None, single well from scalar params
    ...
    dt_days: float = 0.25,
    reporting_dt_days: float = 30.0,
    year_end_reporting: bool = True,
    ...
    refined_cells = max(2, min(refined_cells, nx - 1))
    # 1D grid length: radial depletion => L = drainage radius;
    # injection scenarios => L = injector-to-producer well spacing.
    if length_ft is None:
        if scenario_name == "natural_depletion":
            length_ft = float(radius_ft)
        else:
            length_ft = float(radius_ft)  # caller should pass explicit well_spacing via length_ft
    ...
    deviation_md_ft, deviation_tvd_ft = parse_deviation_survey_text(deviation_survey_text)

    def _build_well(cfg: dict, default_cell: int) -> Well:
        """Build a Well from a config dict, falling back to scalar params for missing keys."""
        pos = float(cfg.get("position_fraction", 1.0))
        pos = float(np.clip(pos, 0.0, 1.0))
        # Map position fraction to nearest cell index
        cell = int(round(pos * (nx - 1)))
        cell = max(0, min(cell, nx - 1))
        dev_text = cfg.get("deviation_survey_text", deviation_survey_text)
        dev_md, dev_tvd = parse_deviation_survey_text(dev_text)
        w = Well(
            cell_index=cell,
            control_mode=cfg.get("control_mode", well_control_mode),
            ...
            tubing_segments=int(cfg.get("tubing_segments", tubing_segments)),
            min_lift_thp_psia=float(cfg.get("min_lift_thp_psia", min_lift_thp_psia)),
            ...
            deviation_md_ft=dev_md,
            deviation_tvd_ft=dev_tvd,
        )
        # Attach name for reporting
        object.__setattr__(w, "name", str(cfg.get("name", f"Well {default_cell}")))
        return w

    if wells_config is not None and len(wells_config) > 0:
        wells_list = [_build_well(cfg, i + 1) for i, cfg in enumerate(wells_config)]
    else:
        # Single-well backward-compat path
        wells_list = [_build_well({"position_fraction": 1.0, "name": "Well 1"}, 1)]
    well = wells_list[0]  # primary well alias
    ...
    sim = CompositionalSimulator1D(
        ...
        separator=separator,
        reporting=reporting,
        detailed_mass_balance_diagnostics=False,
    )

    state0 = sim.initialize_state(p_init_psia=p_init_psia, z_init=z_init, sw_init=sw_init)
    final_state, history = sim.run(state0, t_end_days=t_end_days, dt_days=dt_days, reporting_dt_days=reporting_dt_days, start_date_text=start_date_text, year_end_reporting=year_end_reporting)
    final_diag = sim.spatial_diagnostics(final_state)

    summary_rows = generate_chapter4_summary(history)

    # -------------------------------------------------------------------------
    # Volumetrics block — prepended to the summary table
    # -------------------------------------------------------------------------
    FT3_PER_STB_VOL = 5.614583333333333

    # Volumetrics — consistent with industry formula:
    #   G = 43,560 * A(acres) * h(ft) * phi * (1-Swi) / Bgi
    # area_ft2 = pi*r^2 * h, so areal footprint = area_ft2 / h
    # area_ft2 = pi*r*h, length_ft = r => pi*r^2 = area_ft2 * length_ft / h
    drainage_area_ft2   = area_ft2 * length_ft / max(float(thickness_ft), 1e-6)
    drainage_area_acres = drainage_area_ft2 / 43560.0
    grv_ft3 = float(grid.length_ft * grid.area_ft2)  # = pi*r^2 * h * r  (L = radius)
    pv_ft3  = float(np.sum(sim.pv))                  # per-cell PV already in sim

    # Hydrocarbon and water pore volumes at initial conditions
    hcpv_ft3   = pv_ft3 * (1.0 - sw_init)
    wpv_ft3    = pv_ft3 * sw_init

    # Bg at initial pressure — prefer PVT table, fall back to EOS Z-factor
    if pvt_table is not None and pvt_table.has('gas_fvf_ft3_scf'):
        bg_init = pvt_table.interp(p_init_psia, 'gas_fvf_ft3_scf')
        bg_init = float(bg_init) if bg_init is not None else None
    else:
        bg_init = None

    if bg_init is None or bg_init <= 0.0:
        Z_init = sim.eos.z_factor(z_init, p_init_psia, temperature_R, phase="v")
        # Bg in ft3/scf: Bg = Z * T_sc/P_sc * P/T  (field units via R)
        bg_init = Z_init * R * temperature_R / (p_init_psia * SCF_PER_LBMOL / R / temperature_R * R * temperature_R)
        # Simpler: Bg = R*T*Z / (P * SCF_PER_LBMOL)  in ft3/lbmol / (scf/lbmol) = ft3/scf
        bg_init = Z_init * R * temperature_R / (p_init_psia * SCF_PER_LBMOL)

    # GIIP in scf then Bscf
    giip_scf  = hcpv_ft3 / max(bg_init, 1e-12)
    giip_bscf = giip_scf / 1.0e9

    # CIIP — use vaporised CGR at initial pressure from PVT table if available
    if pvt_table is not None and pvt_table.has('vaporized_cgr_stb_mmscf'):
        cgr_init = pvt_table.interp(p_init_psia, 'vaporized_cgr_stb_mmscf')
        if cgr_init is None:
            cgr_init = pvt_table.interp(p_init_psia, 'reservoir_cgr_stb_mmscf')
    elif pvt_table is not None and pvt_table.has('reservoir_cgr_stb_mmscf'):
        cgr_init = pvt_table.interp(p_init_psia, 'reservoir_cgr_stb_mmscf')
    else:
        cgr_init = None

    if cgr_init is not None and float(cgr_init) > 0.0:
        ciip_stb  = giip_scf / 1.0e6 * float(cgr_init)   # CGR in STB/MMscf
        ciip_mstb = ciip_stb / 1.0e3
    else:
        ciip_mstb = float("nan")

    # WIIP in STB and MSTB
    wiip_stb  = wpv_ft3 / FT3_PER_STB_VOL
    wiip_mstb = wiip_stb / 1.0e3

    vol_rows = [
        ["Drainage Area (acres)",       drainage_area_acres, drainage_area_acres, 0.0, drainage_area_acres],
        ["Net Pay Thickness (ft)",      float(thickness_ft), float(thickness_ft), 0.0, float(thickness_ft)],
        ["Gross Rock Volume (MMft3)",   grv_ft3 / 1.0e6,  grv_ft3 / 1.0e6,  0.0, grv_ft3 / 1.0e6],
        ["Pore Volume (MMft3)",         pv_ft3  / 1.0e6,  pv_ft3  / 1.0e6,  0.0, pv_ft3  / 1.0e6],
        ["GIIP (Bscf)",                 giip_bscf,        giip_bscf,         0.0, giip_bscf],
        ["CIIP (MSTB)",                 ciip_mstb,        ciip_mstb,         0.0, ciip_mstb],
        ["WIIP (MSTB)",                 wiip_mstb,        wiip_mstb,         0.0, wiip_mstb],
    ]
    summary_rows = vol_rows + summary_rows
    # -------------------------------------------------------------------------
    ...
    try:
        _, _, _agg_sc, _per_well = sim.well_sinks_all(final_state)
        per_well_rows = []
...
def display_streamlit_app() -> None:
    ...
    # Sidebar navigation — values persist because all keys are pre-loaded
    # from session_state (via the shadow-key mechanism above) before any
    # widget renders. Only the selected section's widgets are mounted,
    # but unrendered sections retain their values in session_state.
    #
    # Navigation uses a pending_section intermediary so the prev/next buttons
    # can request a section change without writing to the radio widget's own key
    # (Streamlit forbids that). The pending key is consumed here before the
    # radio renders, setting the correct initial index.
    st.sidebar.title("Input Sections")
    _pending = st.session_state.pop("pending_section", None)
    _default_idx = section_options.index(_pending) if _pending in section_options else 0
    ...
    # Dummy context manager so each "with tab_X:" block works unchanged.
    import contextlib
    @contextlib.contextmanager
    def _section(name):
        if _active_section == name:
            yield
        else:
            yield  # always yield — widgets inside guard themselves via the if-block below

    def _show(name): return _active_section == name

    # Thin wrappers that mimic tab objects but only render when active.
    class _Tab:
        def __init__(self, name): self._name = name
        def __enter__(self): return self
        def __exit__(self, *_): pass

    tab_case_setup            = _Tab("Case Setup")
    tab_reservoir_grid        = _Tab("Reservoir / Grid")
    tab_components_composition= _Tab("Components / Composition")
    tab_pvt_separator         = _Tab("PVT / Separator")
    tab_rock_rockfluid        = _Tab("Rock / Rock-Fluid")
    tab_initial_conditions    = _Tab("Initial Conditions")
    tab_producer_well         = _Tab("Producer Well")
    tab_injector_well         = _Tab("Injector Well")
    tab_aquifer_boundaries    = _Tab("Aquifer / Boundaries")
    tab_advanced_physics      = _Tab("Advanced Physics")
    tab_analysis_tools        = _Tab("Analysis Tools")
    tab_results               = _Tab("Results")

    # Show section heading and navigation arrows in main area
    _idx = section_options.index(_active_section)
    _prev = section_options[_idx - 1] if _idx > 0 else None
    _next = section_options[_idx + 1] if _idx < len(section_options) - 1 else None
    st.title(_active_section)
    _nav_cols = st.columns([1, 6, 1])
    if _prev:
        if _nav_cols[0].button(f"← {_prev}", key="nav_prev", use_container_width=True):
            st.session_state["pending_section"] = _prev
            st.rerun()
    if _next:
        if _nav_cols[2].button(f"{_next} →", key="nav_next", use_container_width=True):
            st.session_state["pending_section"] = _next
            st.rerun()
    st.markdown("---")
    ...
    persistent_defaults = {
        ...
        "dt_days": 0.05,
        "reporting_dt_days": 30.0,
        "nx": 40,
        "radius_ft": 100.0,
        "well_spacing_ft": 1000.0,
        "thickness_ft": 50.0,
        "refined_cells": 10,
        "min_dx_ft": 12.0,
        "growth": 1.45,
    ...
    reporting_dt_days = float(_ss("reporting_dt_days", 30.0))
    year_end_reporting = bool(_ss("year_end_reporting", True))

    nx = int(_ss("nx", 40))
    length_ft = float(_ss("length_ft", 4000.0))
    radius_ft = float(_ss("radius_ft", 100.0))
    thickness_ft = float(_ss("thickness_ft", 50.0))
    well_spacing_ft = float(_ss("well_spacing_ft", 1000.0))
    # For depletion: 1D grid = radial drainage, L = r.
    # For injection scenarios: L = injector-to-producer distance (well spacing).
    if scenario_name == "natural_depletion":
        length_ft = radius_ft
    else:
        length_ft = well_spacing_ft
    area_ft2 = math.pi * radius_ft * thickness_ft  # GRV = area*L = pi*r*h*r (depletion) or pi*r*h*L (injection)
    ...
    if _show("Case Setup"):
        ...
        dt_days = st.number_input("Internal time step (days)", min_value=0.005, max_value=365.0, value=0.05, step=0.01, key="dt_days",
            help="Trial step size for the adaptive solver. Reduced automatically on convergence failure.")
        reporting_dt_days = st.number_input("Reporting interval (days)", min_value=1.0, max_value=3650.0, value=30.0, step=1.0, key="reporting_dt_days",
            help="How often results are recorded. Each interval may contain multiple internal steps.")
        year_end_reporting = st.toggle("Snap output to year-ends (31 Dec)", value=True, key="year_end_reporting",
            help="Always write an output row on 31 December of each calendar year, regardless of the reporting interval.")
        if reporting_dt_days < dt_days:
            st.warning("Reporting interval is smaller than internal time step — it will be ignored and each internal step will produce one output point.")
        st.caption("Adaptive timestep retry is enabled. Output is recorded every reporting interval.")

    if _show("Reservoir / Grid"):
        st.subheader("Reservoir / Grid")
        nx = st.number_input("Number of grid cells", min_value=10, max_value=200, value=40, step=1, key="nx")
        radius_ft = st.number_input("Drainage radius (ft)", min_value=1.0, max_value=100000.0, value=100.0, step=10.0, key="radius_ft")
        thickness_ft = st.number_input("Net pay thickness (ft)", min_value=5.0, max_value=500.0, value=50.0, step=5.0, key="thickness_ft")
        is_injection = scenario_name in ("gas_cycling", "lean_gas_injection")
        if is_injection:
            well_spacing_ft = st.number_input("Injector-to-producer distance (ft)", min_value=100.0, max_value=50000.0, value=1000.0, step=100.0, key="well_spacing_ft",
                help="Flow path length between injector (cell 0) and producer (last cell). Determines breakthrough time and sweep efficiency.")
            length_ft = well_spacing_ft
        else:
            well_spacing_ft = float(_ss("well_spacing_ft", 1000.0))
            length_ft = radius_ft
        area_ft2 = math.pi * radius_ft * thickness_ft
        ...
        drainage_area_acres = math.pi * radius_ft ** 2 / 43560
        grv_mmft3 = area_ft2 * length_ft / 1e6
        if is_injection:
            st.caption(f"Drainage area = π × r² = {drainage_area_acres:,.2f} acres  |  Flow path = {length_ft:,.0f} ft  |  GRV = {grv_mmft3:,.2f} MMft³")
        else:
            st.caption(f"Drainage area = π × r² = {drainage_area_acres:,.2f} acres  |  GRV = π × r² × h = {grv_mmft3:,.2f} MMft³")
        st.info(
            "Grid: 1D nonuniform Cartesian with near-well refinement. "
            + ("Injector-to-producer distance sets the 1D flow path length. " if is_injection else "Grid length = drainage radius for radial depletion. ")
            + "Producer is in the last cell; injector, when active, is in the first cell."
        )

    if _show("Components / Composition"):
    ...
    if _show("PVT / Separator"):
        ...
        _api = float(_ss("condensate_api_gravity", 50.0))
        _rho_sc = 141.5 / (_api + 131.5) * 62.4
        st.caption(f"Stock-tank condensate density = {_rho_sc:.1f} lb/ft³. "
                   "Tubing calculations apply a pressure correction (~0.5%/1,000 psi) for in-situ density.")
    ...
    if _show("Producer Well"):
        st.subheader("Producer Wells")
        n_producers = int(st.number_input("Number of producer wells", min_value=1, max_value=5, value=int(_ss("n_producers", 1)), step=1, key="n_producers"))
        st.caption("Each well is placed at a fractional position along the 1D flow path (0 = injector end, 1 = outer boundary).")
        ...
        control_map = {
            ...
            "Fixed THP": "thp",
            "Top-down THP + Gas Rate": "top_down_vlp",
        }

        wells_config = []
        # Collect per-well config via expanders
        for _wi in range(n_producers):
            _wk = f"w{_wi}"  # key prefix
            _default_pos = round((_wi + 1) / n_producers, 2)
            with st.expander(f"Well {_wi + 1}", expanded=(_wi == 0)):
                _name = st.text_input("Well name", value=_ss(f"{_wk}_name", f"Well {_wi + 1}"), key=f"{_wk}_name")
                _pos = float(st.slider("Position along flow path (0=injector end, 1=outer boundary)", 0.0, 1.0, float(_ss(f"{_wk}_pos", _default_pos)), step=0.05, key=f"{_wk}_pos"))
                _ctrl_label = st.selectbox("Control mode", list(control_map.keys()), index=0, key=f"{_wk}_ctrl")
                _ctrl = control_map[_ctrl_label]
                _bhp = float(st.number_input("BHP target (psia)", min_value=100.0, max_value=20000.0, value=float(_ss(f"{_wk}_bhp", 3000.0)), step=100.0, disabled=(_ctrl != "bhp"), key=f"{_wk}_bhp"))
                _dd = float(st.number_input("Drawdown target (psi)", min_value=1.0, max_value=10000.0, value=float(_ss(f"{_wk}_dd", 200.0)), step=10.0, disabled=(_ctrl != "drawdown"), key=f"{_wk}_dd"))
                _qg = float(st.number_input("Target gas rate (MMscf/day)", min_value=0.0, max_value=5000.0, value=float(_ss(f"{_wk}_qg", 5.0)), step=0.5, disabled=(_ctrl not in {"gas_rate", "top_down_vlp"}), key=f"{_wk}_qg"))
                _thp = float(st.number_input("THP target (psia)", min_value=14.7, max_value=10000.0, value=float(_ss(f"{_wk}_thp", 500.0)), step=25.0, disabled=(_ctrl not in {"thp", "top_down_vlp"}), key=f"{_wk}_thp"))
                _tvd = float(st.number_input("TVD (ft)", min_value=100.0, max_value=30000.0, value=float(_ss(f"{_wk}_tvd", 8000.0)), step=100.0, key=f"{_wk}_tvd"))
                _tid = float(st.number_input("Tubing ID (in)", min_value=1.0, max_value=6.0, value=float(_ss(f"{_wk}_tid", 2.441)), step=0.1, key=f"{_wk}_tid"))
                _skin = float(st.number_input("Skin factor", min_value=-10.0, max_value=50.0, value=float(_ss(f"{_wk}_skin", 0.0)), step=0.5, key=f"{_wk}_skin"))
                _rw = float(st.number_input("Wellbore radius (ft)", min_value=0.05, max_value=5.0, value=float(_ss(f"{_wk}_rw", 0.35)), step=0.05, key=f"{_wk}_rw"))
                _pi = float(st.number_input("PI multiplier", min_value=0.01, max_value=100.0, value=float(_ss(f"{_wk}_pi", 1.0)), step=0.1, key=f"{_wk}_pi"))
                wells_config.append({
                    "name": _name,
        ...
        # Scalar well params — used as defaults for single-well backward compat
        ...
        tubing_segments = wells_config[0]["tubing_segments"]
    ...
    if _show("Advanced Physics"):
        ...
        hysteresis_gas_trapping_strength = st.number_input(
            "Gas trapping strength  [Land: higher C = less trapping]",
    ...
    run_kwargs = dict(
        ...
        reporting_dt_days=float(reporting_dt_days),
        year_end_reporting=bool(year_end_reporting),
        z_init=z_init,
        Sgc=float(Sgc),
        Swc=float(Swc),
        Sorw=float(Sorw),
        ...
        pvt_table=pvt_table,
        deviation_survey_text=str(deviation_survey_text),
    ...
    with st.spinner("Running compositional simulation..."):
        ...
        try:
            ...
            return
        simulation_runtime_seconds = time.perf_counter() - run_start_time
    ...
    def _cached_run(kwargs_frozen: tuple) -> dict:
        """Thin cache wrapper — converts frozen tuple of (key,val) pairs back to kwargs."""
        kwargs = {}
        for k, v in kwargs_frozen:
            if isinstance(v, list):
                kwargs[k] = np.array(v, dtype=float)
            else:
                kwargs[k] = v
        r = run_example(**kwargs)
        return {"history": r["history"], "summary_df": r["summary_df"]}

    def _freeze_kwargs(kw: dict) -> tuple:
        """Convert run_kwargs to a hashable tuple so st.cache_data can key on it."""
        out = []
        for k, v in sorted(kw.items()):
            if isinstance(v, np.ndarray):
                out.append((k, tuple(v.tolist())))
    ...
    if enable_sensitivity:
        ...
        for case_label, pct_change in pct_cases:
            ...
            try:
                sens_result = _cached_run(_freeze_kwargs(case_kwargs))
    ...
    if enable_monte_carlo:
        ...
        if selected_mc:
            ...
            for run_idx in range(int(monte_carlo_runs)):
                ...
                try:
                    mc_result = _cached_run(_freeze_kwargs(case_kwargs))
    ...
    if _show("Results"):
        ...
        with result_tabs[0]:
            ...
            d2.write(f"Drainage area πr² (acres): {math.pi * radius_ft**2 / 43560:,.3f}")
            d3.write(f"Net pay thickness (ft): {thickness_ft:,.1f}")
