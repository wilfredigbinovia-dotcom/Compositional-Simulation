...
class CompositionalSimulator1D:
    ...
    def well_response_at_pwf(self, state: SimulationState, pwf_psia: float) -> Dict[str, float | np.ndarray]:
        ...
        if lam_hc > 1e-12:
            ...
            fg_hc_flash = beta
            fg_hc = 0.75 * fg_hc_mob + 0.25 * fg_hc_flash
            min_fg_hc = 0.01 + 0.04 * beta if beta > 1e-6 else 0.0
            fg_hc = float(np.clip(max(fg_hc, min_fg_hc), 0.0, 1.0))
            fo_hc = 1.0 - fg_hc
            hc_frac = 1.0 - fw
            fg = hc_frac * fg_hc
            fo = hc_frac * fo_hc
    ...
    def separator_flash(self, response: Dict[str, float | np.ndarray]) -> Dict[str, float | np.ndarray]:
        ...
        inlet_gas_frac = float(np.clip(response.get("gas_frac", beta_sep_flash), 0.0, 1.0))
        beta_sep = float(np.clip(0.85 * beta_sep_flash + 0.15 * inlet_gas_frac, 0.0, 1.0))
