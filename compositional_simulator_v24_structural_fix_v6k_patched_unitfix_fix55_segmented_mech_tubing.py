...
def drift_flux_liquid_holdup(vsg_ft_s: float, vsl_ft_s: float, rho_l_lbft3: float, rho_g_lbft3: float) -> float:
    vm = max(vsg_ft_s + vsl_ft_s, 1e-9)
    lambda_l = np.clip(vsl_ft_s / vm, 0.0, 1.0)
    c0 = 1.20 if lambda_l < 0.30 else (1.10 if lambda_l < 0.70 else 1.02)
    density_ratio = max((rho_l_lbft3 - rho_g_lbft3) / max(rho_l_lbft3, 1e-12), 0.0)
    vd = float(np.clip(1.53 * density_ratio ** 0.25, 0.05, 3.0))
    hg = np.clip(vsg_ft_s / max(c0 * vm + vd, 1e-9), 0.0, 1.0)
    hl = 1.0 - hg
    return float(np.clip(max(hl, lambda_l), 0.0, 0.98))
...
class CompositionalSimulator1D:
    ...
    def tubing_pressure_profile_from_response(self, state: SimulationState, response: Dict[str, float | np.ndarray]) -> Dict[str, float]:
        ...
        for seg in range(nseg):
            ...
            try:
                fl_seg = self.flash.flash(z_hc, p_local, t_local)
            ...
            rho_g = gas_density_lbft3_from_eos(self.eos, y_seg, p_local, t_local) if qg_seg_lbmol_day > 1e-12 else 0.05
            rho_o = self.reporting.condensate_density_lbft3
            rho_w = WATER_DENSITY_LBFT3
            rho_l = (qo_seg_lbmol_day * rho_o + qw_lbmol_day * rho_w) / max(qo_seg_lbmol_day + qw_lbmol_day, 1e-12) if (qo_seg_lbmol_day + qw_lbmol_day) > 1e-12 else rho_o

            gas_vol_ft3_day = hydrocarbon_gas_rate_ft3_day(qg_seg_lbmol_day, y_seg, p_local, t_local, self.eos)
            ...
            vm = vsg + vsl
            hl = drift_flux_liquid_holdup(vsg, vsl, rho_l, rho_g)
            rho_mix = hl * rho_l + (1.0 - hl) * rho_g

            mu_g_cp = phase_viscosity_cp("v", y_seg, p_local, t_local) if qg_seg_lbmol_day > 1e-12 else 0.01
            mu_o_cp = phase_viscosity_cp("l", x_seg, p_local, t_local) if qo_seg_lbmol_day > 1e-12 else 0.10
            mu_l_cp = (qo_seg_lbmol_day * mu_o_cp + qw_lbmol_day * WATER_VISCOSITY_CP) / max(qo_seg_lbmol_day + qw_lbmol_day, 1e-12) if (qo_seg_lbmol_day + qw_lbmol_day) > 1e-12 else mu_o_cp
            ...
            friction_grad_psi_ft = self.well.tubing_calibration_factor * friction_factor * rho_mix * max(vm, 0.0) ** 2 / max(2.0 * 32.174 * diameter_ft * 144.0, 1e-12)
            segment_hydro_psi = hydro_grad_psi_ft * seg_len_ft
            segment_friction_psi = friction_grad_psi_ft * seg_len_ft

            momentum_psi = rho_mix * max(vm, 0.0) ** 2 / max(2.0 * 32.174 * 144.0, 1e-12)
            if prev_momentum_psi is None:
                segment_accel_psi = 0.0
            else:
                segment_accel_psi = max(momentum_psi - prev_momentum_psi, 0.0)
            prev_momentum_psi = momentum_psi
