...
class CompositionalSimulator1D:
    ...
    def record_mass_balance(
        ...
        HC production is derived from the EXACT change in reservoir moles
        (state_before.nt - state_after.nt) rather than rate × dt, which is
        only an approximation when transport is sub-cycled. This eliminates
        the systematic negative drift previously caused by the mismatch between
        the outer-boundary trapezoid rate average and the inner subcycle removals.

        For the final zero-dt record, state_before == state_after so
        hc_produced_this_step = 0, which is correct.
        """
        hc_before = float(np.sum(state_before.nt))
        hc_after  = float(np.sum(state_after.nt))
        hc_produced_this_step = max(hc_before - hc_after, 0.0)  # exact removal
        hc_inj_this_step = injector_rate_lbmol_day * dt_days

        self.cum_hc_produced_lbmol   += hc_produced_this_step
        self.cum_water_produced_lbmol += qw_produced_lbmol * dt_days
        self.cum_hc_injected_lbmol    += hc_inj_this_step
    ...
    def _record_history_step(
        ...
        self.record_mass_balance(
            history, state_before, state, qw_lbmol_day, injector_rate_lbmol_day, dt_days
        )
        ...
        while t <= t_end_days + 1e-12:
            ...
            if next_dt <= 1e-12:
                ...
                self._record_history_step(
                    ...
                    state_before=state, dt_days=0.0,  # final record: no change
                )
                break
