...
class CompositionalSimulator1D:
    ...
    def _total_compressibility_psi(self, p_psia: float, sw: float) -> float:
        """
        Estimate total compressibility at pressure p_psia using PVT Bg if available,
        otherwise fall back to ideal-gas approximation.  Includes rock and water terms.
        """
        cf = 4.0e-6   # rock compressibility, psi-1 (typical sandstone)
        cw = 3.0e-6   # water compressibility, psi-1

        if self.pvt_table is not None and self.pvt_table.has('gas_fvf_ft3_scf'):
            dp = max(p_psia * 0.01, 10.0)
            bg_hi = self.pvt_table.interp(p_psia + dp, 'gas_fvf_ft3_scf')
            bg_lo = self.pvt_table.interp(p_psia - dp, 'gas_fvf_ft3_scf')
            if bg_hi is not None and bg_lo is not None and bg_lo > 1e-12:
                cg = (bg_hi - bg_lo) / (bg_lo * 2.0 * dp)
            else:
                cg = 1.0 / max(p_psia, 1.0)
        else:
            cg = 1.0 / max(p_psia, 1.0)

        sg = max(1.0 - float(sw), 0.0)
        ct = sg * abs(cg) + float(sw) * cw + cf
        return float(np.clip(ct, 1.0e-6, 5.0e-3))
    ...
    def pressure_update(self, state: SimulationState, dt_days: float) -> np.ndarray:
        ...
        ct_arr = np.array([
            self._total_compressibility_psi(float(p_old[i]), float(state.sw[i]))
            for i in range(n)
        ], dtype=float)
    ...
    def transport_update(self, state: SimulationState, dt_days: float) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float]:
        ...
        for _ in range(n_sub):
            ...
            # Integrate the actual accepted sink/source basis over the subcycles
            # only when detailed mass-balance diagnostics are enabled.
            hc_rate_lbmol_day = float(np.sum(np.asarray(hc_sink, dtype=float)))
...
def run_example(
    ...
    well = Well(
        ...
        rw_ft=rw_ft,
        skin=skin,
        deviation_md_ft=deviation_md_ft,
        deviation_tvd_ft=deviation_tvd_ft,
    )
...
def display_streamlit_app() -> None:
    ...
    with tab_producer_well:
        ...
        well_control_label = st.selectbox("Producer control mode", ["Fixed BHP", "Fixed Drawdown", "Fixed Gas Rate", "Fixed THP", "Top-down THP + Gas Rate"], index=0, key="well_control_label")
        control_map = {
            "Fixed BHP": "bhp",
        ...
        target_gas_rate_mmscf_day = st.number_input("Target gas rate (MMscf/day)", min_value=0.0, max_value=5000.0, value=5.0, step=0.5, disabled=(well_control_mode not in {"gas_rate", "top_down_vlp"}), key="target_gas_rate_mmscf_day")
        thp_psia = st.number_input("Tubing head pressure target (psia)", min_value=14.7, max_value=10000.0, value=500.0, step=25.0, disabled=(well_control_mode not in {"thp", "top_down_vlp"}), key="thp_psia")
        ...
        deviation_survey_text = st.text_area(
            "Deviation survey (MD ft, TVD ft; optional)",
            value=_ss("deviation_survey_text", ""),
            height=100,
            help="Enter one MD,TVD pair per line. Leave blank to assume a vertical well where MD equals TVD.",
            disabled=(well_control_mode != "top_down_vlp"),
            key="deviation_survey_text",
        )
