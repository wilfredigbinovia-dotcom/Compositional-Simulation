...
class CompositionalSimulator1D:
    ...
    def transport_update(self, state: SimulationState, dt_days: float) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float]:
        ...
        # Step-integrated production accounting. Use these to return accepted-step
        # average rates that are exactly consistent with what was removed from the
        # reservoir state across subcycles.
        ...
        # Convert integrated removal/source totals back to accepted-step average rates.
