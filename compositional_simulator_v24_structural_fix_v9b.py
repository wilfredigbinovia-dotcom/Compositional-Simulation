...
class CompositionalSimulator1D:
    ...
    def transport_update(self, state: SimulationState, dt_days: float) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, float, float, float, float, float, float, float, float, float, float, float, float]:
        """
        Transport is lightly sub-cycled so the producer-cell composition and phase
        state are refreshed within a reporting timestep without creating runaway
        local substepping. The producer-cell CFL estimate only nudges the number
        of explicit substeps upward when the sink is unusually aggressive.
        """
        max_substep_days = 15.0
        base_n_sub = max(1, min(3, int(math.ceil(dt_days / max_substep_days))))

        wi = self.well.cell_index
        self._set_cache_state(state)
        hc_sink_est, qw_est, _, _, _, _, _, _, _, _, _, _, _, _ = self.well_sink(state)
        ncomp_est = state.nt[:, None] * state.z
        hc_avail = np.maximum(ncomp_est[wi], 1e-12)
        w_avail = max(float(state.nw[wi]), 1e-12)

        hc_req_total = float(np.sum(np.maximum(hc_sink_est, 0.0))) * dt_days
        hc_avail_total = float(np.sum(hc_avail))
        total_frac = hc_req_total / max(hc_avail_total, 1e-12)

        comp_frac = 0.0
        positive_sink = hc_sink_est > 1e-18
        if np.any(positive_sink):
            comp_frac = float(np.max((dt_days * hc_sink_est[positive_sink]) / hc_avail[positive_sink]))

        water_frac = float((dt_days * max(qw_est, 0.0)) / w_avail)

        target_total_frac = 0.25
        target_comp_frac = 0.30
        target_water_frac = 0.25

        sink_cfl = max(
            total_frac / target_total_frac,
            comp_frac / target_comp_frac,
            water_frac / target_water_frac,
            1.0,
        )

        n_sub = max(base_n_sub, int(math.ceil(sink_cfl)))
        n_sub = min(n_sub, 4)
        ...
        for _ in range(n_sub):
            ...
            hc_remove = dt_sub * np.maximum(hc_sink, 0.0)
            w_remove = dt_sub * max(qw, 0.0)

            # Physical safeguard only: avoid tiny negative inventories from roundoff.
            hc_available = np.maximum(ncomp_new[wi] - 1e-12, 0.0)
            if float(np.sum(hc_remove)) > float(np.sum(hc_available)) + 1e-12:
                comp_scales = np.where(hc_remove > 1e-18, hc_available / np.maximum(hc_remove, 1e-18), 1.0)
                hc_scale = float(np.clip(np.min(comp_scales), 0.0, 1.0))
                hc_remove *= hc_scale
                q_total *= hc_scale
                q_undamaged *= hc_scale

            w_available = max(float(nw_new[wi] - 1e-12), 0.0)
            if w_remove > w_available + 1e-12:
                w_scale = w_available / max(w_remove, 1e-18)
                w_remove *= w_scale
                q_total *= w_scale
                q_undamaged *= w_scale
