...
class CompositionalSimulator1D:
    ...
    def separator_rates(self, response: Dict[str, float | np.ndarray]) -> Dict[str, float]:
        ...
        q_o_res_lbmol_day = float(response.get("qo_lbmol_day", 0.0))
        x_sep = np.asarray(sep["x_sep"], dtype=float)
        x_res = np.asarray(response.get("x", x_sep), dtype=float)

        gas_rate_mmscf_day = gas_lbmol_day_to_mmscf_day(q_sep_gas_lbmol_day)

        condensate_rate_sep_stb_day = liquid_lbmol_day_to_stb_day(
            q_sep_liq_lbmol_day,
            x_sep,
            self.mw_components,
            self.reporting.condensate_density_lbft3,
        )
        condensate_rate_res_stb_day = liquid_lbmol_day_to_stb_day(
            q_o_res_lbmol_day,
            x_res,
            self.mw_components,
            self.reporting.condensate_density_lbft3,
        ...
        # Stabilize condensate reporting so isolated separator-flash state switches
        # do not create non-physical spikes in natural-depletion cases.
        condensate_rate_stb_day = condensate_rate_sep_stb_day

        pvt_props = self.pvt_lookup(float(response.get("pwf_psia", response.get("well_cell_pressure_psia", self.pvt.dew_point_psia))))
        cgr_limit = pvt_props.get("reservoir_cgr_stb_mmscf")
        upper_bounds: List[float] = []

        if condensate_rate_res_stb_day > 0.0:
            upper_bounds.append(max(1.25 * condensate_rate_res_stb_day, condensate_rate_res_stb_day + 25.0))

        if cgr_limit is not None and gas_rate_mmscf_day > 0.0:
            cgr_cap = gas_rate_mmscf_day * float(cgr_limit)
            upper_bounds.append(max(1.10 * cgr_cap, cgr_cap + 25.0))

        if upper_bounds:
            condensate_rate_stb_day = min(condensate_rate_stb_day, min(upper_bounds))

        condensate_rate_stb_day = max(condensate_rate_stb_day, 0.0)
