...
class CompositionalSimulator1D:
    ...
    def separator_rates(self, response: Dict[str, float | np.ndarray]) -> Dict[str, float]:
        ...
        # Main production reporting should follow the producer wellstream and
        # use the user PVT CGR table only as a pressure-dependent guide. A raw
        # one-stage separator split can switch abruptly, while using Pwf for CGR
        # lookups can drive condensate to zero too early. Use reservoir/well-cell
        # pressure first, then blend toward the user CGR if it is available.
        ...
        if gas_rate_mmscf_day <= 1e-12:
            ...
            # Blend the user table with the instantaneous wellstream response.
            # This preserves the pressure trend from the PVT table while avoiding
            # zero-or-spike behavior when either source becomes locally unstable.
            effective_cgr_stb_mmscf = 0.70 * cgr_user + 0.30 * derived_cgr_stb_mmscf
            condensate_rate_stb_day = effective_cgr_stb_mmscf * gas_rate_mmscf_day
            condensate_reporting_basis = "blended_pvt_cgr"
        elif user_cgr_valid:
            effective_cgr_stb_mmscf = cgr_user
            condensate_rate_stb_day = effective_cgr_stb_mmscf * gas_rate_mmscf_day
            condensate_reporting_basis = "pvt_cgr"
        else:
            effective_cgr_stb_mmscf = max(derived_cgr_stb_mmscf, 0.0)
            condensate_rate_stb_day = condensate_from_wellstream_stb_day
            condensate_reporting_basis = "wellstream_liquid"
        ...
        return {
            ...
            "raw_separator_condensate_rate_stb_day": float(liquid_lbmol_day_to_stb_day(q_sep_liq_lbmol_day, x_sep, self.mw_components, self.reporting.condensate_density_lbft3)),
    ...
    def step_once(self, state: SimulationState, dt_days: float) -> Tuple[SimulationState, Dict[str, float]]:
        ...
        nt_new, z_new, nw_new, sw_new, q_total, gas_frac, oil_frac, water_frac, dropout, krg, kro, krw, damage_factor, wi_eff, q_undamaged, ploss, injector_actual_rate_lbmol_day = self.transport_update(state_mid, dt_days)
...
def run_example(
    ...
    if scenario_name in ("gas_cycling", "lean_gas_injection"):
        ...
        inj_start_day = days_between_dates(start_date_text, injection_start_date_text)
        inj_end_day = days_between_dates(start_date_text, injection_end_date_text)
...
def composition_editor(label: str, component_names: List[str], default_values: np.ndarray, key_prefix: str) -> tuple[np.ndarray, float, str]:
    ...
    seed_key = f"{key_prefix}_seed_df"
    unit_mode = st.radio(
        f"{label} input mode",
        ["Fraction", "Percentage"],
        horizontal=True,
    ...
    if seed_key not in st.session_state:
        st.session_state[seed_key] = pd.DataFrame({
            ...
            "Value": np.round(default_values * display_scale, 6),
        })
    seed_df = st.session_state[seed_key].copy()
    if list(seed_df.get("Component", [])) != list(component_names):
        seed_df = pd.DataFrame({"Component": component_names, "Value": np.round(default_values * display_scale, 6)})
    ...
    st.session_state[seed_key] = editor_df
    values = pd.to_numeric(editor_df["Value"], errors="coerce").fillna(0.0).to_numpy(dtype=float)
    raw_sum = float(np.sum(values))
    comp = values / 100.0 if unit_mode == "Percentage" else values.copy()
...
def display_streamlit_app() -> None:
            ...
            if scenario_name in ("gas_cycling", "lean_gas_injection"):
                ...
                try:
                    ...
                    if inj_end_day < inj_start_day:
                        st.error("Injection end date must be on or after the injection start date.")
                        return
    ...
    # Keep plots on the full simulation history, even when the history table
    # is condensed to one row per year. Otherwise x (yearly snapshots) and y
    # (full timestep histories) will have different lengths and plotting fails.
    if start_date_text:
        start_dt = parse_ddmmyyyy(start_date_text)
        time_axis = pd.to_datetime(start_dt) + pd.to_timedelta(np.asarray(history.time_days, dtype=float), unit="D")
    ...
    with result_tabs[1]:
        figs = [
            make_line_figure(time_axis, [(np.asarray(history.avg_pressure_psia), "Average Pressure"), (np.asarray(history.well_pressure_psia), "Well-Cell Pressure"), (np.asarray(history.dew_point_psia), "Dew Point")], time_axis_label, "Pressure (psia)", "Pressure and Dew-Point History"),
            make_line_figure(time_axis, [(np.asarray(history.well_pressure_minus_dewpoint_psia), "Well Pressure - Dew Point"), (np.asarray(history.avg_pressure_minus_dewpoint_psia), "Average Pressure - Dew Point")], time_axis_label, "Pressure Margin (psia)", "Pressure Margin to Dew Point"),
            make_line_figure(
                time_axis,
            ...
            make_line_figure(time_axis, [(np.asarray(history.condensate_rate_stb_day), "Condensate Rate"), (np.asarray(history.water_rate_stb_day), "Water Rate")], time_axis_label, "Liquid Rate (STB/day)", "Condensate and Water Production Rate History"),
            make_line_figure(time_axis, [(np.asarray(history.cum_gas_bscf), "Cumulative Gas")], time_axis_label, "Cumulative Gas (Bscf)", "Cumulative Gas Production"),
            make_line_figure(time_axis, [(np.asarray(history.cum_condensate_mstb), "Cumulative Condensate"), (np.asarray(history.cum_water_mstb), "Cumulative Water")], time_axis_label, "Cumulative Liquid (MSTB)", "Cumulative Condensate and Water Production"),
            make_line_figure(time_axis, [(np.asarray(history.well_damage_factor), "Damage Factor"), (np.asarray(history.productivity_loss_fraction), "Productivity Loss Fraction")], time_axis_label, "Dimensionless", "Condensate-Bank Impairment"),
            make_line_figure(time_axis, [(np.asarray(history.avg_sw), "Average Sw")], time_axis_label, "Average Sw", "Average Water Saturation History"),
            make_line_figure(time_axis, [(np.abs(np.asarray(history.hc_mass_balance_error_lbmol)), "|HC Mass Balance Error|"), (np.abs(np.asarray(history.water_mass_balance_error_lbmol)), "|Water Mass Balance Error|")], time_axis_label, "Absolute Error (lbmol)", "Mass Balance Error History"),
            make_line_figure(time_axis, [(np.asarray(history.well_flowing_pwf_psia), "Flowing Pwf")], time_axis_label, "Pwf (psia)", "Flowing Bottom-Hole Pressure History"),
            make_line_figure(time_axis, [(np.asarray(history.well_estimated_thp_psia), "Estimated THP")], time_axis_label, "THP (psia)", "Tubing Head Pressure History"),
            make_line_figure(time_axis, [(np.asarray(history.separator_vapor_fraction), "Separator Vapor Fraction")], time_axis_label, "Vapor Fraction", "Surface Separator Flash Vapor Fraction History"),
        ]
        for fig in figs:
            st.pyplot(fig)
            plt.close(fig)
    ...
    with result_tabs[4]:
        ...
        st.caption(
            "Gas rates are reported in MMscf/day using standard conditions of 14.7 psia and 60°F. Condensate rates are reported from stabilized wellstream liquid production, while separator vapor fraction is retained as a diagnostic from the one-stage surface separator flash. "
            "Producer control may be fixed BHP, fixed gas rate, or simplified fixed THP. Condensate reporting uses the producer wellstream and blends with the user PVT CGR table when available. The THP mode uses an approximate tubing-loss relation and should be treated as a prototype rather than a full VLP implementation."
        )
