...
def to_csv_bytes_from_dataframe(df: pd.DataFrame) -> bytes:
...
class CompositionalSimulator1D:
    ...
    def run(self, state0: SimulationState, t_end_days: float, dt_days: float) -> Tuple[SimulationState, SimulationHistory]:
        ...
        while t <= t_end_days + 1e-12:
            ...
            (
                _hc_sink0, _qw0, q_total_0, gas_frac_0, oil_frac_0, water_frac_0,
                dropout_0, krg_0, kro_0, krw_0, damage_factor_0, wi_eff_0,
                q_undamaged_0, ploss_0
            ) = self.well_sink(state)
            ...
            if injector is None or self.scenario.name == "natural_depletion":
                ...
                injector_rate_lbmol_day = float(injector.rate_lbmol_day)
            ...
            if next_dt <= 1e-12:
                ...
                fl = self.cell_flash_cached(state, self.well.cell_index)
                x = np.asarray(fl["x"], dtype=float)

                gas_rate_mmscf_day = gas_lbmol_day_to_mmscf_day(qg_lbmol_day)
                condensate_rate_stb_day = liquid_lbmol_day_to_stb_day(
                    qo_lbmol_day, x, self.mw_components, CONDENSATE_DENSITY_LBFT3
                )
                ...
                self.record_mass_balance(
                    history, state, 0.0, q_total, gas_frac, oil_frac, water_frac, injector_rate_lbmol_day
                )
            ...
            (
                _hc_sink1, _qw1, q_total_1, gas_frac_1, oil_frac_1, water_frac_1,
                dropout_1, krg_1, kro_1, krw_1, damage_factor_1, wi_eff_1,
                q_undamaged_1, ploss_1
            ) = self.well_sink(state_new)
            ...
            fl0 = self.cell_flash_cached(state, self.well.cell_index)
            fl1 = self.cell_flash_cached(state_new, self.well.cell_index)
            x_avg = 0.5 * (np.asarray(fl0["x"], dtype=float) + np.asarray(fl1["x"], dtype=float))
            ...
            gas_rate_mmscf_day = gas_lbmol_day_to_mmscf_day(qg_lbmol_day)
            condensate_rate_stb_day = liquid_lbmol_day_to_stb_day(
                qo_lbmol_day, x_avg, self.mw_components, CONDENSATE_DENSITY_LBFT3
            )
            water_rate_stb_day = water_lbmol_day_to_stb_day(qw_lbmol_day)
            ...
            self.record_mass_balance(
                history, state_new, accepted_dt, q_total, gas_frac, oil_frac, water_frac, injector_rate_lbmol_day
            )
...
def history_dataframe(history: SimulationHistory) -> pd.DataFrame:
    return pd.DataFrame({
...
# -----------------------------------------------------------------------------
# Streamlit UI with tabs instead of sidebar
# -----------------------------------------------------------------------------
...
def display_streamlit_app() -> None:
    ...
    st.write(
        ...
        "semi-implicit pressure update, adaptive timestepping, Peaceman well model, "
        "local-grid refinement, three-phase relative permeability, dew-point diagnostics, "
    ...
    with input_tabs[0]:
        ...
        with col1:
            ...
            injection_rate_value = st.number_input("Gas injection rate", min_value=0.0, max_value=1.0e6, value=5.0, step=0.5)
        ...
        with col2:
            ...
            area_ft2 = st.number_input("Cross-sectional area (ft²)", min_value=100.0, max_value=50000.0, value=2500.0, step=100.0)
        ...
        st.info("Grid: 1D nonuniform Cartesian with near-well refinement. Producer is in the last cell; injector, when active, is in the first cell.")
    ...
    with input_tabs[1]:
        ...
        with c2:
            bhp_psia = st.number_input("Bottom-hole pressure (psia)", min_value=100.0, max_value=20000.0, value=3000.0, step=100.0)
        ...
        with c6:
            st.write("**RelPerm notes**")
            st.write("Corey-style three-phase proxy curves are used in this prototype.")
            st.write("Condensate-bank impairment is tracked through a separate damage-factor indicator.")

        if Sgc + Swc >= 1.0:
            st.error("Invalid inputs: Sgc + Swc must be less than 1.0.")
            return
        if Swc + Sorw >= 1.0:
            st.error("Invalid inputs: Swc + Sorw must be less than 1.0.")
            return
        if Swc + Sorg >= 1.0:
            st.error("Invalid inputs: Swc + Sorg must be less than 1.0.")
            return
    ...
    with input_tabs[2]:
        ...
        with c2:
            ...
            try:
                ...
                st.error(str(e))
                return
            dt_days = st.number_input("Requested time step (days)", min_value=0.01, max_value=1000.0, value=0.25, step=0.05)
            if dt_days > t_end_days:
                st.warning("Requested time step exceeds total simulation length.")

        with c3:
            st.write("**Schedule notes**")
            st.write("The simulator now applies adaptive timestep retry logic if the requested step causes instability.")
            st.write("Dew point remains user-specified and is used for diagnostics rather than solved from EOS in this UI.")
    ...
    with input_tabs[3]:
        ...
        with c3:
            ...
            z_c5 = st.number_input("C5 mole fraction", min_value=0.0, max_value=1.0, value=0.020, step=0.001, format="%.4f")
        with c4:
        ...
        if scenario_name == "lean_gas_injection":
            ...
            with lc3:
                ...
                lg_c5 = st.number_input("Lean gas C5", min_value=0.0, max_value=1.0, value=0.0, step=0.001, format="%.4f")
            with lc4:
        ...
        if np.sum(lean_gas_composition) <= 0.0:
            st.error("Lean gas composition must have a positive total.")
            return
        lean_gas_composition = lean_gas_composition / np.sum(lean_gas_composition)
        st.caption(f"Normalized lean gas composition sum: {np.sum(lean_gas_composition):.4f}")

    with input_tabs[4]:
        st.write("**Model overview**")
        st.write("Fluid: 11-component hydrocarbon compositional system with explicit immiscible water phase.")
        st.write("Pressure: semi-implicit prototype pressure equation with injector/well source terms.")
        st.write("Transport: explicit component and water update with adaptive timestep retry.")
        st.write("Diagnostics: condensate-dropout indicator, damage factor, dew-point margins, and mass-balance error tracking.")
    ...
    with result_tabs[0]:
        ...
        c16.metric("Last Timestep Retries", f"{history.timestep_retries[-1]}")
        c17.metric("Near-well minimum dx (ft)", f"{grid.cell_width(well.cell_index):.3f}")
        c18.metric("Peaceman WI geom", f"{sim.peaceman_well_index(well.cell_index):.5f}")
    ...
    with result_tabs[1]:
        fig1 = make_line_figure(
            time_axis,
            [
                (np.asarray(history.avg_pressure_psia), "Average Pressure"),
                (np.asarray(history.well_pressure_psia), "Well-Cell Pressure"),
                (np.asarray(history.dew_point_psia), "Dew Point"),
            ],
            "Time (days)",
            "Pressure (psia)",
            "Pressure and Dew-Point History",
        )
        st.pyplot(fig1)
        plt.close(fig1)

        fig2 = make_line_figure(
            time_axis,
            [
                (np.asarray(history.well_pressure_minus_dewpoint_psia), "Well Pressure - Dew Point"),
                (np.asarray(history.avg_pressure_minus_dewpoint_psia), "Average Pressure - Dew Point"),
            ],
            "Time (days)",
            "Pressure Margin (psia)",
            "Pressure Margin to Dew Point",
        )
        st.pyplot(fig2)
        plt.close(fig2)

        gas_rate_plot = pd.Series(history.gas_rate_mmscf_day).rolling(window=3, center=True, min_periods=1).mean().to_numpy()

        fig3 = make_line_figure(
            time_axis,
            [
                (np.asarray(history.gas_rate_mmscf_day), "Raw Gas Rate"),
                (gas_rate_plot, "Smoothed Gas Rate"),
            ],
            "Time (days)",
            "Gas Rate (MMscf/day)",
            "Gas Production Rate History",
        )
        st.pyplot(fig3)
        plt.close(fig3)

        fig4 = make_line_figure(
            time_axis,
            [
                (np.asarray(history.condensate_rate_stb_day), "Condensate Rate"),
                (np.asarray(history.water_rate_stb_day), "Water Rate"),
            ],
            "Time (days)",
            "Liquid Rate (STB/day)",
            "Condensate and Water Production Rate History",
        )
        st.pyplot(fig4)
        plt.close(fig4)

        fig5 = make_line_figure(
            time_axis,
            [(np.asarray(history.cum_gas_bscf), "Cumulative Gas")],
            "Time (days)",
            "Cumulative Gas (Bscf)",
        ...
        fig6 = make_line_figure(
            time_axis,
            [
                (np.asarray(history.cum_condensate_mstb), "Cumulative Condensate"),
                (np.asarray(history.cum_water_mstb), "Cumulative Water"),
            ],
            "Time (days)",
            "Cumulative Liquid (MSTB)",
            "Cumulative Condensate and Water Production",
        )
        st.pyplot(fig6)
        plt.close(fig6)

        fig7 = make_line_figure(
            time_axis,
            [
                (np.asarray(history.well_damage_factor), "Damage Factor"),
                (np.asarray(history.productivity_loss_fraction), "Productivity Loss Fraction"),
            ],
            "Time (days)",
            "Dimensionless",
            "Condensate-Bank Impairment",
        )
        st.pyplot(fig7)
        plt.close(fig7)

        fig8 = make_line_figure(
            time_axis,
            [(np.asarray(history.avg_sw), "Average Sw")],
            "Time (days)",
            "Average Sw",
            "Average Water Saturation History",
        )
        st.pyplot(fig8)
        plt.close(fig8)

        fig9 = make_line_figure(
            time_axis,
            [
                (np.abs(np.asarray(history.hc_mass_balance_error_lbmol)), "|HC Mass Balance Error|"),
                (np.abs(np.asarray(history.water_mass_balance_error_lbmol)), "|Water Mass Balance Error|"),
            ],
            "Time (days)",
            "Absolute Error (lbmol)",
            "Mass Balance Error History",
        )
        st.pyplot(fig9)
        plt.close(fig9)

    with result_tabs[2]:
        fig10 = make_line_figure(
            np.asarray(final_diag["x_ft"]),
            [(np.asarray(final_diag["pressure_psia"]), "Pressure")],
            "Distance (ft)",
            "Pressure (psia)",
        ...
        fig11 = make_line_figure(
            np.asarray(final_diag["x_ft"]),
            [(np.asarray(final_diag["dropout_indicator"]), "Dropout Indicator")],
            "Distance (ft)",
            "Dropout Indicator",
            "Final Condensate Dropout Profile",
        )
        st.pyplot(fig11)
        plt.close(fig11)

        fig12 = make_line_figure(
            np.asarray(final_diag["x_ft"]),
            [
                (np.asarray(final_diag["krg"]), "krg"),
                (np.asarray(final_diag["kro"]), "kro"),
                (np.asarray(final_diag["krw"]), "krw"),
            ],
            "Distance (ft)",
            "Relative Permeability",
            "Final Three-Phase Relative Permeability Profile",
        )
        st.pyplot(fig12)
        plt.close(fig12)

        fig13 = make_line_figure(
            np.asarray(final_diag["x_ft"]),
            [
                (np.asarray(final_diag["Sg"]), "Sg"),
                (np.asarray(final_diag["So"]), "So"),
                (np.asarray(final_diag["Sw"]), "Sw"),
            ],
            "Distance (ft)",
            "Saturation",
            "Final Saturation Profile",
        )
        st.pyplot(fig13)
        plt.close(fig13)

        fig14 = make_line_figure(
            np.asarray(final_diag["x_ft"]),
            [(np.asarray(final_diag["dew_point_minus_pressure_psia"]), "Dew Point - Pressure")],
            "Distance (ft)",
            "Dew Point - Pressure (psia)",
            "Final Dew-Point Margin Profile",
        )
        st.pyplot(fig14)
        plt.close(fig14)
    ...
    with result_tabs[4]:
        ...
        st.download_button(
            "Download final_spatial_diagnostics.csv",
            spatial_df.to_csv(index=False).encode("utf-8"),
            file_name="final_spatial_diagnostics.csv",
            mime="text/csv",
        )
        st.download_button(
            "Download chapter4_summary.csv",
            summary_df.to_csv(index=False).encode("utf-8"),
            file_name="chapter4_summary.csv",
            mime="text/csv",
        )
        ...
        st.caption(
            ...
            "Pressure and transport remain prototype formulations and should be used for comparative scenario analysis rather than high-fidelity field forecasting."
        )
