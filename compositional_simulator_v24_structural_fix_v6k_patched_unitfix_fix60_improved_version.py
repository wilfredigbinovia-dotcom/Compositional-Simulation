...
class PengRobinsonEOS:
    ...
    def fugacity_coefficients(self, x: np.ndarray, P: float, T: float, phase: str) -> np.ndarray:
        ...
        # sum_aij[i] = Σ_j aij[i,j] * x[j]  — already O(nc²) in mixture_params
        sum_aij = self._cached_aij @ x
        sqrt2 = math.sqrt(2.0)
...
def phase_viscosity_cp(
    ...
    """
    Lohrenz-Bray-Clark (LBC) viscosity correlation using Stiel-Thodos dilute-gas
    viscosities and Herning-Zipperer mixing.

    Parameters
    ----------
    Tc_R, Pc_psia, Mw_lbmol : real component critical properties from the EOS
        fluid model. If not supplied, linspace approximations are used as fallback.
    """
    ...
    # Use real component properties if supplied; fall back to linspace approximations
    Tc  = Tc_R     if Tc_R     is not None else np.linspace(343.1, 1100.0, n)
    Pc  = Pc_psia  if Pc_psia  is not None else np.linspace(667.8,  250.0, n)
    Mw  = Mw_lbmol if Mw_lbmol is not None else np.linspace( 16.04, 200.0, n)

    # Stiel-Thodos parameter ξ and dilute-gas viscosity per component
...
class CompositionalSimulator1D:
    ...
    def tubing_pressure_profile_from_response(self, state: SimulationState, response: Dict[str, float | np.ndarray]) -> Dict[str, float]:
        ...
        if self.well.tubing_model == "simple":
            ...
            mu_mix_cp = (
                gas_frac * phase_viscosity_cp("v", y, pwf, self.T, self.Tc, self.Pc, self.mw_components)
                + oil_frac * phase_viscosity_cp("l", x, pwf, self.T, self.Tc, self.Pc, self.mw_components)
    ...
    def step_once(self, state: SimulationState, dt_days: float) -> Tuple[SimulationState, Dict[str, float]]:
        ...
        # accuracy improvement given the explicit transport scheme and
        # the 0.5 under-relaxation already in pressure_update.
        ...
        summary = {
            ...
            "avg_pressure": float(np.mean(p_pred)),
            "min_pressure": float(np.min(p_pred)),
            ...
            "left_boundary_pressure_psia": float(self.boundary.left_pressure_psia if self.boundary.left_mode == "constant_pressure" else p_pred[0]),
            "right_boundary_pressure_psia": float(self.boundary.right_pressure_psia if self.boundary.right_mode == "constant_pressure" else p_pred[-1]),
        }
    ...
    def record_mass_balance(self, history: SimulationHistory, state: SimulationState, dt_days: float, q_total: float, gas_frac: float, oil_frac: float, qw_lbmol_day: float, injector_rate_lbmol_day: float) -> None:
    ...
    def _record_history_step(
        ...
        self.record_mass_balance(history, state, mass_balance_dt, q_total, gas_frac, oil_frac, qw_lbmol_day, injector_rate_lbmol_day)
