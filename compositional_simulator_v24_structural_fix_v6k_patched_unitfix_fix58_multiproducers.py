...
class PengRobinsonEOS:
    ...
    def solve_cubic(A: float, B: float) -> np.ndarray:
        ...
        # Use relative imaginary threshold instead of np.isreal, which uses an
        # absolute tolerance of 1e-10 that can misclassify roots near critical point.
        rel_tol = 1e-8 * np.maximum(np.abs(roots.real), 1.0)
        real_roots = roots[np.abs(roots.imag) < rel_tol].real
        # Reject unphysical roots: Z must be > B (from EOS derivation)
...
class FlashCalculator:
    ...
    def solve_beta(self, z: np.ndarray, K: np.ndarray) -> float:
        ...
        try:
            return float(brentq(lambda b: self.rachford_rice(b, z, K), 0.0, 1.0, xtol=1e-10, maxiter=100))
        except ValueError:
            # Fallback: bracket not found, return midpoint
            return 0.5
...
def phase_viscosity_cp(phase: str, comp: np.ndarray, pressure_psia: float | None = None, temperature_R: float | None = None) -> float:
    ...
    For the gas phase the dilute-gas viscosity of each component is estimated
    using the Stiel-Thodos method, then mixed via the Herning-Zipperer rule,
    and finally the density correction term of LBC is applied.

    For the liquid phase the same framework is used but the dense-fluid
    correction is more significant.

    Falls back to the original heuristic when critical properties are unavailable.
    """
    ...
    # Use approximate critical properties scaled from comp index when EOS Tc/Pc
    # are not passed. These are reasonable for a C1-heavy gas condensate system.
    Tc_approx = np.linspace(343.1, 1100.0, n)    # R  (C1=343.1 R, C7+=~1100 R)
    Pc_approx = np.linspace(667.8, 250.0, n)     # psia
    Mw_approx = np.linspace(16.04, 200.0, n)     # lbm/lbmol
    Vc_approx = np.linspace(1.59, 20.0, n)       # ft³/lbmol  (approx)

    T = float(temperature_R) if temperature_R is not None else 600.0
    P = float(pressure_psia) if pressure_psia is not None else 3000.0

    # Stiel-Thodos dilute-gas viscosity for each component (µ* in cP)
    Tr = T / Tc_approx
    xi = (Tc_approx ** (1.0 / 6.0)) / (Mw_approx ** 0.5 * Pc_approx ** (2.0 / 3.0))
    mu_stiel = np.where(
        Tr <= 1.5,
        (34e-5 * Tr ** 0.94) / xi,
        (17.78e-5 * (4.58 * Tr - 1.67) ** 0.625) / xi,
    )
    mu_stiel = np.clip(mu_stiel, 1e-4, 0.15)

    # Herning-Zipperer mixing rule for dilute-gas mixture viscosity
    sqrt_mw = np.sqrt(Mw_approx)
    denom_hz = np.sum(comp * sqrt_mw)
    mu_mix_dilute = float(np.sum(comp * mu_stiel * sqrt_mw) / max(denom_hz, 1e-12))

    if phase == "v":
        # LBC density correction — approximate reduced density from ideal-gas law
        rho_r = P / (max(T, 1.0) * 10.73)  # rough reduced density proxy
        rho_r = min(rho_r, 0.9)
        a = [0.1023, 0.023364, 0.058533, -0.040758, 0.0093324]
        bracket = a[0] + a[1]*rho_r + a[2]*rho_r**2 + a[3]*rho_r**3 + a[4]*rho_r**4
        xi_mix = float(np.sum(comp * xi))
        mu_lbc = mu_mix_dilute + (bracket**4 - 1e-4) / max(xi_mix, 1e-12)
        return float(np.clip(mu_lbc, 0.008, 0.12))

    if phase == "l":
        # Liquid: denser fluid → larger correction
        rho_r = min(P / (max(T, 1.0) * 3.0), 2.5)
        a = [0.1023, 0.023364, 0.058533, -0.040758, 0.0093324]
        bracket = a[0] + a[1]*rho_r + a[2]*rho_r**2 + a[3]*rho_r**3 + a[4]*rho_r**4
        xi_mix = float(np.sum(comp * xi))
        mu_lbc = mu_mix_dilute + (bracket**4 - 1e-4) / max(xi_mix, 1e-12)
        return float(np.clip(mu_lbc, 0.05, 3.0))
...
class CompositionalSimulator1D:
    ...
    def pressure_update(self, state: SimulationState, dt_days: float) -> np.ndarray:
        ...
        # Total compressibility: fluid compressibility + rock compressibility (cr).
        # Rock compressibility is taken from Rock.compressibility_1_psi if present,
        # defaulting to 3e-6 1/psi (typical sandstone) otherwise.
        cr = getattr(self.rock, "compressibility_1_psi", 3.0e-6)
        ct = 1.0e-5 + cr  # fluid compressibility estimate + rock

        acc_scale = ct * self.pv / max(dt_days, 1e-12)

        # Build the tridiagonal system in banded form (ab shape: 3 x n).
        # Row 0: superdiagonal (ab[0, 1:])
        # Row 1: main diagonal  (ab[1, :])
        # Row 2: subdiagonal    (ab[2, :-1])
        ab = np.zeros((3, n), dtype=float)
        b = np.zeros(n, dtype=float)

        # Accumulation terms on main diagonal
        ab[1, :] += acc_scale
        b += acc_scale * p_old

        # Inter-cell transmissibilities.
        # UNIT NOTE: transmissibility() returns md·ft (mobility units); the 1e-3
        # factor converts to field-unit pressure/rate coupling (psia·lbmol/day).
        _TRANS_SCALE = 1.0e-3
        for i in range(n - 1):
            Tij = self.transmissibility(state, i, i + 1) * _TRANS_SCALE
            ab[1, i] += Tij
            ab[1, i + 1] += Tij
            ab[0, i + 1] -= Tij   # superdiagonal at col i+1
            ab[2, i] -= Tij        # subdiagonal at col i

        # Boundary conditions
        _BOUNDARY_WELL_SCALE = 1.0e-3
        ...
        if self.aquifer.enabled:
            ...
            p_aq = self.aquifer_current_pressure_psia()
            # Scale: aquifer PI (lbmol/day/psi) * unit conversion to match pressure system
            _AQUIFER_SCALE = 2.0e-4
            Jaq = self.aquifer.productivity_index_lbmol_day_psi * _AQUIFER_SCALE
            ab[1, ai] += Jaq
            b[ai] += Jaq * p_aq

        # Producer well coupling — add one diagonal source/sink term per well.
        # Temporarily swap self.well to each producer so that solve_well_control
        # and peaceman_well_index operate on the correct cell and parameters.
        _WELL_PROD_SCALE = 2.5e-3
        orig_well = self.well
        for _w in self.wells:
            ...
            _Jw = self.peaceman_well_index(_wi) * _mob["lam_t"] * _WELL_PROD_SCALE
        ...
        # Injector well coupling
        _WELL_INJ_SCALE = 5.0e-3
        injector = self.active_injector()
        ...
        if injector is not None and self.scenario.name != "natural_depletion" and actual_inj_rate > 0.0:
            ...
            Jin = self.peaceman_well_index(ii) * inj_mob["lam_t"] * _WELL_INJ_SCALE
        ...
        # Solve the banded tridiagonal system — O(n) instead of O(n³)
        try:
            p_solved = solve_banded((1, 1), ab, b)
        except Exception:
            # Fallback to direct solve if banded solver fails (should be rare)
    ...
    def transport_update(self, state: SimulationState, dt_days: float) -> TransportResult:
        ...
        for _ in range(n_sub):
            ...
            for i in range(self.grid.nx - 1):
                ...
                # CFL check: warn if explicit update may be unstable
                pv_min = min(self.pv[i], self.pv[i + 1])
                hc_flux_mag = float(np.sum(np.abs(hc_flux)))
                if pv_min > 1e-12 and hc_flux_mag > 0.0:
                    cfl = hc_flux_mag * dt_sub / pv_min
                    if cfl > 0.8:
                        warnings.warn(
                            f"CFL={cfl:.2f} > 0.8 at interface ({i},{i+1}); "
                            "consider reducing the timestep for numerical stability.",
                            RuntimeWarning,
                            stacklevel=3,
                        )
    ...
    def _record_history_step(
        ...
        self.record_mass_balance(history, state, mass_balance_dt, q_total, gas_frac, oil_frac, water_frac, injector_rate_lbmol_day)
    ...
    def run(self, state0: SimulationState, t_end_days: float, dt_days: float) -> Tuple[SimulationState, SimulationHistory]:
        ...
        while t <= t_end_days + 1e-12:
            ...
            # Update dew point dynamically each timestep from well-cell composition.
            # In gas cycling the injection alters reservoir z, so a static dew point
            # would be wrong; re-flashing at current conditions tracks the phase envelope.
            wi = self.well.cell_index
            avg_z = state.z[wi]
            try:
                fl_dew = self.flash.flash(avg_z, state.pressure[wi], self.T)
                if fl_dew["state"] == "two_phase":
                    # Pressure is below dew point — dew point is above current pressure.
                    # Use the PVT table value as a reference; the EOS flash confirms two-phase.
                    dew_point = max(self.pvt.dew_point_psia, float(state.pressure[wi]))
                else:
            ...
            if next_dt <= 1e-12:
                ...
                qw_lbmol_day = q_total_0 * water_frac_0
                ...
                self._record_history_step(
                    ...
                    water_frac=water_frac_0, dropout=dropout_0,
                    ...
                    summary=None, accepted_dt=0.0, retries=0, mass_balance_dt=0.0,
                )
                break
            ...
            self._record_history_step(
                ...
                water_frac=water_frac, dropout=dropout,
...
def compute_volumetrics(
    ...
    Parameters
    ----------
    pv_cells : per-cell pore volume array from the simulator (sim.pv).
               Using the exact cell sum rather than GRV × φ ensures consistency
               with the simulator's internal accounting for any local grid refinement.
    sw_init  : scalar initial water saturation (uniform) as entered by the user.
               Using the input value rather than mean(state0.sw) keeps the
               volumetric calculation independent of numerical initialisation effects.

    Bgi source priority
    -------------------
    1. PVT table ``gas_fvf_ft3_scf`` column (user-uploaded, most accurate)
    2. PR-EOS Z-factor: Bgi = (P_std/T_std) × (Zi × T / Pi)

    CIIP source priority
    --------------------
    1. PVT table ``vaporized_cgr_stb_mmscf``
    2. PVT table ``reservoir_cgr_stb_mmscf``
    3. EOS flash of average initial composition (liquid fraction → STB)
    If none available, CIIP is reported as NaN.
    """
    # -----------------------------------------------------------------
    # GRV — Gross Rock Volume
    # GRV = length × area_ft2 = L × (π r h) = π r² h  (radial geometry)
    # -----------------------------------------------------------------
    grv_ft3 = float(grid.length_ft * grid.area_ft2)

    # Drainage area — back-calculated from grid geometry
    # area_ft2 = π r h, so π r² = area_ft2 × length_ft / h
    drainage_area_ft2   = grid.area_ft2 * grid.length_ft / max(float(grid.thickness_ft), 1e-6)
...
def run_example(
    ...
    def _build_well(cfg: dict, idx: int) -> Well:
        ...
        object.__setattr__(w, "name", str(cfg.get("name", f"Well {idx}")))
        return w
...
def _timed_run_example(
    ...
    every 0.25 s with a live MM:SS elapsed timer and a progress fraction.

    Parameters
    ----------
    kwargs      : keyword arguments forwarded to run_example
    label       : short human-readable description of this run (shown in the timer)
    current     : 1-based index of this run within the batch
    total       : total number of runs in the batch
    placeholder : a Streamlit empty() element used for live updates

    Returns the result dict from run_example, or raises on failure.
    """
    import threading
    ...
    # Show a brief per-run completion notice (caller clears the placeholder after
    # the batch is done, so this is visible only momentarily for multi-run batches)
    batch_info = f" ({current}/{total})" if total > 1 else ""
    placeholder.success(
        f"✅ **{label}**{batch_info} — done in **{elapsed_total:.1f} s**"
    )
    return _result
...
def display_streamlit_app() -> None:
    ...
    with tab_producer_well:
        ...
        for _wi in range(n_producers):
            ...
            with st.expander(f"Well {_wi + 1}", expanded=(_wi == 0)):
                ...
                _bhp   = float(st.number_input("BHP target (psia)",          min_value=100.0,  max_value=20000.0, value=float(st.session_state.get(f"{_wk}_bhp",  3000.0)), step=100.0,  disabled=(_ctrl != "bhp"),      key=f"{_wk}_bhp"))
                _dd    = float(st.number_input("Drawdown target (psi)",       min_value=1.0,    max_value=10000.0, value=float(st.session_state.get(f"{_wk}_dd",    200.0)),  step=10.0,   disabled=(_ctrl != "drawdown"), key=f"{_wk}_dd"))
                _qg    = float(st.number_input("Target gas rate (MMscf/day)", min_value=0.0,    max_value=5000.0,  value=float(st.session_state.get(f"{_wk}_qg",    5.0)),    step=0.5,    disabled=(_ctrl != "gas_rate"), key=f"{_wk}_qg"))
                _thp   = float(st.number_input("THP target (psia)",           min_value=14.7,   max_value=10000.0, value=float(st.session_state.get(f"{_wk}_thp",   500.0)),  step=25.0,   disabled=(_ctrl != "thp"),      key=f"{_wk}_thp"))
                ...
                _tvd   = float(st.number_input("TVD (ft)",              min_value=100.0,  max_value=30000.0, value=float(st.session_state.get(f"{_wk}_tvd",  8000.0)),  step=100.0, key=f"{_wk}_tvd"))
                _tid   = float(st.number_input("Tubing ID (in)",        min_value=1.0,    max_value=6.0,     value=float(st.session_state.get(f"{_wk}_tid",  2.441)),   step=0.1,   key=f"{_wk}_tid"))
                ...
                _rw    = float(st.number_input("Wellbore radius (ft)",  min_value=0.05,   max_value=5.0,     value=float(st.session_state.get(f"{_wk}_rw",   0.35)),    step=0.05,  key=f"{_wk}_rw"))
                _pi    = float(st.number_input("PI multiplier",         min_value=0.01,   max_value=100.0,   value=float(st.session_state.get(f"{_wk}_pi",   1.0)),     step=0.1,   key=f"{_wk}_pi"))
    ...
    try:
        result = _timed_run_example(run_kwargs, "Compositional simulation", 1, 1, timer_placeholder)
    except Exception as e:
    ...
    if enable_scenario_comparison:
        ...
        if len(selected_comparison_labels) >= 2:
            ...
            for idx, label in enumerate(selected_comparison_labels):
                ...
                try:
                    comp_result = _timed_run_example(compare_kwargs, label, idx + 1, len(selected_comparison_labels), comp_timer)
    ...
    if enable_sensitivity:
        ...
        for sens_idx, (case_label, pct_change) in enumerate(pct_cases):
            ...
            try:
                sens_result = _timed_run_example(case_kwargs, f"{case_label} ({pct_change:+.0f}%)", sens_idx + 1, n_sens_cases, sens_timer)
    ...
    if enable_monte_carlo:
        ...
        if selected_mc:
            ...
            for run_idx in range(n_mc_runs):
                ...
                try:
                    mc_result = _timed_run_example(case_kwargs, f"MC run {run_idx+1}", run_idx + 1, n_mc_runs, mc_timer)
