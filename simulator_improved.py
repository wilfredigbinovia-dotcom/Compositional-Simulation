...
def display_streamlit_app() -> None:
    ...
    # Check that critical inputs have been supplied by the user (not at sentinel/min values)
    _sentinel_checks = [
        (float(radius_ft)      <= 1.0,    "Reservoir / Grid → Drainage radius must be entered (currently at minimum)"),
        (float(thickness_ft)   <= 5.0,    "Reservoir / Grid → Reservoir thickness must be entered (currently at minimum)"),
        (float(porosity)       <= 0.01,   "Rock & Rock-Fluid → Porosity must be entered (currently at minimum)"),
        (float(permeability_md)<= 0.01,   "Rock & Rock-Fluid → Permeability must be entered (currently at minimum)"),
