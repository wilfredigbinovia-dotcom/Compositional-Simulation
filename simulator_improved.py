...
class TransportResult(NamedTuple):
    ...
    sg_max: np.ndarray          # per-cell maximum gas saturation (hysteresis tracker)
...
class FlashCalculator:
    ...
    def flash(self, z: np.ndarray, P: float, T: float, max_iter: int = 30, tol: float = 1e-6) -> Dict[str, np.ndarray | float | str]:
        ...
        if not converged:
            # Retry with fresh Wilson K-values — near the critical point the
            # cached K may be misleading. Double the iterations for the retry.
            K = K_wilson.copy()
            beta = self.solve_beta(z, K)
            for _ in range(max_iter):  # retry with max_iter more iterations
                denom = 1.0 + beta * (K - 1.0)
                x = z / denom; y = K * x
                ...
                if err < tol * 10:
                    ...
                    break
            if not converged:
                # Still not converged — don't cache, return best estimate
                denom = 1.0 + beta * (K - 1.0)
                x = z / denom; y = K * x
                x /= np.sum(x); y /= np.sum(y)
                return {"state": "two_phase", "beta": float(np.clip(beta, 0.0, 1.0)),
                        "x": x, "y": y, "K": K}
        ...
        x /= np.sum(x)
        y /= np.sum(y)
        # Only cache converged K-values to avoid warm-starting from bad near-critical K.
...
class SimulationHistory:
    ...
    early_termination_message: str = ""  # set when simulation stops before end date
...
class CompositionalSimulator1D:
    ...
    def drainage_lam_t(self, state: SimulationState, i: int) -> float:
        """Total mobility using drainage krg (no hysteresis) for inter-cell transmissibility."""
        fl = self.cell_flash_cached(state, i)
        beta = float(fl["beta"])
        x = np.asarray(fl["x"])
        y = np.asarray(fl["y"])
        pvt_props = self.pvt_lookup(state.pressure[i])
        mu_g = pvt_props.get("gas_viscosity_cp") or phase_viscosity_cp("v", y, state.pressure[i], self.T, self.Tc, self.Pc, self.mw_components)
        mu_o = pvt_props.get("oil_viscosity_cp") or phase_viscosity_cp("l", x, state.pressure[i], self.T, self.Tc, self.Pc, self.mw_components)
        mu_w = pvt_props.get("water_viscosity_cp") or WATER_VISCOSITY_CP
        Sw = float(np.clip(state.sw[i], 0.0, 0.95))
        Sg, So, Sw = beta_to_flowing_saturations(beta, Sw, self.relperm)
        krg, kro, krw = three_phase_relperm(Sg, So, Sw, self.relperm)
        lam_t = krg / max(mu_g, 1e-8) + kro / max(mu_o, 1e-8) + krw / max(mu_w, 1e-8)
        return max(lam_t, 1e-10)

    def transmissibility(self, state: SimulationState, i: int, j: int) -> float:
        # Use drainage mobility for inter-cell flow — hysteresis only affects well PI.
        lam_i = self.drainage_lam_t(state, i)
        lam_j = self.drainage_lam_t(state, j)
    ...
    def well_response_at_pwf(self, state: SimulationState, pwf_psia: float) -> Dict[str, float | np.ndarray]:
        ...
        Sg, So, Sw = beta_to_flowing_saturations(beta, Sw, self.relperm)
        krg, kro, krw = three_phase_relperm(Sg, So, Sw, self.relperm)
        # For hysteresis, use Sg from the reservoir-pressure flash (not pwf).
        # At very low pwf, the flash gives Sg >> sg_max causing spurious drainage.
        # sg_max tracks condensate at reservoir conditions — use consistent Sg.
        _fl_res = self.flash.flash(state.z[i], max(float(pr), 14.7), self.T)
        _beta_res = float(_fl_res.get("beta", 1.0))
        _Sg_res = max(1.0 - _beta_res, 0.0) * max(1.0 - Sw, 0.0)
        sg_max_cell = float(np.clip(state.sg_max[i] if hasattr(state, "sg_max") else _Sg_res, 0.0, 1.0))
        krg, kro, trap_fraction, imbibition_flag = apply_relperm_hysteresis(
            krg, kro, _Sg_res, sg_max_cell, self.hysteresis
        )
        ...
        # If hysteresis is enabled, krg is already reduced by the imbibition
        # curve to reflect condensate blocking — applying the additional
        # condensate bank damage factor would double-penalise. Only apply
        # the damage factor when hysteresis is disabled.
    ...
    def pressure_update(self, state: SimulationState, dt_days: float) -> np.ndarray:
        ...
        for _w in self.wells:
            ...
            if _ctrl == "gas_rate":
                ...
                # Neumann sink: only apply if well cell pressure is above min_bhp.
                # If the well cell has already been depleted below min_bhp by the
                # previous step, skip the sink — the well cannot produce more.
                _min_bhp_w = max(float(getattr(_w, "min_bhp_psia", 50.0)), 14.7)
                _p_well_cell = float(p_old[_wi])
                if _p_well_cell > _min_bhp_w * 1.05:
                    _qg_sink = float(_response.get("qg_lbmol_day", 0.0))
                    _qo_sink = float(_response.get("qo_lbmol_day", 0.0))
                    _q_sink  = _qg_sink + _qo_sink
                    # Cap at 2× target gas rate to prevent condensate-dominated collapse
                    _target_lbmol = getattr(_w, "target_gas_rate_mmscf_day", 5.0) * 1e6 * 14.7 / (R * 520.0)
                    _q_sink = min(_q_sink, max(_target_lbmol * 2.0, _qg_sink))
                    b[_wi] -= _q_sink
            else:
        ...
        # Clamp: pressure can never exceed initial reservoir pressure
        # and never drop below min_bhp floor.
        p_new = np.minimum(p_new, float(np.max(p_old)))  # no cell above step-start max
    ...
    def transport_update(self, state: SimulationState, dt_days: float) -> TransportResult:
        ...
        for i in range(nx - 1):
            ...
            if hc_flux_mag > 0.0:
                # CFL = flux [lbmol/day] / nt_cell [lbmol] — must be dimensionless.
                # Previously divided by pv [ft³] which gave wrong units and a
                # CFL number ~33× too large, forcing n_sub=261 when 3 would suffice.
                upstream = i if hc_flux[hc_flux != 0][0] > 0 else i + 1
                nt_upstream = max(float(state.nt[upstream]), 1e-12)
                max_cfl_per_day = max(max_cfl_per_day, hc_flux_mag / nt_upstream)

        CFL_TARGET = 0.5
        if max_cfl_per_day > 0.0:
            # Cap at 100 subcycles to keep runtime bounded; below that let CFL
            # dictate n_sub so transport remains stable regardless of geometry.
            n_sub = max(1, min(20, int(math.ceil(max_cfl_per_day * dt_days / CFL_TARGET))))
        else:
            n_sub = 1
        ...
        for _ in range(n_sub):
            ...
            for i in range(self.grid.nx - 1):
                ...
                # Flux limiter: cap so donor cell cannot go below zero.
                # This prevents clip-floor mass creation when CFL > 1.
                donor = i if np.sum(hc_flux) >= 0.0 else i + 1
                max_allowed = ncomp_new[donor].copy()  # current inventory
                applied_flux = np.where(
                    hc_flux > 0,
                    np.minimum(hc_flux, max_allowed / max(dt_sub, 1e-12)),
                    np.maximum(hc_flux, -max_allowed / max(dt_sub, 1e-12))
                ) if donor == i else np.where(
                    hc_flux < 0,
                    np.maximum(hc_flux, -max_allowed / max(dt_sub, 1e-12)),
                    np.minimum(hc_flux, max_allowed / max(dt_sub, 1e-12))
                )

                ncomp_new[i] -= dt_sub * applied_flux
                ncomp_new[i + 1] += dt_sub * applied_flux
                ...
                transport_total_flux += float(np.sum(np.abs(applied_flux)))
            ...
            # Compute Sg using step-start pressure but updated composition from
            # transport. Step-start pressure is stable (accepted state), preventing
            # sg_max from tracking numerical p_pred oscillations in the well cell.
            _sw_arr = provisional_state.sw
            _shc = np.clip(1.0 - _sw_arr, 0.0, 1.0)
            _sg_transport = np.zeros(self.grid.nx)
            for _ci in range(self.grid.nx):
                try:
                    # Use min of step-start and current work pressure — the lower
                    # value is more likely to be below dew point and give correct Sg.
                    _p_stable = max(min(float(state.pressure[_ci]), float(work_state.pressure[_ci])), 14.7)
                    _fl = self.flash.flash(provisional_state.z[_ci], _p_stable, self.T)
                    _beta = float(_fl.get("beta", 1.0))
                except Exception:
                    _beta = float(self.cell_flash_cached(work_state, _ci).get("beta", 1.0))
                _sg_transport[_ci] = max(1.0 - _beta, 0.0) * float(_shc[_ci])
            work_state = SimulationState(
                ...
                sg_max=np.maximum(work_state.sg_max, _sg_transport),
            )
    ...
    def validate_state(self, state: SimulationState, prev_state: "SimulationState | None" = None) -> tuple[bool, str]:
        ...
        # Pressure drop validation removed — auto dt-reducer handles timestep control.
        return True, "ok"
    ...
    def step_once(self, state: SimulationState, dt_days: float) -> Tuple[SimulationState, Dict[str, float]]:
        ...
        # Use p_pred directly from the implicit pressure solver.
        ...
        # Final sg_max update using p_corrected — ensures consistency with
        # phase_mobility_data which will use new_state.pressure next step.
        _sg_max_final = tr.sg_max.copy()
        for _ci in range(self.grid.nx):
            try:
                _p_final = max(float(p_corrected[_ci]), 14.7)
                _fl_final = self.flash.flash(tr.z[_ci], _p_final, self.T)
                _beta_f = float(_fl_final.get("beta", 1.0))
                _sw_f = max(float(tr.sw[_ci]), 0.0)
                _sg_f = max(1.0 - _beta_f, 0.0) * max(1.0 - _sw_f, 0.0)
                _sg_max_final[_ci] = max(_sg_max_final[_ci], _sg_f)
            except Exception:
                pass
    ...
    def adaptive_step(self, state: SimulationState, dt_days: float, min_dt_days: float = 1e-3, max_retries: int = 10) -> Tuple[SimulationState, Dict[str, float], float, int]:
        ...
        while retries <= max_retries:
            try:
                ...
                ok, msg = self.validate_state(new_state, prev_state=state)
                if ok:
    ...
    def run(self, state0: SimulationState, t_end_days: float, dt_days: float,
             economic_gas_limit_mmscfd: float = 0.1,
             economic_limit_consecutive_steps: int = 3) -> Tuple[SimulationState, SimulationHistory]:
        if dt_days <= 0.0:
        ...
        while t <= t_end_days + 1e-12:
            ...
            # Auto-reduce timestep when krg is below its initial drainage value.
            # Any krg < krg0 means condensate has started dropping out and we
            # need finer steps to track the transition without pressure collapse.
            _krg_now = float(krg_0)
            _krg0_ref = float(getattr(self.relperm, 'krg0', 1.0))
            if _krg_now < _krg0_ref * 0.99:  # includes krg=0
                _krg_factor = max(_krg_now / max(_krg0_ref, 1e-6), 0.25)
                next_dt = min(next_dt, dt_days * _krg_factor)
                next_dt = max(next_dt, max(dt_days * 0.25, 1.0))
            if next_dt <= 1e-12:
            ...
            # Use step-start separator rates for reporting — less noisy than
            # trapezoid average which can spike when end-of-step krg drops suddenly.
            sep_rates_0 = self.separator_rates(response0)
            gas_rate_mmscf_day = float(sep_rates_0["gas_rate_mmscf_day"])
            condensate_rate_stb_day = float(sep_rates["condensate_rate_stb_day"])
            ...
            # Economic limit check — stop when gas rate is below limit for
            # a sustained period (90 days by default), regardless of timestep size.
            _last_qg = float(history.gas_rate_mmscf_day[-1]) if history.gas_rate_mmscf_day else 0.0
            _economic_duration_days = economic_limit_consecutive_steps * 30.0  # 3 steps × 30 days = 90 days
            if economic_gas_limit_mmscfd > 0.0 and _last_qg < economic_gas_limit_mmscfd:
                _below_limit_count += accepted_dt  # accumulate days below limit
                if _below_limit_count >= _economic_duration_days:
                    history.early_termination_message = (
                        f"⚠️ Well reached economic limit at {t/365.25:.1f} years: "
                        f"gas rate ({_last_qg:.3f} MMscfd) fell below {economic_gas_limit_mmscfd} MMscfd "
                        f"for {_below_limit_count:.0f} consecutive days."
                    )
                    break
            else:
                _below_limit_count = 0.0
...
def run_example(
    ...
    final_state, history = sim.run(state0, t_end_days=t_end_days, dt_days=dt_days,
                                     economic_gas_limit_mmscfd=economic_gas_limit_mmscfd)
...
def display_streamlit_app() -> None:
    ...
    with tab_case_setup:
        ...
        economic_gas_limit_mmscfd = st.number_input("Economic gas rate limit (MMscfd)", min_value=0.0, max_value=100.0, value=float(st.session_state.get("economic_gas_limit_mmscfd", 0.1)), step=0.05, format="%.3f", key="economic_gas_limit_mmscfd", help="Simulation stops automatically when gas rate falls below this value for 3 consecutive reporting steps. Set to 0 to disable.")
    ...
    _sentinel_checks = [
        ...
        *[
        ...
        # Cross-check: bhp must be >= min_bhp only when on BHP control
        *[
            (w["control_mode"] == "bhp" and float(w["bhp_psia"]) < float(w["min_bhp_psia"]),
             f"Producer Well {_wi+1} → BHP target ({w['bhp_psia']:.0f} psia) must be ≥ Minimum BHP ({w['min_bhp_psia']:.0f} psia)")
            for _wi, w in enumerate(wells_config)
        ],
    ]
    ...
    run_kwargs = dict(
        economic_gas_limit_mmscfd=float(economic_gas_limit_mmscfd),
    ...
    with tab_results:
        ...
        if result and getattr(result.get("history"), "early_termination_message", ""):
            st.warning(result["history"].early_termination_message)
