...
def display_streamlit_app() -> None:
    ...
    with input_tabs[2]:
        ...
        with c3:
            ...
            st.subheader("Optional REVEAL PVT Table")
            pvt_file = st.file_uploader("Upload PVT CSV", type=["csv"])
            pvt_table = None
            if pvt_file is not None:
                try:
                    pvt_df = pd.read_csv(pvt_file)
                    pvt_table = load_pvt_table_from_dataframe(pvt_df)
                    st.success(f"Loaded PVT table with {len(pvt_table.pressure_psia)} pressure rows.")
                    ...
                    st.error(f"Failed to load PVT CSV: {e}")
                    return
