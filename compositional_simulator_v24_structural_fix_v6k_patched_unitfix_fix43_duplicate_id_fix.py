...
class CompositionalSimulator1D:
    ...
    def tubing_pressure_profile_from_response(self, state: SimulationState, response: Dict[str, float | np.ndarray]) -> Dict[str, float]:
        ...
        if self.well.tubing_model == "simple":
            ...
            gas_vol_ft3_day = max(gas_rate_mmscf_day, 0.0) * 1.0e6
            liquid_vol_ft3_day = max(liquid_rate_stb_day, 0.0) * FT3_PER_STB
            mix_vol_ft3_day = gas_vol_ft3_day + liquid_vol_ft3_day
            velocity_ft_s = mix_vol_ft3_day / max(area_ft2 * 86400.0, 1e-12)
            ...
            mu_mix_cp = (
                gas_frac * phase_viscosity_cp("v", y, pwf, self.T)
                + oil_frac * phase_viscosity_cp("l", x, pwf, self.T)
            ...
            if reynolds < 1e-8:
                ...
                friction_factor = 0.25 / (math.log10(max(rel_roughness / 3.7 + 5.74 / (reynolds ** 0.9), 1e-12)) ** 2)

            friction_grad_psi_ft = friction_factor * rho_mix * max(velocity_ft_s, 0.0) ** 2 / max(2.0 * 32.174 * diameter_ft * 144.0, 1e-12)
            friction_psi = friction_grad_psi_ft * self.well.tvd_ft
            accel_psi = 0.02 * self.well.tubing_calibration_factor * friction_psi
            friction_psi = self.well.tubing_calibration_factor * friction_psi + self.well.thp_friction_coeff * (gas_rate_mmscf_day + 0.001 * liquid_rate_stb_day) ** 2
...
def display_streamlit_app() -> None:
    ...
    st.sidebar.caption("Model inputs are grouped into expandable sidebar sections. The target workflow sections are: Case Setup, Reservoir / Grid, Components / Composition, PVT / Separator, Rock / Rock-Fluid, Initial Conditions, Producer Well, Injector Well, Aquifer / Boundaries, Advanced Physics, Analysis Tools, and Results.")

    input_validation_errors: List[str] = []
    pvt_table = None

    with st.sidebar.expander("Case Setup", expanded=True):
    ...
    with st.sidebar.expander("Reservoir / Grid", expanded=False):
        ...
        radius_ft = st.number_input("Equivalent drainage radius (ft)", min_value=1.0, max_value=100000.0, value=100.0, step=10.0)
    ...
    with st.sidebar.expander("Components / Composition", expanded=False):
    ...
    with st.sidebar.expander("PVT / Separator", expanded=False):
        temperature_F = st.number_input("Reservoir temperature (°F)", min_value=-60.0, max_value=740.0, value=220.0, step=10.0)
        ...
        separator_stages = int(st.selectbox("Separator train", options=[1, 2, 3, 4], index=3, format_func=lambda x: {1: "1-stage separator", 2: "2-stage separator", 3: "3-stage separator", 4: "Stage 1 + Stage 2 + Stage 3 + Stock Tank"}[x]))
        separator_pressure_psia = st.number_input("Stage 1 separator pressure (psia)", min_value=14.7, max_value=5000.0, value=300.0, step=25.0)
        separator_temperature_F = st.number_input("Stage 1 separator temperature (°F)", min_value=-10.0, max_value=340.0, value=60.0, step=5.0)
        separator_temperature_R = fahrenheit_to_rankine(separator_temperature_F)
        separator_second_stage_pressure_psia = st.number_input("Stage 2 separator pressure (psia)", min_value=14.7, max_value=5000.0, value=100.0, step=10.0, disabled=separator_stages < 2)
        separator_second_stage_temperature_F = st.number_input("Stage 2 separator temperature (°F)", min_value=-10.0, max_value=340.0, value=80.0, step=5.0, disabled=separator_stages < 2)
        separator_second_stage_temperature_R = fahrenheit_to_rankine(separator_second_stage_temperature_F)
        separator_third_stage_pressure_psia = st.number_input("Stage 3 separator pressure (psia)", min_value=14.7, max_value=5000.0, value=50.0, step=5.0, disabled=separator_stages < 3)
        separator_third_stage_temperature_F = st.number_input("Stage 3 separator temperature (°F)", min_value=-10.0, max_value=340.0, value=70.0, step=5.0, disabled=separator_stages < 3)
        separator_third_stage_temperature_R = fahrenheit_to_rankine(separator_third_stage_temperature_F)
        stock_tank_pressure_psia = st.number_input("Stock tank pressure (psia)", min_value=14.7, max_value=100.0, value=14.7, step=1.0, disabled=separator_stages < 4)
        stock_tank_temperature_F = st.number_input("Stock tank temperature (°F)", min_value=-10.0, max_value=340.0, value=60.0, step=5.0, disabled=separator_stages < 4)
        stock_tank_temperature_R = fahrenheit_to_rankine(stock_tank_temperature_F)
        gas_specific_gravity = st.number_input("Gas specific gravity (air = 1)", min_value=0.1, max_value=3.0, value=0.65, step=0.01, format="%.2f")
        condensate_api_gravity = st.number_input("Condensate API gravity", min_value=1.0, max_value=100.0, value=50.0, step=1.0, format="%.1f")
    ...
    with st.sidebar.expander("Rock / Rock-Fluid", expanded=False):
    ...
    with st.sidebar.expander("Initial Conditions", expanded=False):
    ...
    with st.sidebar.expander("Producer Well", expanded=False):
        well_control_label = st.selectbox("Producer control mode", ["Fixed BHP", "Fixed Drawdown", "Fixed Gas Rate", "Fixed THP"], index=0)
        control_map = {
            "Fixed BHP": "bhp",
        ...
        bhp_psia = st.number_input("Bottom-hole pressure target (psia)", min_value=100.0, max_value=20000.0, value=3000.0, step=100.0, disabled=(well_control_mode != "bhp"))
        drawdown_psia = st.number_input("Pressure drawdown target (psi)", min_value=1.0, max_value=10000.0, value=200.0, step=10.0, disabled=(well_control_mode != "drawdown"))
        target_gas_rate_mmscf_day = st.number_input("Target gas rate (MMscf/day)", min_value=0.0, max_value=5000.0, value=5.0, step=0.5, disabled=(well_control_mode != "gas_rate"))
        thp_psia = st.number_input("Tubing head pressure target (psia)", min_value=14.7, max_value=10000.0, value=500.0, step=25.0, disabled=(well_control_mode != "thp"))
        min_bhp_psia = st.number_input("Minimum flowing BHP allowed (psia)", min_value=50.0, max_value=5000.0, value=50.0, step=25.0, disabled=(well_control_mode != "drawdown"))
        ...
        tvd_ft = st.number_input("True vertical depth, TVD (ft)", min_value=100.0, max_value=30000.0, value=8000.0, step=100.0)
        tubing_id_in = st.number_input("Tubing ID (in)", min_value=1.0, max_value=6.0, value=2.441, step=0.1)
        wellhead_temperature_F = st.number_input("Wellhead temperature (°F)", min_value=-60.0, max_value=340.0, value=60.0, step=5.0)
        wellhead_temperature_R = fahrenheit_to_rankine(wellhead_temperature_F)
        tubing_model_label = st.selectbox("Tubing hydraulics model", ["Mechanistic proxy", "Simple coefficient"], index=0)
        tubing_model = "mechanistic" if tubing_model_label == "Mechanistic proxy" else "simple"
        thp_friction_coeff = st.number_input("THP friction coefficient", min_value=0.0, max_value=1.0, value=0.02, step=0.005, help="Retained as an empirical backpressure term and as the main loss term under Simple coefficient mode.")
        tubing_roughness_in = st.number_input("Tubing roughness (in)", min_value=0.0, max_value=0.05, value=0.0006, step=0.0002, format="%.4f", disabled=(tubing_model != "mechanistic"))
        tubing_calibration_factor = st.number_input("Tubing calibration factor", min_value=0.1, max_value=10.0, value=1.0, step=0.1, disabled=(tubing_model != "mechanistic"))

    with st.sidebar.expander("Injector Well", expanded=False):
        injection_rate_value = st.number_input("Gas injection rate", min_value=0.0, max_value=1.0e6, value=5.0, step=0.5, disabled=(scenario_name == "natural_depletion"))
        injection_rate_unit = st.selectbox("Gas injection rate unit", ["Mscf/day", "MMscf/day"], index=1)
        injection_control_label = st.selectbox("Injection mode", ["Simple schedule-driven", "Enhanced pressure-controlled"], index=1, disabled=(scenario_name == "natural_depletion"))
        injection_control_mode = "enhanced" if injection_control_label == "Enhanced pressure-controlled" else "simple"
        injection_pressure_psia = st.number_input("Injection pressure (psia)", min_value=14.7, max_value=20000.0, value=7000.0, step=100.0, disabled=(scenario_name == "natural_depletion"))
        max_injection_bhp_psia = st.number_input("Maximum bottom-hole injection pressure (psia)", min_value=14.7, max_value=25000.0, value=9000.0, step=100.0, disabled=(scenario_name == "natural_depletion" or injection_control_mode != "enhanced"))
        injectivity_index_mmscf_day_psi = st.number_input("Injectivity index (MMscf/day/psi)", min_value=0.0, max_value=1.0e4, value=gas_lbmol_day_to_mmscf_day(500.0), step=0.01, format="%.4f", disabled=(scenario_name == "natural_depletion" or injection_control_mode != "enhanced"))
        ...
        if scenario_name in ("gas_cycling", "lean_gas_injection"):
            injection_start_date_text = st.text_input("Injection start date (dd/mm/yyyy)", value=start_date_text)
            injection_end_date_text = st.text_input("Injection end date (dd/mm/yyyy)", value=end_date_text)
            try:
                inj_start_day = days_between_dates(start_date_text, injection_start_date_text, allow_same_day=True)
                inj_end_day = days_between_dates(start_date_text, injection_end_date_text, allow_same_day=True)
    ...
    with st.sidebar.expander("Aquifer / Boundaries", expanded=False):
        left_boundary_label = st.selectbox("Left boundary", ["Closed", "Constant pressure"], index=0)
        left_boundary_mode = "closed" if left_boundary_label == "Closed" else "constant_pressure"
        left_boundary_pressure_psia = st.number_input("Left boundary pressure (psia)", min_value=14.7, max_value=20000.0, value=5600.0, step=100.0, disabled=(left_boundary_mode != "constant_pressure"))
        left_boundary_transmissibility_multiplier = st.number_input("Left boundary transmissibility multiplier", min_value=0.0, max_value=100.0, value=1.0, step=0.1)
        right_boundary_label = st.selectbox("Right boundary", ["Closed", "Constant pressure"], index=0)
        right_boundary_mode = "closed" if right_boundary_label == "Closed" else "constant_pressure"
        right_boundary_pressure_psia = st.number_input("Right boundary pressure (psia)", min_value=14.7, max_value=20000.0, value=3000.0, step=100.0, disabled=(right_boundary_mode != "constant_pressure"))
        right_boundary_transmissibility_multiplier = st.number_input("Right boundary transmissibility multiplier", min_value=0.0, max_value=100.0, value=1.0, step=0.1)
        aquifer_enabled = st.checkbox("Enable analytic aquifer", value=False)
        aquifer_side = st.selectbox("Aquifer side", ["left", "right"], index=0, disabled=(not aquifer_enabled))
        aquifer_initial_pressure_psia = st.number_input("Aquifer initial pressure (psia)", min_value=14.7, max_value=20000.0, value=5600.0, step=100.0, disabled=(not aquifer_enabled))
        _aq_pi_default_stb_day_psi = 500.0 / water_lbmol_per_stb()
        _aq_cap_default_mstb_psi = 5.0e5 / water_lbmol_per_mstb()
        aquifer_productivity_index_stb_day_psi = st.number_input("Aquifer productivity index (STB/day/psi)", min_value=0.0, max_value=1.0e7, value=float(_aq_pi_default_stb_day_psi), step=1.0, format="%.2f", disabled=(not aquifer_enabled))
        aquifer_total_capacity_mstb_per_psi = st.number_input("Aquifer total capacity (MSTB/psi)", min_value=0.001, max_value=1.0e7, value=float(_aq_cap_default_mstb_psi), step=1.0, format="%.2f", disabled=(not aquifer_enabled))
        aquifer_water_influx_fraction = st.number_input("Aquifer influx fraction", min_value=0.0, max_value=1.0, value=1.0, step=0.05, format="%.2f", disabled=(not aquifer_enabled))
        aquifer_allow_backflow = st.checkbox("Allow aquifer backflow", value=False, disabled=(not aquifer_enabled))
    ...
    with st.sidebar.expander("Advanced Physics", expanded=False):
        capillary_enabled = st.checkbox("Enable capillary pressure", value=True)
        pcow_entry_psia = st.number_input("Pcow entry pressure (psia)", min_value=0.0, max_value=500.0, value=15.0, step=1.0)
        pcog_entry_psia = st.number_input("Pcog entry pressure (psia)", min_value=0.0, max_value=500.0, value=8.0, step=1.0)
        pc_lambda_w = st.number_input("Pcow exponent", min_value=0.1, max_value=10.0, value=2.0, step=0.1, format="%.1f")
        pc_lambda_g = st.number_input("Pcog exponent", min_value=0.1, max_value=10.0, value=2.0, step=0.1, format="%.1f")
        st.caption(f"Using Swir = {Swc:.2f}, Sorw = {Sorw:.2f}, Sgc = {Sgc:.2f}, Sorg = {Sorg:.2f}")
        hysteresis_enabled = st.checkbox("Enable gas-relperm hysteresis", value=True)
        hysteresis_reversal_tolerance = st.number_input("Reversal tolerance in Sg", min_value=0.0, max_value=0.30, value=0.01, step=0.005, format="%.3f")
        hysteresis_gas_trapping_strength = st.number_input("Gas trapping strength", min_value=0.0, max_value=1.0, value=0.60, step=0.05, format="%.2f")
        hysteresis_imbibition_krg_reduction = st.number_input("Imbibition krg reduction multiplier", min_value=0.0, max_value=1.5, value=0.75, step=0.05, format="%.2f")
        hysteresis_imbibition_kro_reduction = st.number_input("Imbibition kro reduction multiplier", min_value=0.0, max_value=1.0, value=0.15, step=0.05, format="%.2f")
        transport_enabled = st.checkbox("Enable dispersive transport", value=True)
        transport_phase_split_advection = st.checkbox("Use phase-split hydrocarbon advection", value=True)
        transport_dispersivity_ft = st.number_input("Longitudinal dispersivity (ft)", min_value=0.0, max_value=5000.0, value=15.0, step=1.0)
        transport_molecular_diffusion_ft2_day = st.number_input("Molecular diffusion (ft²/day)", min_value=0.0, max_value=100.0, value=0.15, step=0.05, format="%.3f")
        transport_max_dispersive_fraction = st.number_input("Max dispersive/advective flux ratio", min_value=0.0, max_value=1.0, value=0.35, step=0.05, format="%.2f")
    ...
    with st.sidebar.expander("PVT / Separator", expanded=False):
        ...
        with c1:
            temperature_F = st.number_input("Temperature (°F)", min_value=-60.0, max_value=740.0, value=220.0, step=10.0)
            temperature_R = fahrenheit_to_rankine(temperature_F)
            dew_point_psia = st.number_input("Dew-point pressure (psia)", min_value=100.0, max_value=20000.0, value=4200.0, step=100.0)

        with c2:
            st.subheader("Surface Separator Train")
            separator_stages = int(st.selectbox("Separator train", options=[1, 2, 3, 4], index=3, format_func=lambda x: {1: "1-stage separator", 2: "2-stage separator", 3: "3-stage separator", 4: "Stage 1 + Stage 2 + Stage 3 + Stock Tank"}[x]))
            separator_pressure_psia = st.number_input("Stage 1 separator pressure (psia)", min_value=14.7, max_value=5000.0, value=300.0, step=25.0)
            separator_temperature_F = st.number_input("Stage 1 separator temperature (°F)", min_value=-10.0, max_value=340.0, value=60.0, step=5.0)
            separator_temperature_R = fahrenheit_to_rankine(separator_temperature_F)
            ...
            st.subheader("Reporting Fluid Properties")
            gas_specific_gravity = st.number_input("Gas specific gravity (air = 1)", min_value=0.1, max_value=3.0, value=0.65, step=0.01, format="%.2f")
            condensate_api_gravity = st.number_input("Condensate API gravity", min_value=1.0, max_value=100.0, value=50.0, step=1.0, format="%.1f")
    ...
    with st.sidebar.expander("Analysis Tools", expanded=False):
        ...
        history_match_enabled = st.checkbox("Enable history-matching diagnostics", value=False)
        ...
        if history_match_enabled:
            ...
            with hm1:
                history_match_weights['avg_pressure_psia'] = st.number_input("Weight: Avg Pressure", min_value=0.0, max_value=100.0, value=1.0, step=0.25, disabled=not history_match_enabled)
            with hm2:
                history_match_weights['well_pressure_psia'] = st.number_input("Weight: Well Pressure", min_value=0.0, max_value=100.0, value=1.0, step=0.25, disabled=not history_match_enabled)
            with hm3:
                history_match_weights['gas_rate_mmscf_day'] = st.number_input("Weight: Gas Rate", min_value=0.0, max_value=100.0, value=1.0, step=0.25, disabled=not history_match_enabled)
            with hm4:
                history_match_weights['condensate_rate_stb_day'] = st.number_input("Weight: Condensate Rate", min_value=0.0, max_value=100.0, value=1.0, step=0.25, disabled=not history_match_enabled)
            hm5, hm6, hm7, hm8 = st.columns(4)
            with hm5:
                history_match_weights['water_rate_stb_day'] = st.number_input("Weight: Water Rate", min_value=0.0, max_value=100.0, value=1.0, step=0.25, disabled=not history_match_enabled)
            with hm6:
                history_match_weights['cum_gas_bscf'] = st.number_input("Weight: Cum Gas", min_value=0.0, max_value=100.0, value=0.75, step=0.25, disabled=not history_match_enabled)
            with hm7:
                history_match_weights['cum_condensate_mstb'] = st.number_input("Weight: Cum Condensate", min_value=0.0, max_value=100.0, value=0.75, step=0.25, disabled=not history_match_enabled)
            with hm8:
                history_match_weights['cum_water_mstb'] = st.number_input("Weight: Cum Water", min_value=0.0, max_value=100.0, value=0.75, step=0.25, disabled=not history_match_enabled)
        ...
        enable_scenario_comparison = st.checkbox("Enable side-by-side scenario comparison", value=False)
        ...
        enable_sensitivity = st.checkbox("Enable one-way sensitivity analysis", value=False)
        ...
        with sens1:
            sensitivity_low_pct = st.number_input("Low case change (%)", min_value=-95.0, max_value=0.0, value=-20.0, step=5.0, disabled=not enable_sensitivity)
        with sens2:
            sensitivity_base_pct = st.number_input("Base case change (%)", min_value=-95.0, max_value=500.0, value=0.0, step=5.0, disabled=not enable_sensitivity)
        with sens3:
            sensitivity_high_pct = st.number_input("High case change (%)", min_value=-95.0, max_value=500.0, value=20.0, step=5.0, disabled=not enable_sensitivity)
        ...
        enable_monte_carlo = st.checkbox("Enable Monte Carlo uncertainty analysis", value=False)
        ...
        with mc1:
            monte_carlo_runs = int(st.number_input("Monte Carlo runs", min_value=10, max_value=500, value=50, step=10, disabled=not enable_monte_carlo))
        with mc2:
            monte_carlo_low_pct = st.number_input("Low bound change (%)", min_value=-95.0, max_value=0.0, value=-15.0, step=5.0, disabled=not enable_monte_carlo)
        with mc3:
            monte_carlo_high_pct = st.number_input("High bound change (%)", min_value=0.0, max_value=500.0, value=15.0, step=5.0, disabled=not enable_monte_carlo)
        with mc4:
            monte_carlo_seed = int(st.number_input("Random seed", min_value=0, max_value=1000000, value=42, step=1, disabled=not enable_monte_carlo))
    ...
    with st.sidebar.expander("Results", expanded=False):
        st.caption("Simulation results, plots, tables, and downloads appear in the main Results section after you click Run Simulation.")

        run_clicked = st.button("Run Simulation", type="primary")

    if not run_clicked:
        st.info("Configure the inputs in the sidebar sections, then click 'Run Simulation'.")
        return
